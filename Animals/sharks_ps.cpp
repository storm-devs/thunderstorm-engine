...
// node ----------------------------------------------------------------------------
SHARKS_PS * SHARKS_PS::GetLeftNode() { return l_PTR; }
SHARKS_PS * SHARKS_PS::GetRightNode() {	return r_PTR; }
void SHARKS_PS::SetLeftNode(SHARKS_PS * node) {	l_PTR = node; }
void SHARKS_PS::SetRightNode(SHARKS_PS * node) { r_PTR = node; }
void SHARKS_PS::Attach(SHARKS_PS * * Root,SHARKS_PS * * Top)
{
	if(*Root == 0) 
	{
		*Root = this;
...
void SHARKS_PS::AttachTo(SHARKS_PS * la_PTR,SHARKS_PS * * Root,SHARKS_PS * * Top)
{
	SHARKS_PS * t_PTR;
...
void SHARKS_PS::Deattach(SHARKS_PS * * Root,SHARKS_PS * * Top)
{
...
void SHARKS_PS::ProcessOrder(SHARKS_PS * * Root,SHARKS_PS * * Top)
{
	if(r_PTR) 
	{
...
bool SHARKS_PS::Init(INIFILE * ini, char * psname, SEA_BASE *_sea)
{
	GUARD(SHARKS_PS::Init)
	if(!ini) return false;
	long n;
	bool bRes;
	...
	fTrackPointRadius = ini->GetFloat(psname,PSKEY_TRACKPOINTRADIUS,1.2f);
	...
	for(n=0;n<ParticlesNum;n++)
	{
		...
		Particle[n].pos.y = yDelta + Emitter.y + ESpace*(0.5f - (float)rand()/RAND_MAX);
...
void SHARKS_PS::UpdateVertexBuffer()
{
	CVECTOR ipos[4];
	CVECTOR rpos[4];
	CVECTOR pos;
...
void SHARKS_PS::Execute(dword DeltaTime)
{
...
void SHARKS_PS::LayOnSurface(dword index)
{
	COLLISION_OBJECT * pLink;
	CVECTOR from,to;
	float dist;
...
void SHARKS_PS::Realize(dword DeltaTime)
{
	if(Delay > 0) {	Delay = Delay - DeltaTime;	return;	}
/*
	if(bLinkEmitter)
...
*/
	...
	RenderService->SetCamera(&CameraPos,&CameraAng,Perspective);
	RenderService->SetTransform(D3DTS_VIEW,Matrix);
}


bool SHARKS_PS::EmitParticle()
{
	long n;
	for(n=0;n<ParticlesNum;n++)
	{
		...
		else
		{
			...
			Particle[n].pos.z = Emitter.z + ESpace*(0.5f - (float)rand()/RAND_MAX);
			//Particle[n].pos = pFlowTrack[0];
		}
	
		//_VSYSTEM_API->Trace("particle: %f,%f,%f", Particle[n].pos.x, Particle[n].pos.y, Particle[n].pos.z);
		...
		else
		{
			if (bUseFlowTrack)
			{
				Particle[n].ang = pFlowTrack[1] - Particle[n].pos;
			}
			else
			{
				Particle[n].ang.x = EmitterDirection.x + DirectionDeviation*(0.5f - (float)rand()/RAND_MAX);
		...
		Particle[n].spinVal = randUpper(Spin) + SpinDeviation * (0.5f - (float)rand()/RAND_MAX);
		Particle[n].spin = Particle[n].spinVal;
...
void SHARKS_PS::ProcessParticles(dword DeltaTime)
{
	long n;
	...
	for(n=0;n<ParticlesNum;n++)
	{
		...
		if(bUseFlowTrack)
		{
			...
			Particle[n].pos.y = yDelta + sea->WaveXZ(Particle[n].pos.x, Particle[n].pos.z);
		...
		else
		{
			...
			//Particle[n].pos.y += DeltaTime*(Particle[n].v.y + Particle[n].chaos.y);
			Particle[n].pos.z += DeltaTime*(Particle[n].v.z + Particle[n].chaos.z);
			Particle[n].pos.y = sea->WaveXZ(Particle[n].pos.x, Particle[n].pos.z);
			Particle[n].angle += DeltaTime * Particle[n].spin;
			Particle[n].pos.y -= Particle[n].weight*Gravity*DeltaTime;
			Particle[n].speed *= Particle[n].speed_chaos;
			Particle[n].v = Particle[n].ang * Particle[n].speed;
...
void SHARKS_PS::SetDelay(long _delay)
{
	long n;
	Delay = _delay;
	if(Delay > 0)
...
void SHARKS_PS::SetParticlesTracks(dword DeltaTime)
{
	dword color;
	dword alpha;
	dword black;
...
float SHARKS_PS::GetTrackValue(TRACK_EVENT * Track, long Time)
{
	long n;
	float v1,v2;
	long  t1,t2;
...
bool SHARKS_PS::BuildTrack(INIFILE * ini, TRACK_EVENT * Track, char * psname, char * key_name)
{
	long n,i;
...
void SHARKS_PS::SetEmitter(CVECTOR p, CVECTOR a)
{
	long n;
	Emitter = p;
	Emitter.y += yDelta;
...
/*
void SHARKS_PS::LinkToObject(ENTITY_ID id, CVECTOR _LinkPos)
{
	long n;
	bLinkEmitter = true;
	LinkObject = id;
...
*/
bool SHARKS_PS::Complete()
{
	return bComplete;
}

void SHARKS_PS::AddTrackPoint(CVECTOR pos)
{
	bUseFlowTrack = true;
	nFlowTrackSize++;
//	if (!pFlowTrack)
//		pFlowTrack = (CVECTOR *) malloc(sizeof(CVECTOR));
//	else
		pFlowTrack = (CVECTOR *) RESIZE(pFlowTrack,nFlowTrackSize*sizeof(CVECTOR));
	pFlowTrack[nFlowTrackSize - 1] = pos;
}

void SHARKS_PS::SetTrackPoint(long _index, CVECTOR _pos)
{
	GUARD(SHARKS_PS::SetTrackPoint)
	if(_index >= nFlowTrackSize) return;//_THROW(flow track overflow);
	pFlowTrack[_index] = _pos;
	UNGUARD
}

void SHARKS_PS::SetFlowTrack(dword index)
{
	CVECTOR dest;
	if(Particle[index].flow_track_index >= nFlowTrackSize) 
	{
		Particle[index].ang.x += DirectionDeviation*(0.5f - (float)rand()/RAND_MAX);
		Particle[index].ang.y += DirectionDeviation*(0.5f - (float)rand()/RAND_MAX);
		Particle[index].ang.z += DirectionDeviation*(0.5f - (float)rand()/RAND_MAX);
		return;
	}
	...
	//float dist = ~dest;
	float dist = fabs(dest.x) + fabs(dest.z);
	if(dist < fTrackPointRadius)
	{
		Particle[index].flow_track_index++;
		/*
		_VSYSTEM_API->Trace("nxt point: %d, angle: %f,%f,%f", Particle[index].flow_track_index,
															  Particle[index].ang.x, 
															  Particle[index].ang.y, 
															  Particle[index].ang.z);
		*/
	}
...
void SHARKS_PS::UseSurface(ENTITY_ID surface_id)
{
	bLayOnSurface = true;
	SurfaceID = surface_id;
}

void SHARKS_PS::SetSpeed (float _speed)
{
	Inispeed = _speed;
	for(int n=0;n<ParticlesNum;n++) 
	{
		// speed ----------------------------------------------
		Particle[n].speedVal = Inispeed;
	}
}
