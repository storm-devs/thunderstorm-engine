...
void BISignIcon::Init( ATTRIBUTES* pRoot, ATTRIBUTES* pA )
{
	...
	if( pA )
	{
		...
		if( pcTmp ) sscanf( pcTmp, "%f,%f,%f,%f", &m_rSignStateLeftUV.left,&m_rSignStateLeftUV.top, &m_rSignStateLeftUV.right,&m_rSignStateLeftUV.bottom );
		...
		if( pcTmp ) sscanf( pcTmp, "%f,%f,%f,%f", &m_rSignStateRightUV.left,&m_rSignStateRightUV.top, &m_rSignStateRightUV.right,&m_rSignStateRightUV.bottom );
...
void BISignIcon::FillVertexBuffer()
{
	...
	if( pV )
	{
		...
		{
			vn += WriteSquareToVBuff( &pV[vn], m_Sign[n].rFaceUV, m_dwSignFaceColor, m_Sign[n].pntPos+m_pntSignFaceOffset, m_pntSignFaceIconSize );
		}
		...
		for( n=0; n<m_nSignQ; n++ )
		{
			vn += WriteSquareToVBuffWithProgress( &pV[vn], m_rSignStateLeftUV, m_dwSignStateColor, m_Sign[n].pntPos+m_pntSignStateLeftOffset, m_pntSignStateLeftIconSize, 1.f - m_Sign[n].fLeftState, 0.f,0.f,0.f );
			vn += WriteSquareToVBuffWithProgress( &pV[vn], m_rSignStateRightUV, m_dwSignStateColor, m_Sign[n].pntPos+m_pntSignStateRightOffset, m_pntSignStateRightIconSize, 1.f - m_Sign[n].fRightState, 0.f,0.f,0.f );
		}

		for( n=0; n<m_nSignQ; n++ )
			vn += WriteSquareToVBuffWithProgress( &pV[vn], m_rSignStarUV, m_dwSignStarColor, m_Sign[n].pntPos+m_pntSignStarOffset, m_pntSignStarIconSize, 0.f,0.f,0.f, 1.f - m_Sign[n].fStarProgress );
