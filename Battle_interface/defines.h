...
#define FONT_RELEASE(rs,id) if(id!=-1) {rs->UnloadFont(id);id=-1;}
#define VERTEX_BUFFER_RELEASE(rs,id) if(id!=-1) {rs->ReleaseVertexBuffer(id);id=-1;}
#define INDEX_BUFFER_RELEASE(rs,id) if(id!=-1) {rs->ReleaseIndexBuffer(id);id=-1;}
...
struct BIFPOINT
{
	...
	const BIFPOINT& operator -= (const BIFPOINT& bp) {x-=bp.x; y-=bp.y; return *this;}
	friend BIFPOINT operator + (const BIFPOINT& p1, const BIFPOINT& p2) {BIFPOINT bp(p1); bp+=p2; return bp;}
	friend BIFPOINT operator + (const FPOINT& p1, const BIFPOINT& p2) {BIFPOINT bp(p2); bp.x+=p1.x; bp.y+=p1.y; return bp;}
	friend BIFPOINT operator + (const BIFPOINT& p1, const FPOINT& p2) {BIFPOINT bp(p1); bp.x+=p2.x; bp.y+=p2.y; return bp;}
