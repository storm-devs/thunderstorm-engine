...
void BIShipIcon::Draw()
{
	...
	for( long n=0; n<m_nShipQuantity; n++ )
	{
		if( m_Ship[n].pASailorQuantity ) {
			m_pRS->ExtPrint( m_idSailorFont, m_dwSailorFontColor, 0, PR_ALIGN_CENTER, true, m_fSailorFontScale, 0,0,  // boal тень
		...
		if( !m_Ship[n].sShipName.empty() ) {
			m_pRS->ExtPrint( m_idShipNameFont, m_dwShipNameFontColor, 0, PR_ALIGN_CENTER, true, m_fShipNameFontScale, 0,0, // boal тень шрифта
