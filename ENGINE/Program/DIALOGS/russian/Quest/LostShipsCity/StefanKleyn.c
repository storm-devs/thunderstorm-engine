...
#include "TEXT\DIALOGS\Quest\LostShipsCity\StefanKleyn.h"
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if (npchar.quest.meeting == "0")
			{
				...
				if (CheckAttribute(loadedLocation, "storm"))
				{
					...
					link.l1 = DLG_TEXT_LSC[1] + pchar.name + ", " + GetFullName(pchar) + DLG_TEXT_LSC[2];
					link.l1.go = "FT_1";
				}
				else
				{				
					dialog.text = DLG_TEXT_LSC[3];
					link.l1 = DLG_TEXT_LSC[4] + pchar.name + ", " + GetFullName(pchar) + DLG_TEXT_LSC[5];
					link.l1.go = "FT_1";
				}
			}
			else
		...
		break;
		case "FT_6":
			dialog.text = DLG_TEXT_LSC[29] + pchar.lastname + ".";
		...
		case "LSCNotBlade":
			...
			link.l1 = LinkRandPhrase(DLG_TEXT_LSC[40], DLG_TEXT_LSC[41], DLG_TEXT_LSC[42]);
			link.l1.go = "exit";
			NextDiag.TempNode = "First Time";
		break;		
		case "CitizenNotBlade":
			...
			{
				dialog.text = NPCharSexPhrase(NPChar, DLG_TEXT_LSC[43], DLG_TEXT_LSC[44]);
				link.l1 = LinkRandPhrase(DLG_TEXT_LSC[45], DLG_TEXT_LSC[46], DLG_TEXT_LSC[47]);
			}
			else
			{
				dialog.text = NPCharSexPhrase(NPChar, DLG_TEXT_LSC[48], DLG_TEXT_LSC[49]);
				link.l1 = RandPhraseSimple(DLG_TEXT_LSC[50], DLG_TEXT_LSC[51]);
			}
			link.l1.go = "exit";
			NextDiag.TempNode = "First Time";
		break;
		...
		case "SeekCitizen_Choice_1":
			...
			else
			{
				...
				{
					dialog.text = GetFullName(sld) + DLG_TEXT_LSC[61];
					Link.l1 = DLG_TEXT_LSC[62];
					Link.l1.go = "SeekCitizen_agree";				
					Link.l2 = DLG_TEXT_LSC[63];
					Link.l2.go = "SeekCitizen_Choice_2";
				}
				Link.l3 = DLG_TEXT_LSC[64];
				Link.l3.go = "exit";
		...
		case "SeekCitizen_agree":
			...
			if (npchar.quest.seekIdx.where == "none")
			{
				if (sld.sex == "man")
				{
					dialog.text = LinkRandPhrase(DLG_TEXT_LSC[66], DLG_TEXT_LSC[67], DLG_TEXT_LSC[68]);
					link.l1 = RandPhraseSimple(DLG_TEXT_LSC[69], DLG_TEXT_LSC[70]);
					link.l1.go = "exit";
				}
				else
				{
					dialog.text = LinkRandPhrase(DLG_TEXT_LSC[71], DLG_TEXT_LSC[72], DLG_TEXT_LSC[73]);
					link.l1 = RandPhraseSimple(DLG_TEXT_LSC[74], DLG_TEXT_LSC[75]);
					link.l1.go = "exit";
				}
			}
			else
			{
				if (sld.sex == "man")
				{
					...
					{						
						...
						{
							dialog.text = LinkRandPhrase(DLG_TEXT_LSC[76], DLG_TEXT_LSC[77], DLG_TEXT_LSC[78]);
						}
						else
						{
					...
					else
					{
						...
						{
							dialog.text = LinkRandPhrase(DLG_TEXT_LSC[83] + npchar.quest.seekIdx.where + DLG_TEXT_LSC[84], DLG_TEXT_LSC[85], DLG_TEXT_LSC[86]);
						}
						else
						{
					...
					link.l1 = RandPhraseSimple(DLG_TEXT_LSC[91], DLG_TEXT_LSC[92]);
					link.l1.go = "exit";
				}
				else
				{
					...
					{						
						...
						{
							dialog.text = LinkRandPhrase(DLG_TEXT_LSC[93], DLG_TEXT_LSC[94], DLG_TEXT_LSC[95]);
						}
						else
						{
					...
					else
					{
						if (npchar.location == sld.location)
						{
							dialog.text = LinkRandPhrase(DLG_TEXT_LSC[100] + npchar.quest.seekIdx.where + DLG_TEXT_LSC[101], DLG_TEXT_LSC[102], DLG_TEXT_LSC[103]);
						}
						else
						{
					...
					link.l1 = RandPhraseSimple(DLG_TEXT_LSC[108], DLG_TEXT_LSC[109]);
					link.l1.go = "exit";
				}
			}
		break;
