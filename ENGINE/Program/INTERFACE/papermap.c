...
int CalculateShipRum(ref _character)
{
	...
	float rumNeeded 	= makefloat( (crewQuantity + 5.1) / (RUM_BY_CREW - MOD_SKILL_ENEMY_RATE * 2.5)); // Сколько жрут за день);
...
int _GetDistanceToColony2D(string _sColony)
{
	...
	ref rColony = GetColonyByIndex(FindColony(_sColony));
...
void ShowColonyInfo(int iColony)
{
	...
	for(iType=0; iType<2; iType++)
	{
		...
		{
			sGood = Goods[i].name;
			if(refStore.Goods.(sGood).TradeType == TRADE_TYPE_CONTRABAND)
				continue;
			
			if(TRADEASSISTANT_MODE == 1)
			{
				if(refStore2.Goods.(sGood).TradeType == TRADE_TYPE_CONTRABAND)
					continue;
			}		
			else
			{
				...
				if(arefStore2.(sGood).TradeType == TRADE_TYPE_CONTRABAND)
					continue;
			}	
