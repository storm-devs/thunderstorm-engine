string DLG_TEXT_TV [14] = {
	"Can I help you?",
	"What do you desire, ",
	"Not too long ago, you tried to ask me a question, ",
	"You keep mentioning you have a question for me. This is the third time today...",
	"Asking questions again?",
	"Sorry. Never mind...",
	"Uh, yes. I can see that you are busy, we can talk next time.",
	"Something's happened to my memory...",
	"One, two... you're right! Third it is.",
	"Oh no, no questions...",
	"I'm telling you, I've never seen such a bloody fight in my life...",
	"Slashed those English dogs for good, eh?",
	"Well, what can I say, Mynheer. It doesn't come better than this. Many thanks!",
	"Sure thing.",
};
