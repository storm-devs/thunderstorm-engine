	...
	while(TokenIs(OP_BOOL_AND) || TokenIs(OP_BOOL_OR))
	{
		...
		switch(op)
		{
			...
			case OP_BOOL_AND:
				...
				while(BC_TokenGet() == DEBUG_LINE_CODE){};
				value->Get(lRes);
				if(lRes == 0) BC_ProcessExpression_L1(&value2,true);//SkipExpression_L1();
				else
				{
			...
			case OP_BOOL_OR:
				...
				while(BC_TokenGet() == DEBUG_LINE_CODE){};
				value->Get(lRes);
				if(lRes) BC_ProcessExpression_L1(&value2,true);//SkipExpression_L1();
				else
				{
					BC_ProcessExpression_L1(&value2);
					value->RefConvert();
...
void COMPILER::BC_ProcessExpression_L6(DATA * value, bool bSkip)
{
	...
	else BC_ProcessExpression_L7(value,bSkip);
}
	...
	switch(TokenType())
	{
		...
		case CALL:
			if(bSkip) 
			{
				...
				if(BC_TokenGet() != OPEN_BRACKET) {SetError("missing '('");return;}
				local_in = 1;
				do
				{
					switch(BC_TokenGet())
			...
			if(vtype == VARIABLE)
			{
				if(!VarTab.GetVar(vi,var_code))	{ SetError("Global variable not found"); return; }
				pVV = vi.pDClass;
			}
			else
			{
			...
			} else SetError("void function result in expression");
			while(BC_TokenGet() == DEBUG_LINE_CODE){};
		...
		case VARIABLE:
			...
			if(TokenType() == VARIABLE)
			{
				if(!VarTab.GetVar(vi,var_code))	{ SetError("Global variable not found"); break; }
				pV = vi.pDClass;
			} 
			else
			{
			...
			{
				if(!(pV->IsArray() || bref2a)) { SetError("wrong '[' in expression"); return; }
			...
			while(TokenType() == DEBUG_LINE_CODE){BC_TokenGet();};
			//while(BC_TokenGet() == DEBUG_LINE_CODE){};
			if(TokenType() != ACCESS_WORD_CODE && TokenType() != ACCESS_WORD && TokenType() != ACCESS_VAR)
			{
				...
				if(!bSkip)
				{
					if(!pV->IsArray()) { value->Copy(pV); return; }	// copy single value
		...
		if(sttTokenType == SQUARE_OPEN_BRACKET)
		{
			...
			if(sttVariableType == VARIABLE)
			{
				...
				if(!vi.bArray) 
				{
					if(vi.type != VAR_REFERENCE)
					{
						...
						SetError(" A Invalid '[' operator, %s - isnt array",vi.name); return false;
					}
				}
			}
			else
			{
				...
				if(!lvi.bArray) 
				{
					...
					{
						SetError(" B Invalid '[' operator, %s - isnt array",lvi.name); return false;
					}
				}
			}
...
{
	if(!CompileExpression_L5(Segment)) { SetError(ERR_INVALID_EXPRESSION); return false;}
...
bool COMPILER::CompileExpression_L6(SEGMENT_DESC& Segment)
{
	if(Token.GetType() == OPEN_BRACKET)
	{
		CompileAuxiliaryTokens(Segment);
		if(!CompileExpression_L1(Segment)) { SetError(ERR_INVALID_EXPRESSION); return false;}
		if(Token.GetType() != CLOSE_BRACKET) {SetError("No matching ')' in expression"); return false;}
		CompileAuxiliaryTokens(Segment);
		return true;
	}
	else return CompileExpression_L7(Segment);
}
		...
		case UNKNOWN:
			...
			switch(sttResult)
			{
				...
				case CALL:
					...
					if(Token.Get()!=UNKNOWN) { SetError("invalid dynamic call syntax"); return false; }
					...
					{
						VarTab.GetVarX(vi,dwVarCode);
						if(vi.type != VAR_STRING) { SetError("'%s' must be string variable", Token.GetData()); return false; }
					}
					else
					if(sttVariableField == LOCAL_VARIABLE)
					{
						FuncTab.GetVar(lvi,CurrentFuncCode,dwVarCode);
						if(lvi.type != VAR_STRING) { SetError("'%s' must be string variable", Token.GetData()); return false; }
					}
					else { SetError("invalid dynamic call syntax: '%s' must be string variable", Token.GetData()); return false; }
				...
				case CALL_FUNCTION:
					...
					if(Token.Get()!=OPEN_BRACKET) { SetError("missing '('"); return false; }
					...
					else
					{
						...
						do
						{
							...
							if(!CompileExpression(Segment)) { SetError("invalid function argument"); return false; }
							if(Token.GetType()!=COMMA)
							{
								if(Token.GetType()!=CLOSE_BRACKET) { SetError("invalid function argument"); return false; }
							}
							func_args++;
					...
					if(sttResult == SQUARE_OPEN_BRACKET)
					{
						...
						else
						{
							...
							if(!lvi.bArray) 
							{
								if(lvi.type != VAR_REFERENCE)
								{
									SetError(" D Invalid '[' operator, %s - isnt array",lvi.name); return false;
								}
							}
						}
