...
public:
	...
	~FSDATA_LIST(){ free(Data_PTR);};
	void Release() { free(Data_PTR); structure_size = 0; initiate_blocks = 0; Data_PTR = nullptr; used_blocks = 0;};
	...
	bool Init(uint32_t _structure_size, uint32_t _initiate_blocks)
	{
		...
		Data_PTR = malloc(structure_size*initiate_blocks);
	...
	bool AddData(void * _data_ptr)
	{
		...
		if(used_blocks >= initiate_blocks)
		{
			...
			Data_PTR = realloc(Data_PTR,structure_size*initiate_blocks);
			if(Data_PTR == nullptr) return false;
		}
		char* mem_PTR = (char*)Data_PTR + used_blocks * structure_size;
	...
	bool GetData(uint32_t _n, void * struct_PTR)
	{
		...
		if(Data_PTR == nullptr) return false;
		char* mem_PTR = (char*)Data_PTR + _n * structure_size;
