...
{
    InterfaceStack.SelectMenu_node = "LaunchItems"; // запоминаем, что звать по ‘2
...
void FillControlsList(int nMode)
{
	switch (nMode)
	{
	    ...
	    case 2: FillItemsTable(2); break;  // снар€жение
		...
		if (itmGroup == MAPS_ITEM_TYPE)
		{
            ...
            {// клады
            	...
            	if (GetCharacterItem(pchar, "map_part1")>0  && GetCharacterItem(pchar, "map_part2")>0)
			    {
			        ...
			        // здесь генераци€ назначение и типа клада
			        pchar.GenQuest.TreasureBuild = true;
			        FillMapForTreasure(itmRef);
			        SetVariable();
			    }
			    // тут применЪем логику двух кусков, из них одна карта <--
