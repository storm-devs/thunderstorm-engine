...
bool bEncType   = false;
bool bShowVideo; // дл€ показа квестовых роликов, если будут
...
void InitInterface(string iniName)
{
	...
	SetEventHandler("InterfaceBreak","ProcessBreakExit",0); // ¬ыход на море
	SetEventHandler("exitCancel","ProcessCancelExit",0); // ¬ыход на море по крестику или Esc
	SetEventHandler("ievnt_command","ProcCommand",0); // выход на карту только тут (по Ќ≈“)
...
void CalculateInfoData()
{
	...
	if( (rand(100) / iSKILL_SNEAK) > GetSummonSkillFromNameToOld(pchar, SKILL_SAILING) )// сделать учет скила навигаци€ дл€ запрета кнопки Ќет
	{
