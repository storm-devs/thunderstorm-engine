...
NetIsland::NetIsland() : aForts(_FL_), aSpheres(_FL_)
{
...
void NetIsland::Realize(dword Delta_Time)
{
...
bool NetIsland::GetDepth(FRECT * pRect, float * fMinH, float * fMaxH)
{
	return false;
}

dword _cdecl NetIsland::ProcessMessage(MESSAGE & message)
{
	ENTITY_ID	eID;
...
void NetIsland::AddLocationModel(ENTITY_ID & eid, char * pIDStr, char * pDir)
{
	Assert(pDir && pIDStr);
	bForeignModels = true;
...
inline float NetIsland::GetDepthNoCheck(DWORD iX, DWORD iZ)
{
...
inline float NetIsland::GetDepthCheck(DWORD iX, DWORD iZ)
{
...
bool NetIsland::Check2DBoxDepth(CVECTOR vPos, CVECTOR vSize, float fAngY, float fMinDepth)
{
...
bool NetIsland::GetDepthFast(float x, float z, float * fRes)
{
...
bool NetIsland::GetDepth(float x, float z, float * fRes)
{
	//if (!mzDepth.isLoaded()) {if (fRes) *fRes = -50.0f;	return false; }
	x -= vBoxCenter.x;
	z -= vBoxCenter.z;
...
bool NetIsland::ActivateCamomileTrace(CVECTOR & vSrc)
{
...
void NetIsland::CalcBoxParameters(CVECTOR & _vBoxCenter, CVECTOR & _vBoxSize)
{
	GEOS::INFO	ginfo;
	ENTITY_ID	* pEID;
...
void NetIsland::CreateDirectories(char * pDir)
{
#ifndef _XBOX
...
void NetIsland::Blur8(byte * * pBuffer, dword dwSize)
{
	dword	x, z; 
...
bool NetIsland::CreateHeightMap(char * pDir, char * pName)
{
	TGA_H	tga_head;
	string	sDir; 
...
bool NetIsland::SaveTga8(char * fname, byte * pBuffer, dword dwSizeX, dword dwSizeY)
{
	TGA_H tga_head;
...
bool NetIsland::Mount(char * fname, char * fdir, ENTITY_ID * eID)
{
...
float NetIsland::Cannon_Trace(long iBallOwner, const CVECTOR & vSrc, const CVECTOR & vDst)
{
	float fRes = Trace(vSrc,vDst);
	if (fRes <= 1.0f)
	{
		CVECTOR vTemp = vSrc + fRes * (vDst - vSrc);
		api->Event("NetServer_OnBallIslandHit", "lfff", iBallOwner, vTemp.x, vTemp.y, vTemp.z);
	}
	return fRes; 
}

float NetIsland::Trace(const CVECTOR & vSrc, const CVECTOR & vDst)
{
	return pCollide->Trace(*pIslandTraceWalker, vSrc, vDst, null, 0);
}

// Path section
bool NetIsland::GetMovePoint(CVECTOR & vSrc, CVECTOR & vDst, CVECTOR & vRes)
{
	// check for one side 
	dword	i,j;
	vRes = vDst;
...
/*AIFlowGraph::VectorPath	* NetIsland::FindPath(CVECTOR & vSrc, CVECTOR & vDst)
{
