...
bool NetFlag::CreateState(ENTITY_STATE_GEN * state_gen)
{
	return true;
}

bool NetFlag::LoadState(ENTITY_STATE * state)
{
	return true;
}

void NetFlag::Execute(dword Delta_Time)
{
    ...
    if(bUse)
    {
        ...
        globalWind.base = pAWeather->GetAttributeAsFloat("WindSpeed") / fWindMaxValue;
...
void NetFlag::Realize(dword Delta_Time)
{
    if(bUse)
    {
        DWORD rtm;
		...
		if(nVert!=0 && nIndx!=0) {
			...
			for(long fn=0; fn<flagQuantity; fn++)
			{
				RenderService->SetTexture(0,flist[fn]->pTexture);
				//RenderService->DrawBuffer(vBuf, sizeof(FLAGLXVERTEX), iBuf, flist[fn]->sv, flist[fn]->nv, flist[fn]->st, flist[fn]->nt, "ShipFlag");
...
dword _cdecl NetFlag::ProcessMessage(MESSAGE & message)
{
...
void NetFlag::SetTextureCoordinate()
{
    if(bUse)
    {
        int i;
        ...
        if(pv)
        {
            for(int fn=0; fn<flagQuantity; fn++)
            {
                ...
                sIdx=flist[fn]->sv;
                addtu = 1.f;//1.f/(float)FlagTextureQuantity;
                stu = 0.f;//addtu*flist[fn]->texNum;
                stv = 0.f;
...
void NetFlag::DoMove(FLAGDATA *pr,float delta_time)
{
	if(pr==NULL) return;
    CVECTOR cPos;
...
void NetFlag::LoadIni()
{
    GUARD(NetFlag::LoadIni());
	char	section[256];
    char    param[256];
...
void NetFlag::GroupDelete(ENTITY_ID m_id)
{
...
void NetFlag::SetAdd(int flagNum)
{
    // set vertex and index buffers
    nIndx*=3;
