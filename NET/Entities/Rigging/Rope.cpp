...
bool NetRope::CreateState(ENTITY_STATE_GEN * state_gen)
{
	return true;
}

bool NetRope::LoadState(ENTITY_STATE * state)
{
	return true;
}
...
void NetRope::Realize(dword Delta_Time)
{
    if(bUse)
    {
        DWORD rtm;
...
dword _cdecl NetRope::ProcessMessage(MESSAGE & message)
{
...
/*
void NetRope::DoMove(ROPEDATA *pr)
{
    if(pr->bUse)
    {
        int vertnum=pr->sv;
...
*/
void NetRope::AddLabel(GEOS::LABEL &lbl,NODE *nod)
{
    ROPEDATA *rd;
    int  ropeNum,grNum;
    int  rn;
    ...
    if( rd->bMatWorld!=0 && rd->eMatWorld!=0 )
    {
		...
		else
		{
			...
			ENTITY_ID sailEI;
			if(NetFindClass(IsServer(), &sailEI, "NetSail"))
			{
...
void NetRope::LoadIni()
{
    GUARD(NetRope::LoadIni());
	char	section[256];
    char    param[256];
...
void NetRope::SetTextureGrid(ROPEDATA *pv)
{
    int iv=pv->sv;
...
bool NetRope::IsAbsentRope(ENTITY_ID &mdl_id, int ropenum)
{
    bool retVal=true;
