...
#define SAILID_MAST_SHIFT	8
#define SAILID_MASK_MAST	0x00F00
#define SAILID_REYCODE_A	0x01000
#define SAILID_REYCODE_B	0x02000
#define SAILID_REYCODE_C	0x04000
#define SAILID_REYCODE_D	0x08000
#define SAILID_REYCODE_E	0x10000
#define SAILID_REYCODE_F	0x20000
#define SAILID_REYCODE_MASK	0x3F000
...
SAILONE::~SAILONE()
{
	ROPE_BASE * prb = NULL;
	ENTITY_ID eid;
	if( NetFindClass(pp->IsServer(), &eid, "NetRope") )	prb = (ROPE_BASE*)eid.pointer;
    ...
    PTR_DELETE(sroll);
    PTR_DELETE(sailtrope.rrs[0]);
    PTR_DELETE(sailtrope.rrs[1]);
...
bool SAILONE::SetSail()
{
	...
	if( sailWidth>0 && sailHeight>0 ) {
		m_fHorzGeraldScale = sailHeight / sailWidth * .5f;
		m_fVertGeraldScale = sailWidth / sailHeight * .5f;
		while( m_fHorzGeraldScale > 1.f ) {
			m_fHorzGeraldScale *= .5f;
			m_fVertGeraldScale *= .5f;
		}
		while( m_fVertGeraldScale > 1.f ) {
			m_fHorzGeraldScale *= .5f;
			m_fVertGeraldScale *= .5f;
		}
	}
...
{
    if( sroll ){
        if( sroll->rollup==bRoll ) return; // не делать повторно
    }
    else
        if( bRoll==bRolling ) return; // не делать повторно
...
void SAILONE::TurnSail(float fTurnStep)
{
    ...
    if( windAng<-pp->WINDANGL_DISCRETE || windAng>pp->WINDANGL_DISCRETE )
    {
		...
		else if(NetFindClass(pp->IsServer(), &ropeEI, "NetRope"))
        {
            bool bChange=false;
            for(int i=0; i<2; i++)
                if(sailtrope.rrs[i])
...
void SAILONE::SetTurnLimits()
{
	...
	ENTITY_ID ropeEI;
	if( !NetFindClass(pp->IsServer(), &ropeEI, "NetRope") ) return;
...
{
	m_dwSailIDCode = -1;
	dword dwCode = groupNum;
	dwCode |= (mastNum<<SAILID_MAST_SHIFT) & SAILID_MASK_MAST;
	if( hostNode && hostNode->GetName() && strnicmp(hostNode->GetName(),"rey_",4)==0 ) {
		switch(hostNode->GetName()[4]) {
			case 'a': dwCode |= SAILID_REYCODE_A; break;
			case 'b': dwCode |= SAILID_REYCODE_B; break;
			case 'c': dwCode |= SAILID_REYCODE_C; break;
			case 'd': dwCode |= SAILID_REYCODE_D; break;
			case 'e': dwCode |= SAILID_REYCODE_E; break;
			case 'f': dwCode |= SAILID_REYCODE_F; break;
		}
	}
	m_dwSailIDCode = dwCode;
}

void SAILONE::ChangeHoleDwordCode()
{
	if( pp->IsServer() ) {
		m_bHoleChanged = true;
	}
...
void SAILONE::SetHoleState(long nHoleCode)
{
	SetAllHole(nHoleCode);
}
