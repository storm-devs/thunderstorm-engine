...
bool NetSail::CreateState(ENTITY_STATE_GEN * state_gen)
{
    return true;
}

bool NetSail::LoadState(ENTITY_STATE * state)
{
	return true;
}

void NetSail::Execute(dword Delta_Time)
{
    DWORD rtime;
    int i;
    ...
    if(bUse)
    {
		...
		globalWind.ang.x = pAWeather->GetAttributeAsFloat("WindAngle");
        ...
        for(i=0; i<sailQuantity; i++)
        {
			...
			if( slist[i]->m_bHoleChanged ) {
				...
				long iSailIndex = slist[i]->GetSailIDCode();
				long iSailState = slist[i]->GetHoleDword();
				api->Event("NetServer_OnShipSailChangeState", "lll", wCliendID,iSailIndex,iSailState);
			}
        }
...
void NetSail::Realize(dword Delta_Time)
{
...
dword _cdecl NetSail::ProcessMessage(MESSAGE & message)
{
    GUARD(dword _cdecl NetSail::ProcessMessage(MESSAGE message))
    ENTITY_ID tmpEI;
    int i;
    ...
    case MSG_SAIL_INIT: // format "liil" (code,shipEntity,modelEntity,bSailUp)
		{
			...
			if(bSailUp)
			{
				gdata[groupQuantity-1].bFinalSailDo = false;
...
void NetSail::AddSailLabel(GEOS::LABEL &lbl, NODE *nod, bool bSailUp)
{
    SAILONE* cs;
    int grNum, i;
...
void NetSail::SetAllSails(int groupNum)
{
...
void NetSail::LoadSailIni()
{
    GUARD(NetSail::LoadSailIni());
...
float NetSail::Trace(const CVECTOR &src,const CVECTOR &dst)
{
    float retVal=2.f;
...
const char* NetSail::GetCollideMaterialName()
{
    return "sail";
}

bool NetSail::GetCollideTriangle(struct TRIANGLE & trg)
{
	return false;
}

bool NetSail::Clip(const PLANE *planes, long nplanes, const CVECTOR &center, float radius, ADD_POLYGON_FUNC addpoly)
{
    return false;
}

void NetSail::FirstRun()
{
    int i,sn;
...
float NetSail::Cannon_Trace(long iBallOwner, const CVECTOR &src,const CVECTOR &dst)
{
	if( !IsServer() ) return 2.f;
    bCannonTrace = true;
...
void NetSail::DoSailToNewHost(ENTITY_ID newModelEI, ENTITY_ID newHostEI, int grNum, NODE *nod, ENTITY_ID oldModelEI)
{
...
void NetSail::SetAddSails(int firstSail)
{
...
SAILONE * NetSail::FindSailFromData(int gn,char* nodeName,char* grName)
{
...
void NetSail::SetSailTextures(long grNum, VDATA* pvd)
{
...
int NetSail::FindGroupForCharacter(int chrIdx)
{
...
int NetSail::GetCharacterForGroup(int grNum)
{
	ATTRIBUTES* pA = null;
...
SAILONE_BASE * NetSail::FindSailForCharacter(int chrIdx,char* nodeName,int grNum)
{
...
int NetSail::GetSailStateForCharacter(int chrIdx)
{
	int gn = FindGroupForCharacter(chrIdx);
...
dword _cdecl NetSail::ScriptProcessing(char * name, MESSAGE & message)
{
	...
	if( stricmp(name,"SailMakeHole")==0 )
	{
		long wClientID = message.Long();
		long iSailIDCode = message.Long();
		long iSailState = message.Long();
		MakeHoleFromServer(wClientID,iSailIDCode,iSailState);
	}

	return 0;
}

void NetSail::DoRandomsSailsDmg(int chrIdx, int gn, float fDmg)
{
...
dword NetSail::AttributeChanged(ATTRIBUTES * pAttr)
{
...
void NetSail::MakeHoleFromServer(long wClientID, long iSailIDCode, long iSailState)
{
	for( long n=0; n<groupQuantity; n++)
	{
		ENTITY* pE = api->GetEntityPointer( &gdata[n].shipEI );
		if( pE ) {
			if( pE->GetNetID() == wClientID ) {
				for( long i=0; i<gdata[n].sailQuantity; i++ )
				{
					long sn = gdata[n].sailIdx[i];
					if( sn>=0 && sn<sailQuantity && slist[sn]->GetSailIDCode()==iSailIDCode ) {
						slist[sn]->SetHoleState( iSailState );
					}
				}
			}
		}
