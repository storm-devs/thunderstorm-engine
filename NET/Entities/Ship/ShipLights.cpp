...
void NetShipLights::Release(VAI_OBJBASE * pObject)
{
}

float NetShipLights::GetAttributeAsFloat(ATTRIBUTES * pA, const char * pName, float fDefault)
{
...
NetShipLights::LightType * NetShipLights::FindLightType(string sLightType)
{
...
void NetShipLights::AddDynamicLights(VAI_OBJBASE * pObject, const CVECTOR & vPos)
{
...
bool NetShipLights::SetLabel(ShipLight * pL, MODEL * pModel, const char * pStr)
{
...
void NetShipLights::AddFlare(VAI_OBJBASE * pObject, bool bLight, MODEL * pModel, const GEOS::LABEL & label)
{
	CMatrix m;
...
void NetShipLights::SetLightsOff(VAI_OBJBASE * pObject, float fTime, bool bLights, bool bFlares, bool bNow)
{
...
void NetShipLights::KillMast(VAI_OBJBASE * pObject, NODE * pNode, bool bNow)
{
...
void NetShipLights::AddLights(VAI_OBJBASE * pObject, MODEL * pModel, bool bLights, bool bFlares)
{
	if (!bLoadLights && !LoadLights()) return;
...
void NetShipLights::SetLights(VAI_OBJBASE * pObject)
{
	dword i;
...
void NetShipLights::Execute(dword dwDeltaTime)
{
...
// draw flares
void NetShipLights::Realize(dword dwDeltaTime)
{
	float fDeltaTime = float(dwDeltaTime) * 0.001f;
...
void NetShipLights::SetDead(VAI_OBJBASE * pObject)
{
	for (dword i=0; i<aLights.Size(); i++) if (aLights[i].pObject == pObject)
		aLights[i].bDead = true;
}

void NetShipLights::UnSetLights(VAI_OBJBASE * pObject)
{
...
dword _cdecl NetShipLights::ProcessMessage(MESSAGE & message)
{
	switch (message.Long())
	{
