...
VDX9RENDER * NetShipTracks::ShipTrack::pRS = null;
SEA_BASE * NetShipTracks::ShipTrack::pSea = null;
long NetShipTracks::ShipTrack::iRefCount = 0;
dword NetShipTracks::ShipTrack::dwMaxBufferSize1 = 0, NetShipTracks::ShipTrack::dwMaxBufferSize2 = 0;
long NetShipTracks::ShipTrack::iVTmpBuffer1 = -1, NetShipTracks::ShipTrack::iVTmpBuffer2 = -1;
long NetShipTracks::ShipTrack::iITmpBuffer1 = -1, NetShipTracks::ShipTrack::iITmpBuffer2 = -1;

NetShipTracks::NetShipTracks() : aShips(_FL)
{
}

NetShipTracks::~NetShipTracks()
{
	aShips.DelAllWithPointers();
}

bool NetShipTracks::Init()
{
	ENTITY_ID sea_id;
...
void NetShipTracks::ResetTrack(SHIP_BASE * pShip)
{
...
void NetShipTracks::AddShip(SHIP_BASE * pShip)
{
...
void NetShipTracks::DelShip(SHIP_BASE * pShip)
{
	for (long i=0; i<aShips; i++) if (aShips[i]->pShip == pShip)
	{
		DELETE(aShips[i]);
...
void NetShipTracks::Execute(dword dwDeltaTime)
{
	for (long i=0; i<aShips; i++)
		aShips[i]->Execute(float(dwDeltaTime) * 0.001f);
}

void NetShipTracks::Realize(dword dwDeltaTime)
{
	for (long i=0; i<aShips; i++)
		aShips[i]->Realize(float(dwDeltaTime) * 0.001f);
}

NetShipTracks::ShipTrack::ShipTrack() : aTrack1(_FL_, 128), aTrack2(_FL_, 32)
{
	bFirstExecute = true;
	iTrackTexture1 = -1;
	iTrackTexture2 = -1;
...
NetShipTracks::ShipTrack::~ShipTrack()
{
	iRefCount--;
	if (iRefCount <= 0)
	{
...
bool NetShipTracks::ShipTrack::Update(SHIP_BASE * pShip)
{
...
bool NetShipTracks::ShipTrack::Reserve1(dword dwSize)
{
...
bool NetShipTracks::ShipTrack::Reserve2(dword dwSize)
{
...
void NetShipTracks::ShipTrack::Execute(float fDeltaTime)
{
...
void NetShipTracks::ShipTrack::Reset()
{
	bFirstExecute = true;
...
dword NetShipTracks::AttributeChanged(ATTRIBUTES * pA)
{
	if (*pA == "ResetTracks")
	{
...
void NetShipTracks::ShipTrack::Realize(float fDeltaTime)
{
	if (!pShip || !pSea) return;
	...
	if (Reserve2(aTrack2()))
		if (aTrack2.Len() > 1)
		{
			...
			for (long i=0; i<aTrack2; i++)
			{
				...
				{
					float x = T.fWidth * (xx / (fTrackStep2 - 1.0f) - 0.5f);
					float z = 0.0f;
					RotateAroundY(x, z, T.fCos, T.fSin);
					x += T.vPos.x; z += T.vPos.z;
					CVECTOR vPos = CVECTOR(x, fWaveUP + pSea->WaveXZ(x, z), z);
					pV[i * dwTrackStep2 + xxx].vPos = vPos;
	...
	if (aTrack1.Len() > 1) 
	{
		//pShip->Render().SetTexture(0, pTrackTexture1);
		//pShip->Render().SetStreamSource(0, pVTmpBuffer1);
		//pShip->Render().SetIndices(pITmpBuffer1);
		//pShip->Render().DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, aTrack1.Len() * 20, 0, 19 * (aTrack1.Len() - 1) * 2, "SeaTrack");
	...
	if (aTrack2.Len() > 1)
	{
		//pShip->Render().SetTexture(0, pTrackTexture2);
		//pShip->Render().SetStreamSource(0, pVTmpBuffer2);
		//pShip->Render().SetIndices(pITmpBuffer2);
		//pShip->Render().DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, aTrack2.Len() * 4, 0, 3 * (aTrack2.Len() - 1) * 2, "SeaTrack");
