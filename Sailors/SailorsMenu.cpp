...
void Menu :: Draw(VDX9RENDER * rs, SailorsPoints &sailorsPoints)
{
	...
	if (selected== 1 && blocked== 1){
		...
		if (sailorsPoints.points.selected>= 0 && sailorsPoints.points.selected< sailorsPoints.points.count){
			...
			case(PT_TYPE_NORMAL):       rs->Print(rs->GetCurFont(), D3DCOLOR_XRGB(200,200,200) ,10, 160,"%s" , "type: PT_TYPE_NORMAL('F9')");
								        break;
			...
			case(PT_TYPE_MAST_1): rs->Print(rs->GetCurFont(), D3DCOLOR_XRGB(200,200,200) ,10, 160,"%s" , "type: PT_TYPE_MAST_1('F9')");
								        break;

			case(PT_TYPE_MAST_2): rs->Print(rs->GetCurFont(), D3DCOLOR_XRGB(200,200,200) ,10, 160,"%s" , "type: PT_TYPE_MAST_2('F9')");
								        break;

			case(PT_TYPE_MAST_3): rs->Print(rs->GetCurFont(), D3DCOLOR_XRGB(200,200,200) ,10, 160,"%s" , "type: PT_TYPE_MAST_3('F9')");
								        break;

			case(PT_TYPE_MAST_4): rs->Print(rs->GetCurFont(), D3DCOLOR_XRGB(200,200,200) ,10, 160,"%s" , "type: PT_TYPE_MAST_4('F9')");
								        break;

			case(PT_TYPE_MAST_5): rs->Print(rs->GetCurFont(), D3DCOLOR_XRGB(200,200,200) ,10, 160,"%s" , "type: PT_TYPE_MAST_5('F9')");
								        break;

			case(PT_TYPE_NOT_TARGETPOINT): rs->Print(rs->GetCurFont(), D3DCOLOR_XRGB(200,200,200) ,10, 160,"%s" , "type: PT_TYPE_NOT_TARGETPOINT('F9')");
								        break;
			};

		} else rs->Print(rs->GetCurFont(), D3DCOLOR_XRGB(200,200,200) ,10, 160,"%s" , "type: ('F9')");
		
		if(sailrs->shipWalk[0].sailorsPoints.points.count && sailrs->shipWalk[0].sailorsPoints.points.count> sailorsPoints.points.selected)
			if (sailrs->shipWalk[0].sailorsPoints.points.point[sailorsPoints.points.selected].buisy)
			rs->Print(rs->GetCurFont(), D3DCOLOR_XRGB(200,200,200) ,10, 180,"%s" , "buisy= true"); else
...
void Menu :: ChangeControl1(int key, SailorsPoints &sailorsPoints) //Change people count
{
	...
	if (key== VK_LEFT){ 
		...
		sailrs->shipWalk[0].sailorsPoints.points.point[
			sailrs->shipWalk[0].shipMan[0].lastWayPoint
		].buisy= false;
		...
		if (key== VK_SPACE){
			...
			if (shiftKey){
 				...
 				if (sailorsPoints.points.selected)
					sailorsPoints.points.selected--; else 
				if (sailorsPoints.points.count)
					sailorsPoints.points.selected= sailorsPoints.points.count- 1; else
				...
				if (sailorsPoints.points.selected< sailorsPoints.points.count- 1)
					sailorsPoints.points.selected++; else 
				if (sailorsPoints.points.count)
					sailorsPoints.points.selected= 0; else
					sailorsPoints.points.selected= -1;
			};
...
*/
	...
	if (key== VK_SPACE){
		...
		if (shiftKey){
			...
			if (sailorsPoints.links.selected)
				sailorsPoints.links.selected--; else 
			if (sailorsPoints.links.count)
				sailorsPoints.links.selected= sailorsPoints.links.count- 1; else
			...
			if (sailorsPoints.links.selected< sailorsPoints.links.count- 1)
				sailorsPoints.links.selected++; else 
			if (sailorsPoints.links.count)
				sailorsPoints.links.selected= 0; else
				sailorsPoints.links.selected= -1;
		};
	...
	if (key== VK_F2){
		...
		if (selLnk< sailorsPoints.links.count && selLnk>= 0){
			...
			if (shiftKey)
				sailorsPoints.links.link[selLnk].first--; else
	...
	if (key== VK_F3){
		...
		if (selLnk< sailorsPoints.links.count && selLnk>=0){
			...
			if (shiftKey)
				sailorsPoints.links.link[selLnk].next--; else
...
void Menu :: ChangeControl(int key, SailorsPoints &sailorsPoints)
{
	...
	if (key== VK_F4){ 
		...
		sailrs->shipWalk[0].shipState.mode= SHIP_WAR; else
		if (sailrs->shipWalk[0].shipState.mode== SHIP_WAR)
		sailrs->shipWalk[0].shipState.mode= SHIP_STORM; else
...
void Menu :: OnKeyPress(SailorsPoints &sailorsPoints)
{
	...
	if (GetAsyncKeyState(VK_UP)< 0)		key= VK_UP;	     else
	if (GetAsyncKeyState(VK_DOWN)< 0)	key= VK_DOWN;	 else
	if (GetAsyncKeyState(VK_LEFT)< 0)	key= VK_LEFT;    else
	if (GetAsyncKeyState(VK_RIGHT)< 0)	key= VK_RIGHT;   else
	if (GetAsyncKeyState(VK_RETURN)< 0) key= VK_RETURN;	 else
	if (GetAsyncKeyState(VK_INSERT)< 0) key= VK_INSERT;	 else
	if (GetAsyncKeyState(VK_DELETE)< 0) key= VK_DELETE;  else
	if (GetAsyncKeyState(VK_SPACE)< 0)  key= VK_SPACE;	 else
	if (GetAsyncKeyState(VK_F1)< 0)     key= VK_F1;		 else
	if (GetAsyncKeyState(VK_F2)< 0)     key= VK_F2;		 else
	if (GetAsyncKeyState(VK_F3)< 0)     key= VK_F3;		 else
	if (GetAsyncKeyState(VK_F5)< 0)     key= VK_F5;		 else
	if (GetAsyncKeyState(VK_F6)< 0)     key= VK_F6;		 else
	if (GetAsyncKeyState(VK_F9)< 0)     key= VK_F9;		 else
	if (GetAsyncKeyState(VK_F4)< 0)     key= VK_F4;		 else
	if (GetAsyncKeyState(VK_F10)< 0)    key= VK_F10;	 else
	if (GetAsyncKeyState(VK_F11)< 0)    key= VK_F11;
	...
	if (!key) { keyPressed= false; return; }
	...
	switch(key){
		...
		case VK_RETURN:
			...
			if (blocked== -1) 
				blocked= selected; else
				blocked= -1;

			if (blocked== 0 && selected== 0) {
				sailrs->disabled= false; 
			} else sailrs->disabled= true;
