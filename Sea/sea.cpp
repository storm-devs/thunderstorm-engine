...
void SEA::Realize(dword dwDeltaTime)
{
	...
	if (iVStart && iTStart)
	{
		...
		Render().SetVertexShaderConstantF(GC_SHADOW_CONST1, (const int*)&CVECTOR4(fFoamV, fFoamK, fFoamUV, 6.0f), 1);
		Render().SetVertexShaderConstantF(GC_ANIMATION, (const int*)&CVECTOR4(fTmp, fTmp, fTmp, fTmp), 1);
		Render().SetVertexShaderConstantF(GC_CAMERA_POS, (const int*)&CVECTOR4(vCamPos.x-vWorldOffset.x, vCamPos.y, vCamPos.z-vWorldOffset.z, 1.0f), 1);
		...
		else
		{
			...
			{
				//Render sea foam
				Render().SetPixelShaderConstantF(0, (const int*)&CVECTOR4(fFoamTextureDisturb, 0.0f, 0.0f, 0.0f), 1);
