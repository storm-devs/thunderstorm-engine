...
inline bool IntersectLines2D(const CVECTOR& v1, const CVECTOR& v2, const CVECTOR& v3, const CVECTOR& v4,
                             CVECTOR& vRes) {
...
inline uint32_t Number2Shift(uint32_t dwNumber) {
  for (uint32_t i = 0; i < 31; i++) if (static_cast<uint32_t>(1 << i) == dwNumber) return i;
  return 0;
}
