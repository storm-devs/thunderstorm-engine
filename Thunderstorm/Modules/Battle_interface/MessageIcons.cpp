...
void MESSAGE_ICONS::Update(uint32_t deltaTime) {
  ...
  if (m_bShowMsgIcon) {
    ...
    if (m_bBlindDirectUp)
      blindColor = BIUtils::GetIntervalColor(m_dwHighBlindColor, m_dwLowBlindColor,
                                             m_fCurBlindTime / m_fBlindTimeUp);
    else
      blindColor = BIUtils::GetIntervalColor(m_dwLowBlindColor, m_dwHighBlindColor,
      ...
      for (j = 0; j < m_pMsgColumns[i].rowQ; j++) {
        ...
        if (m_pMsgColumns[i].pRow[j].bDoBlend) {
          ...
          m_pMsgColumns[i].pRow[j].color = BIUtils::GetIntervalColor(
            ARGB(0, 128, 128, 128), ARGB(255, 128, 128, 128),
