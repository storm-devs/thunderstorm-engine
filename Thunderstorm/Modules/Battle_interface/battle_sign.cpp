  ...
  for (long n = 0; n < m_nSignQ; n++) {
    if (m_Sign[n].sText.length() > 0) {
      m_pRS->ExtPrint(m_idSignTextFont, m_dwSignTextFontColor, 0, PR_ALIGN_CENTER, false, m_fSignTextFontScale, 0,
                      0,
                      static_cast<long>(m_Sign[n].pntPos.x) + m_SignTextFontOffset.x,
                      static_cast<long>(m_Sign[n].pntPos.y) + m_SignTextFontOffset.y,
...
void BISignIcon::Init(ATTRIBUTES* pRoot, ATTRIBUTES* pA) {
  ...
  if (pA) {
    ...
    if (pcTmp)
      sscanf(pcTmp, "%f,%f,%f,%f", &m_rSignStateLeftUV.left, &m_rSignStateLeftUV.top,
...
void BISignIcon::UpdateBuffers(long nQ) {
  ...
  const auto nMaxSignQ = BIUtils::GetMaxFromFourLong(nBackSquareQ, nSignStateSquareQ, nSignStarSquareQ,
                                                     nSignFaceSquareQ);
