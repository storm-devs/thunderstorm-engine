	...
	for (long n = 0; n < m_nSignQ; n++)
	{
		if (m_Sign[n].sText.length() > 0)
		{
			m_pRS->ExtPrint(m_idSignTextFont, m_dwSignTextFontColor, 0, PR_ALIGN_CENTER, false, m_fSignTextFontScale, 0,
			                ...
			                (long)m_Sign[n].pntPos.y + m_SignTextFontOffset.y,
...
void BISignIcon::Init(ATTRIBUTES* pRoot, ATTRIBUTES* pA)
{
	...
	if (pA)
	{
		...
		if (pcTmp) sscanf(pcTmp, "%f,%f,%f,%f", &m_rSignStateLeftUV.left, &m_rSignStateLeftUV.top,
		                  &m_rSignStateLeftUV.right, &m_rSignStateLeftUV.bottom);
		...
		if (pcTmp) sscanf(pcTmp, "%f,%f,%f,%f", &m_rSignStateRightUV.left, &m_rSignStateRightUV.top,
		...
		if (pcTmp) sscanf(pcTmp, "%f,%f,%f,%f", &m_rSignStarUV.left, &m_rSignStarUV.top, &m_rSignStarUV.right,
...
void BISignIcon::UpdateBuffers(long nQ)
{
		...
		(m_nBackSquareQ + m_nSignStateSquareQ + m_nSignStarSquareQ + m_nSignFaceSquareQ))
	{
		...
		m_nVBufID = m_pRS->CreateVertexBuffer(BI_COLOR_VERTEX_FORMAT,
		                                      (m_nBackSquareQ + m_nSignStateSquareQ + m_nSignStarSquareQ +
			                                      m_nSignFaceSquareQ) * 4 * sizeof(BI_COLOR_VERTEX),
		                                      D3DUSAGE_WRITEONLY);
	}
	FillVertexBuffer();
}
...
void BISignIcon::FillVertexBuffer()
{
	...
	if (pV)
	{
		...
		for (n = 0; n < m_nSignQ; n++)
		{
			...
			vn += WriteSquareToVBuffWithProgress(&pV[vn], m_rSignStateRightUV, m_dwSignStateColor,
			                                     m_Sign[n].pntPos + m_pntSignStateRightOffset,
			                                     m_pntSignStateRightIconSize, 1.f - m_Sign[n].fRightState, 0.f, 0.f,
			                                     0.f);
		}
