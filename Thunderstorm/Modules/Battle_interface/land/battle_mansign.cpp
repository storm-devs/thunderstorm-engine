...
void BIManSign::Init(ATTRIBUTES* pRoot, ATTRIBUTES* pA)
{
	...
	if (pA)
	{
		...
		if (pcTmp) sscanf(pcTmp, "%f,%f,%f,%f", &m_rAlarmUV.left, &m_rAlarmUV.top, &m_rAlarmUV.right,
		                  &m_rAlarmUV.bottom);
		...
		if (pcTmp) sscanf(pcTmp, "%f,%f,%f,%f", &m_rManHPUV.left, &m_rManHPUV.top, &m_rManHPUV.right,
		                  &m_rManHPUV.bottom);
		...
		if (pcTmp) sscanf(pcTmp, "%f,%f,%f,%f", &m_rManEnergyUV.left, &m_rManEnergyUV.top, &m_rManEnergyUV.right,
		...
		if (pcTmp) sscanf(pcTmp, "%f,%f,%f,%f", &m_rGunChargeUV.left, &m_rGunChargeUV.top, &m_rGunChargeUV.right,
...
void BIManSign::UpdateBuffers(long nShipQ)
{
  ...
  const auto nMaxSquareQ = BIUtils::
    ...
    const long n = 6 * 4 + 1;
		VERTEX_BUFFER_RELEASE(m_pRS, m_nVBufID);
		m_nVBufID = m_pRS->CreateVertexBuffer(BI_COLOR_VERTEX_FORMAT, n * 4 * sizeof(BI_COLOR_VERTEX),
		                                      D3DUSAGE_WRITEONLY);
	}
	//FillVertexBuffer();
}
