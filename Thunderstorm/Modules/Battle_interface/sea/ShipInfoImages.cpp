...
void ShipInfoImages::UpdateShipList()
{
	...
	// lock buffers
	m_pVBuffRelation = (SII_VERTEX*)pRS->LockVertexBuffer( m_vbRelation );
	if( !m_pVBuffRelation ) {
		m_nShipQ = 0;
		return;
	}
	m_pVBuffProgress = (SII_VERTEX*)pRS->LockVertexBuffer( m_vbProgress );
	if( !m_pVBuffProgress ) {
		pRS->UnLockVertexBuffer( m_vbRelation );
		m_nShipQ = 0;
		return;
	}
	m_pVBuffBackProgress = (SII_VERTEX*)pRS->LockVertexBuffer( m_vbBackProgress );
...
void ShipInfoImages::WriteSquareVertex(SII_VERTEX* pV,const CVECTOR& center,const CVECTOR& offset,const FPOINT& size,const FRECT& uv, float fProgress)
{
