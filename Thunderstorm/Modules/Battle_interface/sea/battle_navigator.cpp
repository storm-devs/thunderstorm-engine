...
void BATTLE_NAVIGATOR::Update() {
  ...
  if (pV != nullptr) {
    SetRectangleVertexPos(&pV[4], static_cast<float>(m_XNavigator), static_cast<float>(m_YNavigator),
                          ...
                          static_cast<float>(m_NavigationWidth), -m_fAngle);
  ...
  if (pV != nullptr) {
    SetRectangleSegVertexPos(pV, static_cast<float>(m_XNavigator), static_cast<float>(m_YNavigator),
                             static_cast<float>(m_NavigationWidth),
                             static_cast<float>(m_NavigationWidth), m_fBegAnglWindSpeed, m_fCurAnglWindSpeed);
    ...
    SetRectangleSegVertexPos(&pV[m_nvSpeed], static_cast<float>(m_XNavigator), static_cast<float>(m_YNavigator),
                             static_cast<float>(m_NavigationWidth),
                             static_cast<float>(m_NavigationWidth), m_fCurAnglShipSpeed, m_fBegAnglShipSpeed);
...
void BATTLE_NAVIGATOR::Init(VDX9RENDER* RenderService, Entity* pOwnerEI) {
  ...
  m_fBegAnglForwardCharge = static_cast<float>(BIUtils::GetLongFromAttr(pARoot, "forwardChargeBegAngle", 0)) / 180.f *
    PI;
  m_fEndAnglForwardCharge = static_cast<float>(BIUtils::GetLongFromAttr(pARoot, "forwardChargeEndAngle", 0)) / 180.f *
    PI;

  m_fBegAnglBackCharge = static_cast<float>(BIUtils::GetLongFromAttr(pARoot, "backwardChargeBegAngle", 0)) / 180.f * PI;
  m_fEndAnglBackCharge = static_cast<float>(BIUtils::GetLongFromAttr(pARoot, "backwardChargeEndAngle", 0)) / 180.f * PI;

  // get speed angles
  m_fBegAnglShipSpeed = static_cast<float>(BIUtils::GetLongFromAttr(pARoot, "shipSpeedBegAngle", 0)) / 180.f * PI;
  m_fCurAnglShipSpeed = m_fEndAnglShipSpeed = static_cast<float>(BIUtils::GetLongFromAttr(
      pARoot, "shipSpeedEndAngle", 0)) / 180.f
    * PI;
  m_fBegAnglWindSpeed = static_cast<float>(BIUtils::GetLongFromAttr(pARoot, "windSpeedBegAngle", 0)) / 180.f * PI;
  m_fCurAnglWindSpeed = m_fEndAnglWindSpeed = static_cast<float>(BIUtils::GetLongFromAttr(
      pARoot, "windSpeedEndAngle", 0)) / 180.f
    * PI;
...
void BATTLE_NAVIGATOR::UpdateWindParam() {
  if (!m_wb && !m_pAWeather) {
    ...
    /*if( core.IsNetActive() && !m_wb && m_pOwnerEI ) {
      VDATA * pSVWeather = (VDATA*)core.GetScriptVariable((m_pOwnerEI->IsServer()) ? "NSWeather" : "NCWeather"); Assert(pSVWeather);
