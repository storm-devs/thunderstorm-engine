...
void BATTLE_NAVIGATOR::Update()
{
	...
	if (pV != nullptr)
	{
		SetRectangleVertexPos(&pV[4], (float)m_XNavigator, (float)m_YNavigator, (float)m_NavigationWidth,
		                      (float)m_NavigationWidth, -m_fAngle);
		SetRectangleVertexPos(&pV[8], (float)m_XNavigator, (float)m_YNavigator, (float)m_windWidth, (float)m_windHeight,
	...
	if (pV != nullptr)
	{
		...
		{
			SetRectangleSegVertexPos(pV, (float)m_XNavigator, (float)m_YNavigator, (float)m_NavigationWidth,
			                         (float)m_NavigationWidth, m_fBegAnglLeftCharge, m_fCurAnglLeftCharge);
			m_nvCannonCharge = SetRectangleSegVertexTex(pV, .5f, .5f, 1.f, 1.f, m_fBegAnglLeftCharge,
			                                            m_fCurAnglLeftCharge);
		}
		if (!FloatCompare(m_fCurAnglRightCharge, m_fCurAnglRightDamage))
		{
			SetRectangleSegVertexPos(&pV[m_nvCannonCharge], (float)m_XNavigator, (float)m_YNavigator,
			                         (float)m_NavigationWidth, (float)m_NavigationWidth, m_fCurAnglRightCharge,
		...
		{
			SetRectangleSegVertexPos(&pV[m_nvCannonCharge], (float)m_XNavigator, (float)m_YNavigator,
			                         (float)m_NavigationWidth, (float)m_NavigationWidth, m_fBegAnglForwardCharge,
		...
		{
			SetRectangleSegVertexPos(&pV[m_nvCannonCharge], (float)m_XNavigator, (float)m_YNavigator,
			                         (float)m_NavigationWidth, (float)m_NavigationWidth, m_fCurAnglBackCharge,
		...
		{
			SetRectangleSegVertexPos(&pV[m_nvCannonCharge], (float)m_XNavigator, (float)m_YNavigator,
			                         (float)m_NavigationWidth, (float)m_NavigationWidth, m_fBegAnglLeftCharge,
			                         m_fCurAnglLeftCharge);
		...
		if (FloatCompare(m_fCurAnglRightCharge, m_fCurAnglRightDamage))
		{
			SetRectangleSegVertexPos(&pV[m_nvCannonCharge + m_nvCannonReady], (float)m_XNavigator, (float)m_YNavigator,
			                         (float)m_NavigationWidth, (float)m_NavigationWidth, m_fCurAnglRightCharge,
			                         m_fBegAnglRightCharge);
		...
		{
			SetRectangleSegVertexPos(&pV[m_nvCannonCharge + m_nvCannonReady], (float)m_XNavigator, (float)m_YNavigator,
			                         (float)m_NavigationWidth, (float)m_NavigationWidth, m_fCurAnglLeftDamage,
		...
		{
			SetRectangleSegVertexPos(&pV[m_nvCannonCharge + m_nvCannonReady + m_nvCannonDamage], (float)m_XNavigator,
			                         (float)m_YNavigator, (float)m_NavigationWidth, (float)m_NavigationWidth,
			                         m_fEndAnglRightCharge, m_fCurAnglRightDamage);
			m_nvCannonDamage += SetRectangleSegVertexTex(&pV[m_nvCannonCharge + m_nvCannonReady + m_nvCannonDamage],
			                                             .5f, .5f, 1.f, 1.f, m_fEndAnglRightCharge,
		...
		if (!FloatCompare(m_fCurAnglForwardDamage, m_fEndAnglForwardCharge))
		{
			...
			m_nvCannonDamage += SetRectangleSegVertexTex(&pV[m_nvCannonCharge + m_nvCannonReady + m_nvCannonDamage],
			                                             .5f, .5f, 1.f, 1.f, m_fCurAnglForwardDamage,
		...
		if (!FloatCompare(m_fCurAnglBackDamage, m_fEndAnglBackCharge))
		{
			...
			m_nvCannonDamage += SetRectangleSegVertexTex(&pV[m_nvCannonCharge + m_nvCannonReady + m_nvCannonDamage],
			                                             .5f, .5f, 1.f, 1.f, m_fEndAnglBackCharge,
	...
	{
		SetRectangleSegVertexPos(pV, (float)m_XNavigator, (float)m_YNavigator, (float)m_NavigationWidth,
		                         (float)m_NavigationWidth, m_fBegAnglWindSpeed, m_fCurAnglWindSpeed);
		m_nvSpeed = SetRectangleSegVertexTex(pV, .5f, .5f, 1.f, 1.f, m_fBegAnglWindSpeed, m_fCurAnglWindSpeed);
		SetRectangleSegVertexPos(&pV[m_nvSpeed], (float)m_XNavigator, (float)m_YNavigator, (float)m_NavigationWidth,
		                         (float)m_NavigationWidth, m_fCurAnglShipSpeed, m_fBegAnglShipSpeed);
		m_nvSpeed += SetRectangleSegVertexTex(&pV[m_nvSpeed], .5f, .5f, 1.f, 1.f, m_fCurAnglShipSpeed,
		                                      m_fBegAnglShipSpeed);
		m_nvSpeed -= 2;
...
void BATTLE_NAVIGATOR::Init(VDX9RENDER* RenderService, Entity* pOwnerEI)
{
	...
	if (tmpstr == nullptr) m_idPowderTexture = -1;
	else m_idPowderTexture = rs->TextureCreate(tmpstr);
	...
	m_fCurAnglShipSpeed = m_fEndAnglShipSpeed = (float)BIUtils::GetLongFromAttr(pARoot, "shipSpeedEndAngle", 0) / 180.f
		* PI;
	m_fBegAnglWindSpeed = (float)BIUtils::GetLongFromAttr(pARoot, "windSpeedBegAngle", 0) / 180.f * PI;
	m_fCurAnglWindSpeed = m_fEndAnglWindSpeed = (float)BIUtils::GetLongFromAttr(pARoot, "windSpeedEndAngle", 0) / 180.f
		* PI;
	...
	// create buffers
	m_idEmptyVBuf = rs->CreateVertexBuffer(BI_ONETEX_VERTEX_FORMAT, (4 + 4 + 4) * sizeof(BI_ONETEXTURE_VERTEX),
	                                       D3DUSAGE_WRITEONLY);
	m_idCannonVBuf = rs->CreateVertexBuffer(BI_ONETEX_VERTEX_FORMAT, 7 * 4 * sizeof(BI_ONETEXTURE_VERTEX),
	                                        D3DUSAGE_WRITEONLY);
	m_idSpeedVBuf = rs->CreateVertexBuffer(BI_ONETEX_VERTEX_FORMAT, 7 * 2 * sizeof(BI_ONETEXTURE_VERTEX),
	                                       D3DUSAGE_WRITEONLY);
	m_idMapVBuf = rs->CreateVertexBuffer(BI_ONETEX_VERTEX_FORMAT, (RADIAL_QUANTITY + 2) * sizeof(BI_ONETEXTURE_VERTEX),
	                                     D3DUSAGE_WRITEONLY);
	...
	m_idGradBackVBuf = rs->CreateVertexBuffer(BI_COLORONLY_VERTEX_FORMAT, 3 * sizeof(BI_COLORONLY_VERTEX),
	                                          D3DUSAGE_WRITEONLY);
	m_idCurChargeVBuf = rs->CreateVertexBuffer(BI_ONETEX_VERTEX_FORMAT, 4 * 4 * sizeof(BI_ONETEXTURE_VERTEX),
	                                           D3DUSAGE_WRITEONLY);
	if (m_idEmptyVBuf == -1 || m_idCannonVBuf == -1 || m_idSpeedVBuf == -1 || m_idMapVBuf == -1 || m_idFireZoneVBuf == -
		1 || m_idShipsVBuf == -1 || m_idGradBackVBuf == -1 || m_idCurChargeVBuf == -1)
	{
		throw std::exception("Can`t create vertex\index buffer");
	}
	m_nvShips = 0;
	...
	if (pV != nullptr)
	{
		...
		SetRectangleVertexPos(&pV[12], (float)m_PowderPos.x, (float)m_PowderPos.y,
		                      (float)m_PowderSize.x, (float)m_PowderSize.y);
...
void BATTLE_NAVIGATOR::SetMainCharacterData()
{
	...
	if (m_fWindStrength > m_fWindMaxStrength)
		...
		m_fCurAnglWindSpeed = GetBetwinFloat(m_fBegAnglWindSpeed, m_fEndAnglWindSpeed,
		                                     m_fWindStrength / m_fWindMaxStrength);
...
void BATTLE_NAVIGATOR::UpdateCurrentCharge()
{
	...
	if (curCharge == m_curCharge && curSailState == m_curSailState && curWindPic == m_curWindPic && curPowder ==
		m_curPowder) return;
