...
void BIShipCommandList::FillIcons()
{
	...
	if (m_nCurrentCommandMode & BI_COMMODE_MY_SHIP_SELECT)
		nIconsQuantity += ShipAdding((m_nCurrentCommandMode & BI_COMMODE_ALLLOCATOR_SELECT) != 0, true, false, false,
		                             false);
	if (m_nCurrentCommandMode & BI_COMMODE_NEUTRAL_SHIP_SELECT)
		nIconsQuantity += ShipAdding((m_nCurrentCommandMode & BI_COMMODE_ALLLOCATOR_SELECT) != 0, false, false, true,
		                             false);
	if (m_nCurrentCommandMode & BI_COMMODE_FRIEND_SHIP_SELECT)
		nIconsQuantity += ShipAdding((m_nCurrentCommandMode & BI_COMMODE_ALLLOCATOR_SELECT) != 0, false, false, false,
		                             true);
	if (m_nCurrentCommandMode & BI_COMMODE_ENEMY_SHIP_SELECT)
		nIconsQuantity += ShipAdding((m_nCurrentCommandMode & BI_COMMODE_ALLLOCATOR_SELECT) != 0, false, true, false,
		                             false);
	...
	if (m_nCurrentCommandMode & BI_COMMODE_ENEMY_TOWN)
		nIconsQuantity += TownAdding((m_nCurrentCommandMode & BI_COMMODE_ALLLOCATOR_SELECT) != 0, true, true, true,
		                             false, false);

	if (m_nCurrentCommandMode & BI_COMMODE_DISEASED_TOWN)
		nIconsQuantity += TownAdding((m_nCurrentCommandMode & BI_COMMODE_ALLLOCATOR_SELECT) != 0, true, false, true,
		                             true, true);

	if (m_nCurrentCommandMode & BI_COMMODE_NOTDISEASED_TOWN)
		nIconsQuantity += TownAdding((m_nCurrentCommandMode & BI_COMMODE_ALLLOCATOR_SELECT) != 0, false, true, true,
		                             true, true);
	...
	for (; sd != nullptr; sd = sd->next)
	{
		if (sd != selShip && sd != main_sd)
		{
				...
				!bMyShip && bNeutral && !sd->isMyShip && sd->relation == BI_RELATION_NEUTRAL)
			{
				...
				{
					VDATA* pvdat = api->Event("evntCheckEnableShip", "sl", m_sCurrentCommandName.c_str(),
					                          sd->characterIndex);
