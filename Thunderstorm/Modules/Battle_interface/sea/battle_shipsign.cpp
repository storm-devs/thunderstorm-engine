  ...
  for (long n = 0; n < m_nShipQuantity; n++) {
    if (m_Ship[n].pASailorQuantity) {
      m_pRS->ExtPrint(m_idSailorFont, m_dwSailorFontColor, 0, PR_ALIGN_CENTER, true, m_fSailorFontScale, 0, 0,
                      ...
                      static_cast<long>(m_Ship[n].pntPos.y) + m_SailorFontOffset.y,
                      "%d", static_cast<long>(atof(m_Ship[n].pASailorQuantity->GetThisAttr())));
    }
    if (!m_Ship[n].sShipName.empty()) {
      m_pRS->ExtPrint(m_idShipNameFont, m_dwShipNameFontColor, 0, PR_ALIGN_CENTER, true, m_fShipNameFontScale, 0,
                      ...
                      static_cast<long>(m_Ship[n].pntPos.y) + m_ShipNameFontOffset.y,
...
void BIShipIcon::Init(ATTRIBUTES* pRoot, ATTRIBUTES* pA) {
  ...
  if (pA) {
    ...
    if (pcTmp)
      sscanf(pcTmp, "%f,%f,%f,%f", &m_rShipHPUV.left, &m_rShipHPUV.top, &m_rShipHPUV.right,
             &m_rShipHPUV.bottom);
    ...
    if (pcTmp)
      sscanf(pcTmp, "%f,%f,%f,%f", &m_rShipSPUV.left, &m_rShipSPUV.top, &m_rShipSPUV.right,
             &m_rShipSPUV.bottom);
...
void BIShipIcon::ExecuteCommand(CommandType command) {
  ...
  case Command_up:
    if (m_nCurrentShipIndex > 0) {
      ...
      UpdateCommandList();
      if (m_pCommandList)
        m_pCommandList->SetUpDown(m_nCurrentShipIndex > 0,
