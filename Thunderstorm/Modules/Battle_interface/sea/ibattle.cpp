...
void BATTLE_INTERFACE::LoadIniFile() {
  ...
  m_pShipInfoImages = new ShipInfoImages(rs, AttributesPointer
                                               ? AttributesPointer->GetAttributeClass("ShipInfoImages")
                                               : nullptr);
  if (m_pShipInfoImages) {
    m_pShipInfoImages->SetVisible(AttributesPointer
                                    ? (AttributesPointer->GetAttributeAsDword("ShifInfoVisible", 0) != 0)
                                    : false);
  }
}
