...
void ISPYGLASS::ImageParam::LoadFromAttr(BIImageRender* pImgRender, ATTRIBUTES* pA, const char* pcDefName,
  ...
  pImage = static_cast<BIImage*>(pImgRender->CreateImage(BIType_square, sTextureName.c_str(), dwColor, rUV, rPos,
                                                         nPrior));
  Assert(pImage);
}
...
void ISPYGLASS::SetShipInfo(long nCharIndex) {
  ...
  if (m_nInfoCharacterIndex < 0) m_bIsPresentShipInfo = false;
  else m_bIsPresentShipInfo = true;
...
void ISPYGLASS::ChangeTelescopeType(const char* pcTextureName, float fZoomScale, float fActivateTime,
                                    float fUpdateTime) {
  m_Camera.fActivateTime = fActivateTime;
  m_Camera.fUpdateTime = fUpdateTime;
  if (fZoomScale > 0.f) m_Camera.fSpyGlassPerspective = m_Camera.fOldPerspective / fZoomScale;
  else m_Camera.fSpyGlassPerspective = m_Camera.fOldPerspective;
...
void ISPYGLASS::UpdateCamera() {
  ...
  if (!m_Camera.bIsActive) {
    ...
    if (m_Camera.fCurActivateTime >= m_Camera.fActivateTime) {
      ...
      if (m_Camera.bIsGrow) m_Camera.bIsActive = true; // теперь мы работаем по полной
      m_Camera.bIsGrow = !m_Camera.bIsGrow; // сразу готовы
    }
  }

  if (m_Camera.bIsActive) rs->SetPerspective(m_Camera.fSpyGlassPerspective);
  else {
...
void ISPYGLASS::ChangeTargetData(const char* pcShipName, const char* pcShipType,
                                 float fRelativeHP, float fRelativeSP, long nShipCrew, float fShipSpeed, float fSailTo,
                                 long nCurCannons, long nMaxCannons,
  ...
  if (nCurCannons >= 0 && nMaxCannons >= 0) {
    ...
    m_txtCannons.sText = "";
    if (m_Cannon.pImage) m_Cannon.pImage->CutSide(0.f, 1.f, 0.f, 0.f); // прячем boal
  }

  if (nShipClass > 0) {
    float fCutVal = 1.f;
    switch (nShipClass) {
    case 1: fCutVal = 1.f - 13.f / 64.f;
      break;
    case 2: fCutVal = 1.f - 22.f / 64.f;
      break;
    case 3: fCutVal = 1.f - 32.f / 64.f;
      break;
    case 4: fCutVal = 1.f - 40.f / 64.f;
      break;
    case 5: fCutVal = 1.f - 50.f / 64.f;
      break;
    case 6: fCutVal = 0.f;
      break;
    case 7: fCutVal = 0.f;
  ...
  if (nCharge >= 0 && nCharge < m_aChargeUV.size()) {
    ...
    if (m_Charge.pImage) m_Charge.pImage->CutSide(0.f, 1.f, 0.f, 0.f); // прячем boal
  }
