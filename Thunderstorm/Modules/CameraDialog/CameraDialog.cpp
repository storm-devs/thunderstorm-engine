...
void CameraDialog::Execute(uint32_t Delta_Time) {
  ...
  auto dtime = 1.0f / static_cast<float>(api->EngineFps());
  ...
  auto* mdl = static_cast<MODEL*>(EntityManager::GetEntityPointer(person));
  ...
  if (atr == nullptr) perMtx.Pos().y += HDISP1;
    ...
    else if (_strcmpi(mdlType, "sit") == 0) perMtx.Pos().y += 1.2f;
  ...
  auto FPS = 15.0f;
  auto fr = static_cast<long>(floorf(FPS * time)) % (frames - 1);
...
void CameraDialog::Realize(uint32_t Delta_Time) {
  ...
  if ((sti = root->geo->FindLabelN(sti + 1, idSiber)) > -1) {
    ...
    auto* smdl = static_cast<MODEL*>(EntityManager::GetEntityPointer(sMod[0]));
    smdl->mtx = perMtx;
  }
  ...
  if ((sti = root->geo->FindLabelN(sti + 1, idSiber)) > -1) {
    ...
    mbn.Vz().x *= -1.0f;
    auto perMtx = mbn * mdl->mtx;
    auto* smdl = static_cast<MODEL*>(EntityManager::GetEntityPointer(sMod[1]));
    smdl->mtx = perMtx;
  }
}
