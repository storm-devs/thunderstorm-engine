			...
			for (long n = 0; n < nKeyQ; n++)
			{
				if (pKeys[n].bSystem)
				{
					...
					case VK_BACK:
						...
						break;
					case VK_RIGHT:
						if (nEditCharIndex < (long)asText[nTextIdx].size()) nEditCharIndex++;
						break;
					case VK_DELETE:
						if (nEditCharIndex >= 0 && nEditCharIndex < (long)asText[nTextIdx].size())
						{
							asText[nTextIdx].erase(nEditCharIndex, 1);
						}
						break;
...
void DIALOG::CreateBack()
{
  ...
  auto* const pI = (uint16_t*)RenderService->LockIndexBuffer(m_idIBufBack);
	if (pI)
	{
