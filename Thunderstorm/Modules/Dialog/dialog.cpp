...
long DIALOG::DlgTextDescribe::GetShowHeight() {
  ...
  for (n = 0; n < anPageEndIndex.size(); n++)
    if (nStartIndex < anPageEndIndex[n])
      break;
  if (n < anPageEndIndex.size()) n = anPageEndIndex[n] - nStartIndex;
...
void DIALOG::DlgTextDescribe::Show(long nY) {
  ...
  for (n = 0; n < anPageEndIndex.size(); n++)
    if (anPageEndIndex[n] > nStartIndex)
      break;
  if (n < anPageEndIndex.size()) nEnd = anPageEndIndex[n];
...
bool DIALOG::DlgTextDescribe::IsLastPage() {
  ...
  if (n >= anPageEndIndex.size() || anPageEndIndex[n] >= asText.size()) return true;
  return false;
}
      ...
      for (long n = 0; n < nKeyQ; n++) {
        if (pKeys[n].bSystem) {
          ...
          case VK_BACK:
            ...
            break;
          case VK_RIGHT:
            if (nEditCharIndex < static_cast<long>(asText[nTextIdx].size())) nEditCharIndex++;
            break;
          case VK_DELETE:
...
void DIALOG::CreateBack() {
  ...
  if (m_idVBufBack == -1)
    m_idVBufBack = RenderService->CreateVertexBuffer(XI_TEX_FVF, m_nVQntBack * sizeof(XI_TEX_VERTEX),
                                                     D3DUSAGE_WRITEONLY);
  if (m_idIBufBack == -1)
...
void DIALOG::CreateButtons() {
  ...
  if (m_idVBufButton == -1)
    m_idVBufButton = RenderService->CreateVertexBuffer(XI_TEX_FVF, m_nVQntButton * sizeof(XI_TEX_VERTEX),
                                                       D3DUSAGE_WRITEONLY);
  if (m_idIBufButton == -1)
...
void DIALOG::FillButtons() {
  ...
  if (m_dwButtonState & BUTTON_STATE_UPLIGHT) {
    SetVerticesForSquare(&pV[0], m_ButtonParams.frUpLightButtonUV,
                         m_BackParams.m_frBorderInt.right - m_ButtonParams.fRightOffset - m_ButtonParams
                         .fpButtonSize.x,
                         m_BackParams.m_frBorderInt.top + m_ButtonParams.fTopOffset,
                         m_BackParams.m_frBorderInt.right - m_ButtonParams.fRightOffset,
                         m_BackParams.m_frBorderInt.top + m_ButtonParams.fTopOffset + m_ButtonParams
                         .fpButtonSize.y);
  }
  else {
  ...
  if (m_dwButtonState & BUTTON_STATE_DOWNLIGHT) {
    SetVerticesForSquare(&pV[4], m_ButtonParams.frDownLightButtonUV,
                         m_BackParams.m_frBorderInt.right - m_ButtonParams.fRightOffset - m_ButtonParams
                         .fpButtonSize.x,
                         m_BackParams.m_frBorderInt.bottom - m_ButtonParams.fBottomOffset - m_ButtonParams
                         .fpButtonSize.y,
                         m_BackParams.m_frBorderInt.right - m_ButtonParams.fRightOffset,
...
void DIALOG::Realize(uint32_t Delta_Time) {
  ...
  RenderService->ExtPrint(m_nCharNameTextFont, m_dwCharNameTextColor, 0, PR_ALIGN_LEFT, true, m_fCharNameTextScale, 0,
                          0,
                          static_cast<long>(m_BackParams.m_frBorderExt.left + m_fpCharNameTextOffset.x),
                          static_cast<long>(m_BackParams.m_frBorderExt.top + m_fpCharNameTextOffset.y),
