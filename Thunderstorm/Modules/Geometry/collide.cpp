...
rec_loop:;
    ...
    //if(dist<=diss)
  {
    ...
    else if (node->right == 0) goto rec_avoid;
    else node = &sroot[node->node + node->right];
    goto rec_loop;
  }
    ...
    //if(dist>=dise)
  {
    if (ssrc < 0.0) //left
    {
      ...
      if (node->right == 0) goto rec_avoid;
      else node = &sroot[node->node + node->right];
    goto rec_loop;
  }
  //----------first test----------
  ...
  else if (node->left == 0) second = nullptr;
...
rec_return:;
  ...
  loop0:
    const auto face = (static_cast<long>(*(pface + 2)) << 16) | (static_cast<long>(*(pface + 1)) << 8) | (static_cast<
      long>(*(pface + 0)) << 0);
    long vindex[3];
  ...
  for (i = 0; i < n; i++) {
    ...
    if (sign[0] > 0.0) //outside!
    {
      ...
      if (plane.nrm.x * poly[ii].x + plane.nrm.y * poly[ii].y + plane.nrm.z * poly[ii].z - plane.d <= 0.0)
        //inside!
      {
        for (i3 = n; i3 > ii; i3--) poly[i3] = poly[i3 - 1];
        if (i > ii) i++;
        n++;
        goto ClipEPH;
      }
    ClipBPH:
      ...
      if (plane.nrm.x * poly[ii].x + plane.nrm.y * poly[ii].y + plane.nrm.z * poly[ii].z - plane.d <= 0)
        goto
          ClipEPH; //inside!
      for (i4 = i3; i4 < n - 1; i4++) poly[i4] = poly[i4 + 1];
      n--;
      if (i3 < i) i--;
      if (i3 < ii) ii--;
      goto ClipBPH;
...
rec_loop:;
  ...
  if (ssrc * ssrc < radius * radius) {
    ...
    for (unsigned long f = 0; f < node->nfaces; f++) {
      const long face = (static_cast<long>(*(pface + 2)) << 16) | (static_cast<long>(*(pface + 1)) << 8) | (static_cast<
        long>(*(pface + 0)) << 0);
    ...
    else if (node->right == 0) goto rec_continue;
    else node = &sroot[node->node + node->right];
  }
