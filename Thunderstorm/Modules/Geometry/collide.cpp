...
long ClipByPlane(const GEOS::PLANE &plane, long n)
{
	...
	for(i=0; i<n; i++)
	{
		...
		if(sign[0]>0.0 )//outside!
		{
			...
			ii=i+1; if(ii>=n)	ii=0;
			if(plane.nrm.x*poly[ii].x + plane.nrm.y*poly[ii].y + plane.nrm.z*poly[ii].z - plane.d<=0.0)//inside!
			{
 	...
 	if(ssrc*ssrc < radius*radius)
	{
		...
		for(unsigned long f=0; f<node->nfaces; f++)
		{
			...
			if(trgclip[face]!=attempt)
			{
				...
				for(long v=0; v<3; v++)
				{
					long vindex = (btrg[face].vindex[v][0]<<0) | (btrg[face].vindex[v][1]<<8) | (btrg[face].vindex[v][2]<<16);
