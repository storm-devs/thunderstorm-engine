    ...
    for (long i = 0; i < aFoams.size(); i++) {
      ...
      for (long j = 0; j < pF->aFoamParts.size(); j++) {
        ...
        if (bEditMode) {
          ...
          mWVP.Projection(&v1, &vP1, 1, static_cast<float>(vp.Width) * 0.5f, static_cast<float>(vp.Height) * 0.5f,
                          sizeof(CVECTOR),
                          sizeof(MTX_PRJ_VECTOR));
          mWVP.Projection(&v2, &vP2, 1, static_cast<float>(vp.Width) * 0.5f, static_cast<float>(vp.Height) * 0.5f,
                          sizeof(CVECTOR),
                          sizeof(MTX_PRJ_VECTOR));
          auto fR1 = 0.15f;
          auto fR2 = 0.15f;
      ...
      for (long i = 0; i < aFoams.size(); i++) {
        ...
        for (long j = 0; j < pF->aFoamParts.size(); j++) {
          for (long k = 0; k < 2; k++) {
            ...
            mWVP.Projection(&v, &vP, 1, static_cast<float>(vp.Width) * 0.5f, static_cast<float>(vp.Height) * 0.5f,
                            sizeof(CVECTOR),
                            sizeof(MTX_PRJ_VECTOR));
  ...
  for (long k = 0; k < pF->iNumFoams; k++) {
    ...
    for (long y = 0; y < 8; y++) {
      ...
      for (long x = 0; x < iLen; x++) {
        ...
        const auto dwColor = ARGB(static_cast<uint32_t>(pF->fAlphaColor[k] * fAlpha * fAlpha1 * 255.0f), 255, 255, 255);
