...
bool ISLAND::CreateShadowMap(char* pDir, char* pName) {
  ...
  auto* const pWeather = static_cast<WEATHER_BASE*>(EntityManager::GetEntityPointer(
    EntityManager::GetEntityId("Weather")));
  if (pWeather == nullptr)
...
bool ISLAND::CreateHeightMap(char* pDir, char* pName) {
  ...
  for (fZ = 0; fZ < static_cast<float>(iDMapSize); fZ += 1.0f) {
    if ((static_cast<long>(fZ) & 127) == 127) core.Trace("Z = %.0f", fZ);
    for (fX = 0; fX < static_cast<float>(iDMapSize); fX += 1.0f) {
      ...
      if (fRes <= 1.0f) // island ocean floor exist
      {
        ...
        if (ActivateCamomileTrace(vSrc))
          ...
          pDepthMap[iIdx] = static_cast<unsigned char>((HMAP_START + static_cast<float>(HMAP_NUMBERS) * fHeight / -
            HMAP_MAXHEIGHT));
      }
