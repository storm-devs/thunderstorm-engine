  ...
  for (long i = 0; i < numObjects; i++) {
    ...
    for (long n = 0; n < info.nobjects; n++) {
      ...
      for (vb = 0; vb < numVBuffers; vb++)
        if (vbuffer[vb].vbID == static_cast<long>(obj.vertex_buff)) break;
      if (vb >= numVBuffers) {
...
void LGeometry::UpdateColors(VDX9RENDER* rs) {
  ...
  for (long i = 0; i < numVrt; i++) {
    ...
    clr = (static_cast<uint32_t>(c.x) << 16) | (static_cast<uint32_t>(c.y) << 8) | (static_cast<uint32_t>(c.z) << 0) |
      vrt[i].alpha;
  }
  if (lockedVB >= 0) rs->UnLockVertexBuffer(lockedVB);
}
...
bool LGeometry::Save() {
  ...
  for (long i = 0, pnt = 0; i < numObjects; i++) {
    ...
    for (long j = 0, n = object[i].lBufSize; j < n; j++) {
      ...
      if (c.z < 0.0f) c.z = 0.0f;
      if (c.z > 255.0f) c.z = 255.0f;
      buf[sv++] = (static_cast<uint32_t>(c.x) << 16) | (static_cast<uint32_t>(c.y) << 8) | (static_cast<uint32_t>(c.z)
        << 0) | 0xff000000;
      if (sv >= bufSize) {
