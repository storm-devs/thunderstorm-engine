...
void Window::Draw(float dltTime) {
  ...
  if (GetAsyncKeyState(VK_CONTROL) < 0) {
    ...
    else if (GetAsyncKeyState(VK_NUMPAD7) < 0) {
      selected = 7;
    }
  ...
  if (selected >= 0) {
    ...
    case ListElement::t_blur:
      ...
      isResetBlurLight = Button(winw * 0.5f + 10.0f, winh + list[selected].h - 40.0f, winw * 0.5f - 30.0f, 30.0f,
                                "Reset blur");
      break;
  ...
  if (isList) {
    ...
    if (isCheck && cursy <= lsty + lstbw) {
      ...
      cl = 0xff000000;
      if (isMouseDown) {
        if (GetAsyncKeyState(VK_SHIFT) < 0) listPos -= 3.0f;
        else listPos -= 1.0f;
      }
    }
...
void Window::LoadPreset(long prs) {
  ...
  for (long i = 0; i < numElements; i++) {
    ...
    case ListElement::t_light:
      ...
      *list[i].isOn = ini->GetLong(sect, GenerateName(list[i].name, "_isOn"), static_cast<long>(*list[i].isOn)) != 0;
      break;
    case ListElement::t_glight:
      list[i].st = static_cast<float>(ini->GetDouble(sect, GenerateName(list[i].name, "_intens"), list[i].st));
      list[i].c.x = static_cast<float>(ini->GetDouble(sect, GenerateName(list[i].name, "_clrR"), list[i].c.x));
