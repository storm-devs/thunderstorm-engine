...
void NPCharacter::Update(float dltTime) {
  ...
  if (location->IsDebugView()) {
    ...
    if (isDebugEx)
      location->Print(curPos + CVECTOR(0.0f, height, 0.0f), rad, 1, 1.0f, 0xffffff, 0.5f, "target(%s)",
                      fid);
    if (isDebugEx)
    ...
    if (isDebugEx)
      location->Print(curPos + CVECTOR(0.0f, height, 0.0f), rad, -2, 1.0f, 0xffffff, 0.5f,
    ...
    if (isDebugEx && AttributesPointer) {
      ...
      if (atr) {
        for (long i = atr->GetAttributesNum() - 1; i >= 0; i--) {
          ...
          if (fid && id)
            location->Print(curPos + CVECTOR(0.0f, height, 0.0f), rad, line++, 1.0f, 0xffffff,
...
NPCharacter::NPCTask NPCharacter::GetTaskID(const char* taskName) {
  ...
  for (long i = 0; i < npct_max; i++) {
    ...
    if (_stricmp(task, taskName) == 0) return static_cast<NPCTask>(i);
  }
  return npct_unknow;
}
