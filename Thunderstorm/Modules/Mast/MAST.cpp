...
void MAST::Mount(entid_t modelEI, entid_t shipEI, NODE* mastNodePointer) {
  ...
  if (mastNodePointer != nullptr) //~!~
  {
    ...
    mm.dang = CVECTOR(MIN_X_DANG + VAR_X_DANG * static_cast<float>(rand()) / static_cast<float>(RAND_MAX),
                      0.f, MIN_Z_DANG + VAR_Z_DANG * static_cast<float>(rand()) / static_cast<float>(RAND_MAX));
