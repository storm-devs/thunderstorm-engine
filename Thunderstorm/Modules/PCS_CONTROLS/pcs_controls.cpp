...
long PCS_CONTROLS::AddControlTreeNode(long nParent, const char* pcBaseControl, const char* pcOutControl,
                                      float fTimeOut) {
...
bool PCS_CONTROLS::GetControlState(long control_code, CONTROL_STATE& _state_struct) {
  ...
  if (ControlsTab[system_code].update_frame == nFrameCounter) {
    ...
    if (system_code == CE_MOUSE_X_AXIS || system_code == CE_MOUSE_Y_AXIS) {
      ...
      if (lRes < 0) {
        ...
        if (ControlsTab[system_code].state.state == CST_INACTIVE || ControlsTab[system_code].state.state ==
          CST_INACTIVATED) {
          ControlsTab[system_code].state.state = CST_ACTIVATED;
        }
        else ControlsTab[system_code].state.state = CST_ACTIVE;
      }
      else {
        ControlsTab[system_code].state.lValue = 0;
        ControlsTab[system_code].state.fValue = 0;

        if (ControlsTab[system_code].state.state == CST_ACTIVE || ControlsTab[system_code].state.state ==
          CST_ACTIVATED) {
          ControlsTab[system_code].state.state = CST_INACTIVATED;
        }
        else ControlsTab[system_code].state.state = CST_INACTIVE;
      }
    }
  }
  ...
  if (pUserControls[control_code].flags & INVERSE_CONTROL) {
    ...
    case CST_ACTIVE: _state_struct.state = CST_INACTIVE;
      break;
    case CST_INACTIVE: _state_struct.state = CST_ACTIVE;
      break;
    case CST_ACTIVATED: _state_struct.state = CST_INACTIVATED;
      break;
    case CST_INACTIVATED: _state_struct.state = CST_ACTIVATED;
      break;
    }
  }
