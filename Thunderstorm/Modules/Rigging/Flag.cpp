...
void FLAG::Realize(uint32_t Delta_Time) {
  if (bUse) {
    ...
    // draw nature flag
    if (nVert != 0 && nIndx != 0)
      RenderService->DrawBuffer(vBuf, sizeof(FLAGLXVERTEX), iBuf, 0, nVert, 0, nIndx,
                                "ShipFlag");
...
void FLAG::DoMove(FLAGDATA* pr, float delta_time) const {
  ...
  if ((pr->Alfa += (ALFA_DEPEND + ALFA_RAND * static_cast<float>(rand()) / static_cast<float>(RAND_MAX)) * delta_time) >
    PIm2) pr->Alfa = 0.f;
  if ((pr->Beta += (BETA_DEPEND + BETA_RAND * static_cast<float>(rand()) / static_cast<float>(RAND_MAX)) * delta_time) >
    PIm2) pr->Beta = 0.f;
  auto Alfa = -pr->Alfa;
  auto Beta = -pr->Beta;
...
void FLAG::LoadIni() {
  ...
  FlagTextureQuantity = static_cast<int>(ini->GetLong(section, "TextureCount", 1));
  if (FlagTextureQuantity != tmp) texChange = true;
  //---
  tmp = ENGLISH_PENNON_TEX;
  ENGLISH_PENNON_TEX = static_cast<int>(ini->GetLong(section, "EnglandPennonTex", 0));
  if (ENGLISH_PENNON_TEX != tmp) texChange = true;
  //---
  tmp = SPANISH_PENNON_TEX;
  SPANISH_PENNON_TEX = static_cast<int>(ini->GetLong(section, "SpanishPennonTex", 0));
  if (SPANISH_PENNON_TEX != tmp) texChange = true;
  //---
  tmp = FRENCH_PENNON_TEX;
  FRENCH_PENNON_TEX = static_cast<int>(ini->GetLong(section, "FrenchPennonTex", 0));
  if (FRENCH_PENNON_TEX != tmp) texChange = true;
  //---
  tmp = PIRATE_PENNON_TEX;
  PIRATE_PENNON_TEX = static_cast<int>(ini->GetLong(section, "PiratePennonTex", 0));
  if (PIRATE_PENNON_TEX != tmp) texChange = true;
  //---
  tmp = ENGLISH_FLAG_TEX;
  ENGLISH_FLAG_TEX = static_cast<int>(ini->GetLong(section, "EnglishFlagTex", 0));
  if (ENGLISH_FLAG_TEX != tmp) texChange = true;
  //---
  tmp = SPANISH_FLAG_TEX;
  SPANISH_FLAG_TEX = static_cast<int>(ini->GetLong(section, "SpanishFlagTex", 0));
  if (SPANISH_FLAG_TEX != tmp) texChange = true;
  //---
  tmp = FRENCH_FLAG_TEX;
  FRENCH_FLAG_TEX = static_cast<int>(ini->GetLong(section, "FrenchFlagTex", 0));
  if (FRENCH_FLAG_TEX != tmp) texChange = true;
  //---
  tmp = PIRATE_FLAG_TEX;
  PIRATE_FLAG_TEX = static_cast<int>(ini->GetLong(section, "PirateFlagTex", 0));
...
void FLAG::SetAdd(int flagNum) {
  ...
  for (auto fn = flagNum; fn < flagQuantity; fn++) {
    ...
    if (len < FLAGVECTORLEN) {
      ...
      flist[fn]->vectQuant = static_cast<int>(len / FLAGVECTORLEN); // число сегментов флага
