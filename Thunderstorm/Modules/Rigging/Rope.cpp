      ...
      if (bDraw) {
        for (auto i = 0; i < groupQuantity; i++)
          if (!gdata[i].bDeleted && gdata[i].nt != 0 && nVert != 0)
            ...
            {
              static_cast<SHIP_BASE*>(EntityManager::GetEntityPointer(gdata[i].shipEI))->SetLightAndFog(true);
              ...
              RenderService->DrawBuffer(vBuf, sizeof(ROPEVERTEX), iBuf, 0, nVert, gdata[i].st,
                                        gdata[i].nt);
              static_cast<SHIP_BASE*>(EntityManager::GetEntityPointer(gdata[i].shipEI))->RestoreLightAndFog();
            }
...
void ROPE::AddLabel(GEOS::LABEL& lbl, NODE* nod, bool bDontSage) {
  ...
  if (ropeNum == 0) {
    ...
    core.Trace("Warning! Found rope with number 0: (model = %s) (label = %s)",
  ...
  if (rd->bMatWorld != nullptr && rd->eMatWorld != nullptr) {
    ...
    if (rd->len < .1f || rd->len > 200.f) {
      ...
      if (ftmp >= 0.0001f) {
        ...
        float ca, sa;
        for (int i = 0; i < ROPE_EDGE; i++) {
          ca = cosf(static_cast<float>(i) / static_cast<float>(ROPE_EDGE) * 2.f * PI);
          sa = sinf(static_cast<float>(i) / static_cast<float>(ROPE_EDGE) * 2.f * PI);
          // vert & horz
        ...
        for (int i = 0; i < ROPE_EDGE; i++) {
          // vert & horz
          rd->pos[i].x = ROPE_WIDTH / 2.f * sinf(static_cast<float>(i) / static_cast<float>(ROPE_EDGE) * 2.f * PI);
          rd->pos[i].y = 0.f;
          rd->pos[i].z = ROPE_WIDTH / 2.f * cosf(static_cast<float>(i) / static_cast<float>(ROPE_EDGE) * 2.f * PI);
        }
      }
...
void ROPE::FirstRun() {
  ...
  if (nVert && nIndx) {
    if (ropeQuantity > wRopeLast) {
      ...
      else core.Trace("Can`t create index or vertex buffer (index = %d, vertex = %d)", nIndx, nVert);
    }
...
void ROPE::SetAdd(int firstNum) {
  ...
  for (int rn = firstNum; rn < ropeQuantity; rn++) {
    ...
    while (rlist[rn]->bMatWorld == nullptr || rlist[rn]->eMatWorld == nullptr) {
      ...
      core.Trace("Bad rope data for rope: (model=%s) (rope num = %d) (begin group=%d, end group=%d)",
