  ...
  auto trigger = false;
	...
	else
	{
		...
		else
		{
			...
			if (!sroll)
			{
        const auto delta = (float)Delta_Time * pp->FLEXSPEED;
				if (WindUp)
				{
					SumWind -= delta;
...
void SAILONE::SetGeometry()
{
  ...
  const auto tmpCol = 1.f / (float)(SAIL_COL_MAX - 1);
  const auto tmpRow = 1.f / (float)(SAIL_ROW_MAX - 1);
...
void SAILONE::DoSRollSail(SAILVERTEX* pv)
{
  ...
  const auto dp = (ss.hardPoints[1] - ss.hardPoints[0]) / (float)(m_dwCol - 1);
  ...
  const CVECTOR dv = dirV * (sailHeight * .25f);
	...
	for (int ix = 0; ix < 4; ix++)
	{
		...
		for (int iy = 0; iy < 5; iy++)
		{
			...
			float mul;
			if (iy)
				mul = (pp->FALL_SSAIL_ADD_MIN + (float)rand() / (float)RAND_MAX * pp->FALL_SSAIL_ADD_RAND) /
					sqrtf(~dvec);
			else
				mul = 1.f;
  ...
  const CVECTOR dv = dirV * (sailHeight * .25f);
	...
	for (int ix = 0; ix < 5; ix++)
	{
		...
		for (int iy = 0; iy <= ix; iy++)
		{
			...
			float mul;
			if (iy)
				mul = (pp->FALL_TSAIL_ADD_MIN + (float)rand() / (float)RAND_MAX * pp->FALL_TSAIL_ADD_RAND) /
					sqrtf(~dvec);
			else
				mul = 1.f;
			SailPnt[gidx] += dvec * mul;
