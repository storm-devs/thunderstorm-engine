...
SAILONE::~SAILONE() {
  ...
  if (eid) prb = static_cast<ROPE_BASE*>(EntityManager::GetEntityPointer(eid));
...
void SAILONE::FillIndex(uint16_t* pt) {
  ...
  if (pt) {
    if (bRolling && (ss.eSailType == SAIL_TREANGLE)) {
      ...
      if (m_dwRow == 3) {
        pt[noholeStart] = idx;
        pt[noholeStart + 7] = pt[noholeStart + 3] = pt[noholeStart + 1] = idx + SAIL_ROW_MAX / 2 * (SAIL_ROW_MAX
          / 2 + 1) / 2;
        pt[noholeStart + 9] = pt[noholeStart + 6] = pt[noholeStart + 2] = idx + SAIL_ROW_MAX / 2 * (SAIL_ROW_MAX
          / 2 + 3) / 2;
        pt[noholeStart + 4] = idx + SAIL_ROW_MAX * (SAIL_ROW_MAX - 1) / 2;
        pt[noholeStart + 10] = pt[noholeStart + 8] = pt[noholeStart + 5] = idx + SAIL_ROW_MAX * (SAIL_ROW_MAX -
          1) / 2 + SAIL_ROW_MAX / 2;
      ...
      if (m_dwRow == 3) {
        for (ix = 0; ix < static_cast<int>(m_dwCol - 1); ix++) {
          ...
          pt[noholeStart + 9] = pt[noholeStart + 7] = pt[noholeStart + 4] = idx + SAIL_ROW_MAX * (SAIL_COL_MAX
            / 3) + SAIL_ROW_MAX / 2;
...
bool SAILONE::SetSail() {
  ...
  case SAIL_TRAPECIDAL:
    if (ss.bYesLimitPoint) {
      ss.fDeepZ = GetDistanceFromPointTo3Point(ss.LimitPoint, ss.hardPoints[0], ss.hardPoints[1],
                                               ss.hardPoints[2]);
...
void SAILONE::GoTWave(SAILVERTEX* pv) {
  ...
  auto WindAmplitude = pp->TsailWindDepend * ss.boundSphere.r * (pp->globalWind.base + .3f) / static_cast<float>((
    SAIL_ROW_MAX -
    1)) / static_cast<float>((SAIL_ROW_MAX - 1));
...
void SAILONE::GoVWave(SAILVERTEX* pv) {
  ...
  for (ix = 0; ix < SAIL_COL_MAX;) {
    ...
    ddV = sgeo.ddVv - WindAdd * (2.f - k) / static_cast<float>(SAIL_ROW_MAX) - SailDownVect * 2.f / static_cast<float>(
      SAIL_ROW_MAX);
...
void SAILONE::SetGeometry() {
  ...
  if (ss.eSailType == SAIL_TREANGLE) {
    ...
    pG->dVh = (normLD * ss.fDeepH - p13 * ss.fDeepVh + pG->normR * ss.fDeepVz) * tmpRow * (1.f - ss.holeCount * pp->
      fSHoleFlexDepend);
...
void SAILONE::DoSFreeSail(SAILVERTEX* pv) {
  ...
  for (int ix = 0; ix < 4; ix++) {
    ...
    for (int iy = 0; iy < 5; iy++) {
      ...
      float mul;
      if (iy)
        mul = (pp->FALL_SSAIL_ADD_MIN + static_cast<float>(rand()) / static_cast<float>(RAND_MAX) * pp->
            FALL_SSAIL_ADD_RAND) /
          sqrtf(~dvec);
      else
        mul = 1.f;
...
void SAILONE::DoTFreeSail(SAILVERTEX* pv) {
  ...
  for (int ix = 0; ix < 5; ix++) {
    ...
    for (int iy = 0; iy <= ix; iy++) {
      ...
      float mul;
      if (iy)
        mul = (pp->FALL_TSAIL_ADD_MIN + static_cast<float>(rand()) / static_cast<float>(RAND_MAX) * pp->
            FALL_TSAIL_ADD_RAND) /
          sqrtf(~dvec);
      else
        mul = 1.f;
