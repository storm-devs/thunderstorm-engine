...
void SAIL::Execute(uint32_t Delta_Time) {
  ...
  if (gdata &&
    core.Controls->GetDebugAsyncKeyState(VK_MENU) < 0 &&
    core.Controls->GetDebugAsyncKeyState(VK_CONTROL) < 0 &&
    core.Controls->GetDebugAsyncKeyState(VK_SHIFT) < 0) {
    long nTmpMastNum = -1;
    if (core.Controls->GetDebugAsyncKeyState('1') < 0) nTmpMastNum = 1;
    else if (core.Controls->GetDebugAsyncKeyState('2') < 0) nTmpMastNum = 2;
    else if (core.Controls->GetDebugAsyncKeyState('3') < 0) nTmpMastNum = 3;
    else if (core.Controls->GetDebugAsyncKeyState('4') < 0) nTmpMastNum = 4;
    else if (core.Controls->GetDebugAsyncKeyState('5') < 0) nTmpMastNum = 5;
    else if (core.Controls->GetDebugAsyncKeyState('6') < 0) nTmpMastNum = 6;
    else if (core.Controls->GetDebugAsyncKeyState('7') < 0) nTmpMastNum = 7;
    else if (core.Controls->GetDebugAsyncKeyState('0') < 0) nTmpMastNum = 0;
    if (nTmpMastNum >= 0) {
  ...
  if (bUse) {
    ...
    for (i = 0; i < groupQuantity; i++) {
      ...
      if (gdata[i].bFinalSailDoOld != gdata[i].bFinalSailDo) {
        if (gdata[i].bYesShip) {
          ATTRIBUTES* pA = static_cast<VAI_OBJBASE*>(EntityManager::GetEntityPointer(gdata[i].shipEI))->GetACharacter();
      ...
      if (gdata[i].speed_m < 0.001f) {
        ...
        if (gdata[i].maxHole != 0)
          fSP = static_cast<float>(gdata[i].maxSP) * gdata[i].curHole / gdata[i].maxHole;
        if (fSP > gdata[i].maxSP) fSP = static_cast<float>(gdata[i].maxSP);
      }
      ...
      if (gdata[i].bYesShip) {
        ...
        if (pVai != nullptr && pVai->GetACharacter() != nullptr) {
          ...
          if (pA != nullptr)
            pA->SetAttributeUseDword("SP", static_cast<long>(fSP));
        }
      }
    }
    ...
    if constexpr (false) //Delta_Time==0 )
    {
        ...
        for (idx = 0; idx < gdata[j].sailQuantity; idx++) {
          ...
          if (slist[i]->ss.Nh != 0) {
            RenderService->DrawBuffer(sg.vertBuf, sizeof(SAILVERTEX), sg.indxBuf,
                                      slist[i]->ss.sVert, slist[i]->ss.nVert, slist[i]->ss.shi,
                                      slist[i]->ss.Nh);
          }
          // Draw normal sail
          if (slist[i]->ss.Nn != 0) {
            RenderService->DrawBuffer(sg.vertBuf, sizeof(SAILVERTEX), sg.indxBuf,
                                      slist[i]->ss.sVert, slist[i]->ss.nVert, slist[i]->ss.sni,
                                      slist[i]->ss.Nn);
          }
        }
      ...
      for (j = 0; j < groupQuantity; j++) {
        ...
        for (idx = 0; idx < gdata[j].sailQuantity; idx++) {
          ...
          if (gdata[j].bYesShip) {
            static_cast<SHIP_BASE*>(EntityManager::GetEntityPointer(gdata[j].shipEI))->SetLightAndFog(true);
            static_cast<SHIP_BASE*>(EntityManager::GetEntityPointer(gdata[j].shipEI))->SetLights();
          }
          if (slist[i]->ss.nholeIndx != 0) {
            RenderService->DrawBuffer(sg.vertBuf, sizeof(SAILVERTEX), sg.indxBuf,
                                      slist[i]->ss.sVert, slist[i]->ss.nVert, slist[i]->ss.sholeIndx,
          ...
          if (slist[i]->ss.nnormIndx != 0) {
            RenderService->DrawBuffer(sg.vertBuf, sizeof(SAILVERTEX), sg.indxBuf,
                                      slist[i]->ss.sVert, slist[i]->ss.nVert, slist[i]->ss.sIndx,
                                      slist[i]->ss.nnormIndx);
          }
          if (gdata[j].bYesShip) {
            static_cast<SHIP_BASE*>(EntityManager::GetEntityPointer(gdata[j].shipEI))->UnSetLights();
            static_cast<SHIP_BASE*>(EntityManager::GetEntityPointer(gdata[j].shipEI))->RestoreLightAndFog();
          }
        }
        RenderService->SetRenderState(D3DRS_TEXTUREFACTOR, dwOldTextureFactor);
      }
        ...
        if (so->ss.turningSail && posNum != 0) // установка только для поворачивающихся парусов
          if (tmpEI = EntityManager::GetEntityId("rope"))
            if (so->sailtrope.rrs[0] == nullptr) {
              ...
              static_cast<ROPE_BASE*>(EntityManager::GetEntityPointer(tmpEI))->GetEndPoint(
                &epos, so->sailtrope.rrs[0]->ropenum, gdata[so->HostNum].modelEI);
              ...
              static_cast<ROPE_BASE*>(EntityManager::GetEntityPointer(tmpEI))->GetEndPoint(
                &epos, so->sailtrope.rrs[1]->ropenum, gdata[so->HostNum].modelEI);
...
void SAIL::SetAllSails(int groupNum) {
  ...
  if (gdata[groupNum].sailQuantity > 0) {
    ...
    if (gdata[groupNum].bYesShip && !gdata[groupNum].bDeleted) {
      ATTRIBUTES* pACh = static_cast<VAI_OBJBASE*>(EntityManager::GetEntityPointer(gdata[groupNum].shipEI))->
        GetACharacter();
      ...
      if (slist[sn]->sroll == nullptr && !slist[sn]->bRolling)
        ...
        for (i = 0; i < 2; i++)
          if (slist[sn]->sailtrope.rrs[i]) {
            ...
            CVECTOR endVect;
            static_cast<ROPE_BASE*>(EntityManager::GetEntityPointer(ropeEI))->GetEndPoint(
...
float SAIL::Cannon_Trace(long iBallOwner, const CVECTOR& src, const CVECTOR& dst) {
  ...
  if (retVal < 2.f) {
    if (!slist[traceSail]->bFreeSail && !gdata[slist[traceSail]->HostNum].bDeleted) {
      ...
      auto* pvai = static_cast<VAI_OBJBASE*>(EntityManager::GetEntityPointer(gdata[slist[traceSail]->HostNum].shipEI));
      ATTRIBUTES* pA = nullptr;
...
void SAIL::DoSailToNewHost(entid_t newModelEI, entid_t newHostEI, int grNum, NODE* nod, entid_t oldModelEI) {
  ...
  for (sn = 0; sn < sailQuantity; sn++)
    if (slist[sn]->hostNode == nod &&
      ...
      (grNum == 0 || slist[sn]->groupNum == grNum))
      break;
  ...
  if (m_nMastCreatedCharacter >= 0 && slist[sn] != nullptr)
    core.Event("DoSailHole", "llssllllf", -1, m_nMastCreatedCharacter,
...
int SAIL::GetSailStateForCharacter(int chrIdx) const {
  ...
  case ROLLTYPE_NONE: return 0;
    break;
  case ROLLTYPE_NORMAL: return 1;
    break;
  case ROLLTYPE_FAST: return 2;
    break;
  }
  return 0;
}
...
void SAIL::DoRandomsSailsDmg(int chrIdx, int gn, float fDmg) {
  ...
  for (int _sailsQnt = gdata[gn].sailQuantity; _sailsQnt > 0;) {
    ...
    for (int j = 0; j < 14 && slist[sn]->ss.hole[holeIdx]; j++)
      if (holeIdx < static_cast<int>(slist[sn]->GetMaxHoleCount()) - 1) holeIdx++;
      else holeIdx = 0;
    ...
    VDATA* pvd = core.Event("evntRandomSailDmg", "lslfflll",
                            chrIdx, slist[sn]->hostNode->GetName(), slist[sn]->groupNum,
                            fDmg, static_cast<float>(slist[sn]->maxSpeed) / gdata[gn].speed_m,
