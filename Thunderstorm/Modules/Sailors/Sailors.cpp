...
//#define //UN//GUARD_SAILORS			} catch(...) { core.Trace("ERROR in Sailors.dll : block '%s'", (char*)block_name); throw _EXS(FATAL,"unknown"); }}
...
bool ShipMan::RotateToAngle(uint32_t& dltTime, SailorsPoints& sailorsPoints) {
  ...
  if (fabs(angTo.y - ang.y)) {
    ...
    if (fabs(static_cast<float>(angTo.y - ang.y)) < rotSpeed * dltTime / 100.0f) ang = angTo;
...
bool ShipMan::MoveToPosition(uint32_t& dltTime, SailorsPoints& sailorsPoints, ShipState& shipState) {
  ...
  const auto dNow = static_cast<float>(SQR(pos.x - ptTo.x))
    + SQR(pos.y - ptTo.y)
    + SQR(pos.z - ptTo.z);
  ...
  if (dFuture < dNow) {
    if (RotateToAngle(dltTime, sailorsPoints) || mode == MAN_RUN || (mode == MAN_WALK && fabs(angTo.y - ang.y) < PI
      / 8)) {
...
void ShipMan::NewAction(SailorsPoints& sailorsPoints, ShipState& shipState, uint32_t& dltTime) {
  ...
  if (mode != MAN_CLIMB_UP && path.currentPointPosition == path.length - 1 || mode == MAN_STAY) {
    if (mode == MAN_CANNONRELOAD && (shipState.mode == SHIP_SAIL || shipState.mode == SHIP_STORM || shipState.mode
      == SHIP_WAR)) {
      //Уйти от пушки
      ...
      FindNextPoint(sailorsPoints, shipState);
      if (shipState.mode == SHIP_WAR) mode = MAN_RUN;
      else mode = MAN_WALK;
      return;
    }
    ...
    case SHIP_SAIL:
      ...
      if (mode != MAN_STAY && ran < 0.1f) mode = MAN_RUN;
      else if (ran < 0.6f || mode == MAN_STAY) mode = MAN_WALK;
      else mode = MAN_STAY;
      break;
    case SHIP_WAR:
    case SHIP_STORM:
...
void ShipWalk::OnHullHit(const CVECTOR& v) {
  ...
  for (auto i = 0; i < crewCount; i++)
    if (10 * rand() / RAND_MAX < 3 && shipMan[i].mode != MAN_JUMP && shipMan[i].mode != MAN_SWIM) {
      CVECTOR pos;
      if (ship) pos = shipModel->mtx * shipMan[i].pos;
      else pos = shipMan[i].pos;
      ...
      if (sqrt((pos.x - v.x) * (pos.x - v.x) + (pos.z - v.z) * (pos.z - v.z)) < 1) {
        ...
        shipMan[i].mode = MAN_JUMP;
        shipMan[i].jumpSpeedY = -(JUMP_SPEED_Y_EXPLOSION + rand() * JUMP_SPEED_Y_EXPLOSION / static_cast<float>(RAND_MAX
        ));
  ...
  for (auto m = 0; m < crewCount; m++)
    for (auto i = 0; i < crewCount; i++)
      ...
      if (i != m && Dest(shipMan[m].pos, shipMan[i].pos, 1) &&
        ...
        shipMan[m].path.currentPointPosition >= 0 && shipMan[m].path.currentPointPosition < shipMan[m]
        .path.length &&
        shipMan[i].path.currentPointPosition >= 0 && shipMan[i].path.currentPointPosition < shipMan[i]
        .path.length) {
...
uint64_t Sailors::ProcessMessage(MESSAGE& message) {
  ...
  case AI_MESSAGE_ADD_SHIP:
    ...
    if (!editorMode)
      if (!shipWalk[shipsCount - 1].sailorsPoints.points.count || !shipWalk[shipsCount - 1]
