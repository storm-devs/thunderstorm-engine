...
void Menu::Draw(VDX9RENDER* rs, SailorsPoints& sailorsPoints) const {
	...
	if (selected == 1 && blocked == 1)
	{
		...
		if (sailorsPoints.points.selected >= 0 && sailorsPoints.points.selected < sailorsPoints.points.count)
		{
			...
			{
			case(PT_TYPE_NORMAL): rs->Print(rs->GetCurFont(), D3DCOLOR_XRGB(200, 200, 200), 10, 160, "%s",
			                                "type: PT_TYPE_NORMAL('F9')");
				break;
			...
			case(PT_TYPE_MAST_1): rs->Print(rs->GetCurFont(), D3DCOLOR_XRGB(200, 200, 200), 10, 160, "%s",
			                                "type: PT_TYPE_MAST_1('F9')");
				break;

			case(PT_TYPE_MAST_2): rs->Print(rs->GetCurFont(), D3DCOLOR_XRGB(200, 200, 200), 10, 160, "%s",
			                                "type: PT_TYPE_MAST_2('F9')");
				break;

			case(PT_TYPE_MAST_3): rs->Print(rs->GetCurFont(), D3DCOLOR_XRGB(200, 200, 200), 10, 160, "%s",
			                                "type: PT_TYPE_MAST_3('F9')");
				break;

			case(PT_TYPE_MAST_4): rs->Print(rs->GetCurFont(), D3DCOLOR_XRGB(200, 200, 200), 10, 160, "%s",
			                                "type: PT_TYPE_MAST_4('F9')");
				break;

			case(PT_TYPE_MAST_5): rs->Print(rs->GetCurFont(), D3DCOLOR_XRGB(200, 200, 200), 10, 160, "%s",
			                                "type: PT_TYPE_MAST_5('F9')");
				break;

			case(PT_TYPE_NOT_TARGETPOINT): rs->Print(rs->GetCurFont(), D3DCOLOR_XRGB(200, 200, 200), 10, 160, "%s",
		...
		if (sailrs->shipWalk[0].sailorsPoints.points.count && sailrs->shipWalk[0].sailorsPoints.points.count >
...
void Menu::ChangeControl2(int key, SailorsPoints& sailorsPoints) //Change points
{
	...
	if (sailorsPoints.points.selected >= 0 && sailorsPoints.points.selected < sailorsPoints.points.count)
	{
		...
		case VK_LEFT:
			sailorsPoints.points.point[sailorsPoints.points.selected].x += 0.025f * sin(cameraAng.y + PI / 2) * dltTime
				/ 10.0f;
			sailorsPoints.points.point[sailorsPoints.points.selected].z += 0.025f * cos(cameraAng.y + PI / 2) * dltTime
				/ 10.0f;
			break;
...
void Menu::OnKeyPress(SailorsPoints& sailorsPoints)
{
	...
	if (GetAsyncKeyState(VK_UP) < 0) key = VK_UP;
	else if (GetAsyncKeyState(VK_DOWN) < 0) key = VK_DOWN;
	else if (GetAsyncKeyState(VK_LEFT) < 0) key = VK_LEFT;
	else if (GetAsyncKeyState(VK_RIGHT) < 0) key = VK_RIGHT;
	else if (GetAsyncKeyState(VK_RETURN) < 0) key = VK_RETURN;
	else if (GetAsyncKeyState(VK_INSERT) < 0) key = VK_INSERT;
	else if (GetAsyncKeyState(VK_DELETE) < 0) key = VK_DELETE;
	else if (GetAsyncKeyState(VK_SPACE) < 0) key = VK_SPACE;
	else if (GetAsyncKeyState(VK_F1) < 0) key = VK_F1;
	else if (GetAsyncKeyState(VK_F2) < 0) key = VK_F2;
	else if (GetAsyncKeyState(VK_F3) < 0) key = VK_F3;
	else if (GetAsyncKeyState(VK_F5) < 0) key = VK_F5;
	else if (GetAsyncKeyState(VK_F6) < 0) key = VK_F6;
	else if (GetAsyncKeyState(VK_F9) < 0) key = VK_F9;
	else if (GetAsyncKeyState(VK_F4) < 0) key = VK_F4;
	else if (GetAsyncKeyState(VK_F10) < 0) key = VK_F10;
	else
