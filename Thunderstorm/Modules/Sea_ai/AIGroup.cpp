		...
		if (isMainGroup())
		{
			for (uint32_t i = 0; i < aGroupShips.size(); i++)
				if (!aGroupShips[i]->isMainCharacter())
					aGroupShips[i]->GetTaskController()->SetNewTask(
...
{
	for (uint32_t i = 0; i < AIGroups.size(); i++) if (AIGroups[i]->GetName() == pGroupName) return AIGroups[i];
	return nullptr;
}

AIGroup* AIGroup::FindMainGroup()
{
	for (uint32_t i = 0; i < AIGroups.size(); i++) if (AIGroups[i]->isMainGroup()) return AIGroups[i];
	return nullptr;
}
