...
public:
  ...
  float GetMaxFireDistance() override { return GetCannonController()->GetFireDistance(true); };
  float GetMinFireDistance() override { return GetCannonController()->GetFireDistance(false); };

  // entid_t functions
  entid_t GetShipEID() const { return eidShip; };
  Entity* GetShipPointer() const { return EntityManager::GetEntityPointer(GetShipEID()); };
  SHIP_BASE* GetShipBasePointer() const { return static_cast<SHIP_BASE*>(GetShipPointer()); };
  VAI_OBJBASE* GetAIObjShipPointer() const { return static_cast<VAI_OBJBASE*>(GetShipPointer()); };
  ...
  CVECTOR GetBoxsize() const override { return GetAIObjShipPointer()->GetBoxsize(); };
