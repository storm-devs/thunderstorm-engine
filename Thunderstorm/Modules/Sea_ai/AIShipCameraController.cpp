	...
	else
	{
		...
		if (GetAIShip()->GetCannonController()->isCanFire(vZ))
		{
			...
			for (i = 0; i < AIShip::AIShips.size(); i++)
				...
				{
					CVECTOR vFakeShipPos = AIShip::AIShips[i]->GetPos() + (!(AIShip::AIShips[i]->GetPos() - vOurPos)) *
						AIShip::AIShips[i]->GetBoxsize().z * 0.8f;
					//AIHelper::pRS->DrawSphere(vFakeShipPos, 2.0f, 0xFF00FF00);
					float fDistance = (vFakeShipPos - vFirePos).GetLength2D();
					...
					if (fDistance <= 0.8f * AIShip::AIShips[i]->GetBoxsize().z)
					{
						...
						dwTarget = Helper.GetRelation(GetAIShip()->GetACharacter(),
						                              AIShip::AIShips[i]->GetACharacter());
					}
				}
