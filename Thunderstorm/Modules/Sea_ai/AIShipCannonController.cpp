    ...
    if (aShipBorts[i].aCannons.size()) {
      ...
      // if (GetAIShip()->isMainCharacter())
      {
        ...
        pACurBort->SetAttributeUseFloat("DamageRatio",
                                        1.0f - static_cast<float>(GetBortIntactCannonsNum(i)) / static_cast<float>(pBort
                                          ->aCannons.size()));
      }
    }
}
...
uint32_t AIShipCannonController::GetFirstFireBort(const CVECTOR& vFirePos, float* pfZapasDistance) {
  for (uint32_t i = 0; i < aShipBorts.size(); i++)
    if (aShipBorts[i].aCannons.size() &&
      isCanFireBort(i, vFirePos, pfZapasDistance) && !aShipBorts[i].isBortDamaged())
      return i;
  ...
  if (api->Controls->GetDebugAsyncKeyState('H') < 0) {
    ...
    for (uint32_t i = 0; i < aShipBorts.size(); i++)
      if (aShipBorts[i].aCannons.size()) {
        ...
        if (Verts.size())
          AIHelper::pRS->DrawPrimitiveUP(D3DPT_TRIANGLELIST,TR_FORMAT, Verts.size() / 3,
...
uint32_t AIShipCannonController::GetBortIntactCannonsNum(uint32_t dwBortIdx) {
  ...
  for (uint32_t j = 0; j < aShipBorts[dwBortIdx].aCannons.size(); j++)
    if (!aShipBorts[dwBortIdx].aCannons[j].
      isDamaged())
      dwCannons++;
  return dwCannons;
}
...
bool AIShipCannonController::isHaveEnoughtBallsForBort(uint32_t dwBortIdx) {
  ...
  Assert(pvData);
  if (static_cast<long>(GetBortIntactCannonsNum(dwBortIdx)) > pvData->GetLong()) return false;
  return true;
}
