...
//--------------------------------------------------------------------
void TCarcass::InitCircleMeasure(float _d,
                                 float _kx,
                                 float _ky) {
  float t, tStep, x, y;
...
void TCarcass::RebuildLevels(tCarcassVertex* _vBuffer, bool _firstDraw, uint32_t _dTime) {
  ...
  for (auto level = 0; level < (levelsCount); level++) {
    ...
    for (auto measurePoint = 0; measurePoint < measure.pointsCount; measurePoint++) {
      ...
      //tempVertex->pos = sceneMatrix * (levelStarts[level] + CVECTOR((1.f + WIDEN_K*levelK)*measure.deltaPointX[measurePoint],
      //															  heightK*measure.deltaPointY[measurePoint], 0.f));
      tempVertex->pos = (levelStarts[level] + CVECTOR(
        (1.f + WIDEN_K * levelK) * measure.deltaPointX[measurePoint],
        heightK * measure.deltaPointY[measurePoint], 0.f));
      ...
      if (_firstDraw) {
        ...
        tempVertex->v = 1.f - static_cast<float>(measurePoint) / (measure.pointsCount - 1) + tempVertex->u *
          V_DIAGONAL_K;
      }
      else {
        tempVertex->color = (static_cast<uint32_t>(speedA * sqrtf(levelA) * (1.f - measureK) * 0xFF) << 24) |
          0x00FFFFFF;
