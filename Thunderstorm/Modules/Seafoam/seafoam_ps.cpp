...
// node ----------------------------------------------------------------------------
SEAFOAM_PS* SEAFOAM_PS::GetLeftNode() { return l_PTR; }
SEAFOAM_PS* SEAFOAM_PS::GetRightNode() { return r_PTR; }
void SEAFOAM_PS::SetLeftNode(SEAFOAM_PS* node) { l_PTR = node; }
...
void SEAFOAM_PS::UpdateVertexBuffer()
{
	...
	RenderService->VBLock(VBuffer, 0, sizeof(PARTICLE_VERTEX) * VERTEXS_ON_PARTICLE * ParticlesNum, (uint8_t**)&pVertex,
	                      0);
	for (n = 0; n < ParticlesNum; n++)
	{
...
void SEAFOAM_PS::LayOnSurface(uint32_t index)
{
	...
	CVECTOR from, to;
	float dist;
	pLink = (COLLISION_OBJECT *)EntityManager::GetEntityPointer(SurfaceID);
