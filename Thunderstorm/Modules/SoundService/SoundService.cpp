...
const char* SoundService::GetRandomName(tAlias* _alias) const {
  ...
  for (;;) {
    ...
    if ((currentNameIndex + 1) >= static_cast<int>(_alias->SoundFiles.size())) break;
...
TSD_ID SoundService::SoundPlay(const char* _name,
  ...
  if (_type == PCM_3D) {
    ...
    CHECKFMODERR(
      PlayingSounds[SoundIdx].channel->set3DMinMaxDistance(_minDistance * DISTANCEFACTOR, _maxDistance *
        DISTANCEFACTOR));
...
bool SoundService::AddSoundSchemeChannel(char* in_string, bool _looped /*= false*/) {
  ...
  if (!col) {
    ...
    NewChannel.timeToNextPlay = static_cast<long>(rand(
      static_cast<float>(NewChannel.maxDelayTime - NewChannel.minDelayTime)));
