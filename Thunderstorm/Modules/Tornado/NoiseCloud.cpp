...
void NoiseCloud::Update(float dltTime) {
  for (long i = 0; i < sizeof(rect) / sizeof(Rect); i++) {
    ...
    if (rect[i].r > TRND_CLDRADIUS * 0.6f)
      rect[i].alpha = 1.0f - (rect[i].r - TRND_CLDRADIUS * 0.6f) / (
        TRND_CLDRADIUS * 0.4f);
...
void NoiseCloud::Draw(VDX9RENDER* rs) {
  ...
  for (long i = 0; i < sizeof(rect) / sizeof(Rect); i++) {
    ...
    const auto color = (static_cast<long>(clr * 1.0f) << 16) | (static_cast<long>(clr * 0.95f) << 8) | static_cast<long>
      (clr * 0.9f) | (static_cast<long>(rect[i].alpha * galpha * 0.4f) << 24);
