...
void TOUCH::Execute(uint32_t dwCoreDeltaTime)
{
	...
	{
		pShips[i]->pShip = (SHIP_BASE*)EntityManager::GetEntityPointer(pShips[i]->eID);
	...
	for (i = 0; i < iNumShips; i++)
      ...
      auto yscale = fScale;
        ...
        auto nextj = (j == pShips[i]->iNumVContour - 1) ? 0 : j + 1;
				...
				float fRes;
				//dwColor = 0xFFFFFF;
				if (pIslandBase && pIslandBase->GetDepth(pShips[i]->vContourTemp[0][j].x,
				                                         pShips[i]->vContourTemp[0][j].z, &fRes))
				{
					if (fRes == 0.0f)
						dwColor = 0xFFFF00;
				}
