...
WdmIslands::WdmIslands() {
  ...
  baseModel = static_cast<WdmRenderModel*>(wdmObjects->wm->CreateModel(new WdmRenderModel(), "islands\\islands", false,
                                                                       false,
                                                                       false, 1));
  ...
  for (long i = 0; i < ginfo.nlabels; i++) {
    ...
    name += label.name;
    //Загружаем
    auto* model = static_cast<WdmRenderModel*>(wdmObjects->wm->CreateModel(new WdmRenderModel(), name.c_str(), false,
                                                                           false, true, 2));
    if (model) {
      //Общее
      ...
      name += "_area";
      isl.area = static_cast<WdmRenderModel*>(wdmObjects->wm->CreateModel(
      ...
      name += label.name;
      name += "_palms";
      isl.palms = static_cast<WdmRenderModel*>(wdmObjects->wm->CreateModel(
        new WdmRenderModel(), name.c_str(), false, false,
        true, 4, 800));
      if (isl.palms) {
      ...
      name += label.name;
      name += "_waves";
      isl.waves = static_cast<WdmIslandWaves*>(wdmObjects->wm->CreateModel(
        new WdmIslandWaves(), name.c_str(), false, false,
        true, 4));
    }
    else {
...
void WdmIslands::SetIslandsData(ATTRIBUTES* apnt, bool isChange) {
  ...
  for (uint32_t i = 0; i < numAttr; i++) {
    ...
    //Размеры строки
    const float textWidth = static_cast<float>(wdmObjects->rs->StringWidth(
      (char*)labels[index].text.c_str(), labels[index].font));
