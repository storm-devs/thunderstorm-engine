...
WdmIslands::WdmIslands()
{
	...
	baseModel = (WdmRenderModel *)wdmObjects->wm->CreateModel(new WdmRenderModel(), "islands\\islands", false, false,
	                                                          false, 1);
  ...
  auto isMin = false, isMax = false;
    ...
    auto* model = (WdmRenderModel *)wdmObjects->wm->CreateModel(new WdmRenderModel(), name.c_str(), false,
      ...
      auto& isl = islands.back();
			...
			name += "_area";
			isl.area = (WdmRenderModel *)wdmObjects->wm->CreateModel(new WdmRenderModel(), name.c_str(), false, false,
			                                                         false, 3);
			if (!isl.area)
			{
			...
			name += label.name;
			name += "_palms";
			isl.palms = (WdmRenderModel *)wdmObjects->wm->CreateModel(new WdmRenderModel(), name.c_str(), false, false,
			                                                          true, 4, 800);
			if (isl.palms)
			{
			...
			name += label.name;
			name += "_waves";
			isl.waves = (WdmIslandWaves *)wdmObjects->wm->CreateModel(new WdmIslandWaves(), name.c_str(), false, false,
			                                                          true, 4);
		}
		else
		{
...
bool WdmIslands::CollisionTest(CMatrix& objMtx, float length, float width, bool heighTest)
{
  ...
  const auto boxRadius = sqrtf(length * length + width * width);
    ...
    const float maxDist = boxRadius + island.model->radius;
		...
		p[0].d = (p[0].nrm.x * length + locMtx.Pos().x) * p[0].nrm.x + (p[0].nrm.z * length + locMtx.Pos().z) * p[0]
		                                                                                                        .nrm.z;
		p[1].d = (p[1].nrm.x * length + locMtx.Pos().x) * p[1].nrm.x + (p[1].nrm.z * length + locMtx.Pos().z) * p[1]
		                                                                                                        .nrm.z;
		p[2].d = (p[2].nrm.x * width + locMtx.Pos().x) * p[2].nrm.x + (p[2].nrm.z * width + locMtx.Pos().z) * p[2]
		                                                                                                      .nrm.z;
		p[3].d = (p[3].nrm.x * width + locMtx.Pos().x) * p[3].nrm.x + (p[3].nrm.z * width + locMtx.Pos().z) * p[3]
		                                                                                                      .nrm.z;
...
void WdmIslands::SetIslandsData(ATTRIBUTES* apnt, bool isChange)
{
  ...
  const uint32_t numAttr = apnt->GetAttributesNum();
    ...
    const long icon = (long)a->GetAttributeAsDword("icon", (uint32_t)-1);
    ...
    const uint32_t hash = TOREMOVE::HashNoCase(id);
      ...
      const long iEntry = hash & (sizeof(labelsEntry) / sizeof(labelsEntry[0]) - 1);
		...
		//Размеры строки
    const float textWidth = (float)wdmObjects->rs->StringWidth((char *)labels[index].text.c_str(), labels[index].font);
		...
		for (long i = 0; i < islands.size(); i++)
		{
			if (IsShipInArea(i, pos))
			{
				if (wdmObjects->wm->AttributesPointer)
				{
					...
					ATTRIBUTES* a = wdmObjects->wm->AttributesPointer->FindAClass(
						wdmObjects->wm->AttributesPointer, "island");
					if (a)
					{
...
void WdmIslands::LRender(VDX9RENDER* rs)
{
	...
	rs->GetViewport(&vp);
  const auto w = (float)vp.Width;
  const auto h = (float)vp.Height;
  ...
  const float dAlpha = api->GetDeltaTime() * (0.001f * 1.5f * 255.0f);
	...
	for (long i = 0; i < labelSort.size(); i++)
	{
		...
		//Пишем текст
    const uint32_t color = (long(label.alpha) << 24) | 0xffffff;
		rs->Print(label.font, color, long(label.l + label.textX), long(label.t + label.textY),
...
void WdmIslands::GetNearPointToArea(const char* islandName, float& x, float& z)
{
	...
	islands[i].area->geo->Clip(nullptr, 0, v,
	                           wdmObjects->worldSizeX * wdmObjects->worldSizeX + wdmObjects->worldSizeZ * wdmObjects->
	                           worldSizeZ, FindNearPoint);
	if (checkMode)
	{
		x = centPos.x;
		z = centPos.z;
