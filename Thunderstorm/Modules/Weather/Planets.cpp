...
void Astronomy::PLANETS::Init(ATTRIBUTES* pAP) {
  ...
  for (uint32_t i = 0; i < aPlanets.size(); i++) {
    ...
    aPlanets[i].fScale = static_cast<float>((aPlanets[i].fRealDistance * aPlanets[i].fDiameter) / (fabs(
        static_cast<double>(aPlanets[i].fDistance) - 1.0) *
      150000000.0));
  }
  ...
  if (fFadeTimeStart >= 0.f) {
    if ((fFadeTime > 0.f && fPlanetFade < 1.f) || (fFadeTime < 0.f && fPlanetFade > 0.f)) {
      if (const auto eid = EntityManager::GetEntityId("weather")) {
        auto fTime = static_cast<WEATHER_BASE*>(EntityManager::GetEntityPointer(eid))->GetFloat(whf_time_counter);
