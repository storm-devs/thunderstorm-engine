...
void RAIN::GenerateRain() {
  ...
  iIBSeaDrops = rs->CreateIndexBuffer(NUM_SEA_DROPS * 2 * 3 * sizeof(uint16_t),
...
void RAIN::RealizeDrops(uint32_t Delta_Time) {
  ...
  fDropsDeltaTime -= static_cast<float>(static_cast<double>(iNumNewDrops1 + iNumNewDrops2) / static_cast<double>(
    dwDropsNearNum +
    dwDropsFarNum));
...
void RAIN::Realize(uint32_t Delta_Time) {
  ...
  if (iVertexBuffer >= 0) {
    ...
    if (bDraw)
      for (i = 0; i < dwNumRainBlocks; i++) {
        auto dwAlpha = static_cast<uint32_t>(255.0f * static_cast<float>(pRainBlocks[i].dwTime) / static_cast<float>(
          dwRainTimeBlend));
        ...
        pRainBlocks[i].vPos.x += (fRainSpeed * fRainWindSpeed + f) * static_cast<float>(Delta_Time) / 60.0f * sinf(
          fWindAngle);
        pRainBlocks[i].vPos.z += (fRainSpeed * fRainWindSpeed + f) * static_cast<float>(Delta_Time) / 60.0f * cosf(
          fWindAngle);
