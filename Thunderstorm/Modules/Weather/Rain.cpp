	...
	if (iVertexBuffer >= 0)
	{
		...
		if (bDraw)
			for (i = 0; i < dwNumRainBlocks; i++)
			{
				...
				pRainBlocks[i].vPos.x += (fRainSpeed * fRainWindSpeed + f) * float(Delta_Time) / 60.0f * sinf(
					fWindAngle);
				pRainBlocks[i].vPos.z += (fRainSpeed * fRainWindSpeed + f) * float(Delta_Time) / 60.0f * cosf(
					fWindAngle);
