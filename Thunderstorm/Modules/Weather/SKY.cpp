...
void SKY::UpdateTimeFactor() {
  ...
  if (static_cast<long>(fTimeFactor) >= static_cast<long>(aSkyDirArray.size())) fTimeFactor -= aSkyDirArray.size();
...
float SKY::CalculateAlphaForSun(const CVECTOR& vSunPos, float fSunSize) {
  ...
  if (fSkySize > 0.f) {
    ...
    if (nTexNum != -1) {
      const auto dwCol1 = GetPixelColor(static_cast<IDirect3DTexture9*>(pRS->GetTextureFromID(TexturesID[nTexNum])), fu,
                                        fv);
      const auto dwCol2 = GetPixelColor(static_cast<IDirect3DTexture9*>(pRS->GetTextureFromID(TexturesNextID[nTexNum])),
                                        fu, fv);
