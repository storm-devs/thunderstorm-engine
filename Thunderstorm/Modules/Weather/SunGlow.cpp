...
void SUNGLOW::SetDevice() {
  ...
  if (ent = EntityManager::GetEntityId("sky")) pSky = static_cast<SKY*>(EntityManager::GetEntityPointer(ent));
  else pSky = nullptr;
...
void SUNGLOW::GenerateSunGlow() {
  ...
  if (Flares.sTexture.size()) iFlareTex = pRS->TextureCreate(static_cast<const char*>(Flares.sTexture.c_str()));
  if (Glow.sSunTexture.size()) iSunTex = pRS->TextureCreate(static_cast<const char*>(Glow.sSunTexture.c_str()));
  if (Glow.sMoonTexture.size()) iMoonTex = pRS->TextureCreate(static_cast<const char*>(Glow.sMoonTexture.c_str()));
  if (Glow.sGlowTexture.size()) iSunGlowTex = pRS->TextureCreate(static_cast<const char*>(Glow.sGlowTexture.c_str()));
  if (Overflow.sTexture.size()) iOverflowTex = pRS->TextureCreate(static_cast<const char*>(Overflow.sTexture.c_str()));
...
void SUNGLOW::Realize(uint32_t Delta_Time) {
  ...
  if (Delta_Time && Flares.aFlares.size() && bHaveFlare) {
    ...
    for (uint32_t i = 0; i < Flares.aFlares.size(); i++) {
      ...
      auto r = static_cast<uint32_t>(fFadeout * fAlpha * fAlphaFlare * static_cast<float>((pF->dwColor & 0xFF0000) >>
        16L));
