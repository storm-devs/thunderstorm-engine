...
bool HELPCHOOSER::RunChooser(char* ChooserGroup)
{
	...
	// подсчет числа прямоугольников для выбора помощи
	m_nRectQ = 0;
	...
	else
	{
		...
		if (pv != nullptr)
		{
			...
			pv[0].pos.x = 0.f;
			pv[0].pos.y = 0.f;
			pv[2].pos.x = (float)dscrSurface.Width;
			pv[2].pos.y = 0.f;
			pv[4].pos.x = (float)dscrSurface.Width;
			pv[4].pos.y = (float)dscrSurface.Height;
			pv[6].pos.x = 0.f;
			pv[6].pos.y = (float)dscrSurface.Height;
			pv[8].pos.x = 0.f;
			pv[8].pos.y = 0.f;
...
void HELPCHOOSER::SetRectangle(long newRectNum)
{
	...
	pv[1].pos.x = pv[7].pos.x = pv[9].pos.x = pv[10].pos.x = pv[11].pos.x = m_pRectList[newRectNum].left *
		m_fScreenWidth;
	pv[3].pos.x = pv[5].pos.x = pv[12].pos.x = pv[13].pos.x = m_pRectList[newRectNum].right * m_fScreenWidth;
	pv[1].pos.y = pv[3].pos.y = pv[9].pos.y = pv[10].pos.y = pv[12].pos.y = m_pRectList[newRectNum].top *
		m_fScreenHeight;
