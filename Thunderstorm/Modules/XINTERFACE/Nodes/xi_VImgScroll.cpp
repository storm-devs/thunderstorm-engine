			...
			while (pScroll != nullptr)
			{
				...
				else
				{
					if (m_idBadPic[n] != -1 && m_idBadTexture[n] != -1)
						// частичное использование текстуры для "плохой" картинки
					{
					...
					while (pScroll != nullptr)
					{
						if (pScroll->pCenter.y + m_pStrParam[l].m_nStrY > m_rect.top && !pScroll->bCurNotUse)
						{
							if (m_Image[pScroll->imageNum].strNum[l] != -1)
								ptrOwner->PrintIntoWindow(m_rect.left - m_leftTextLimit,
								                          m_rect.right + m_rightTextLimit,
								                          m_pStrParam[l].m_nFont, m_pStrParam[l].m_dwForeColor,
								                          m_pStrParam[l].m_dwBackColor,
								                          m_pStrParam[l].m_nAlign, true, m_pStrParam[l].m_fScale,
								                          m_screenSize.x, m_screenSize.y,
								                          long(pScroll->pCenter.x + m_pStrParam[l].m_nStrX),
								                          long(pScroll->pCenter.y + m_pStrParam[l].m_nStrY),
								                          pStringService->GetString(
									                          m_Image[pScroll->imageNum].strNum[l]),
								                          (int)(m_ImageSize.x * pScroll->fCurScale),
								                          (int)(-24 * m_pStrParam[l].m_fScale));
				...
				for (n = 0; n < m_nSlotsQnt; n++)
				{
					...
					if (m_Image[i].tex[n] != -1)
						m_Image[i].img[n] = pPictureService->GetImageNum(
							m_sGroupName[m_Image[i].tex[n]], pListEntity->GetAttribute(param));
...
void CXI_VIMAGESCROLL::ChangeScroll(int nScrollItemNum)
{
	...
	if (pAttr != nullptr)
	{
		...
		if (nScrollItemNum == -1 || m_nListSize != long(
			pAttr->GetAttributeAsDword("NotUsed", 0) + pAttr->GetAttributeAsDword("ListSize", 0)))
		{
			RefreshScroll();
			return;
		}
		...
		for (i = nScrollItemNum; i < nScrollLastNum; i++)
		{
			...
			if (pAttribute != nullptr)
			{
				...
				for (n = 0; n < m_nSlotsQnt; n++)
				{
					...
					if (m_Image[i].tex[n] != -1)
						m_Image[i].img[n] = pPictureService->GetImageNum(
							m_sGroupName[m_Image[i].tex[n]], pAttribute->GetAttribute(param));
					else m_Image[i].img[n] = -1;
					sprintf_s(param, "spec%d", n + 1);
