...
void CXI_CHECKBUTTONS::Draw(bool bSelected, uint32_t Delta_Time) {
  ...
  for (long n = 0; n < m_aButton.size(); n++) {
    ...
    if (m_aButton[n]->pImg) {
      m_aButton[n]->pImg->SetDifferentPosition(static_cast<long>(fX + m_fpIconOffset.x),
                                               static_cast<long>(fY + m_fpIconOffset.y),
    ...
    for (long i = 0; i < m_aButton[n]->aStr.size(); i++) {
      m_rs->ExtPrint(m_nFontNum, dwColor, 0,PR_ALIGN_LEFT, true, m_fFontScale, m_screenSize.x, m_screenSize.y,
                     ...
                     static_cast<long>(fY + m_frTextOffset.top),
...
void CXI_CHECKBUTTONS::LoadIni(INIFILE* ini1, const char* name1, INIFILE* ini2, const char* name2) {
  ...
  m_fTextLineHeight = GetIniFloat(ini1, name1, ini2, name2, "lineheight",
                                  static_cast<float>(m_rs->CharHeight(m_nFontNum)));
...
void CXI_CHECKBUTTONS::UpdateTextInfo(long nButtonNum) {
  ...
  for (long n = 0; n < asOutStr.size(); n++) {
    ...
    case PR_ALIGN_LEFT: m_aButton[nButtonNum]->aStr[n].fX = 0.f;
      break;
    case PR_ALIGN_CENTER: m_aButton[nButtonNum]->aStr[n].fX = static_cast<float>((nWidth - nOffset) / 2);
      break;
    case PR_ALIGN_RIGHT: m_aButton[nButtonNum]->aStr[n].fX = static_cast<float>(nWidth - nOffset);
      break;
    }
  }
}
