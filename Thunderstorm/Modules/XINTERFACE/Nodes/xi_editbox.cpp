...
void CXI_EDITBOX::Draw(bool bSelected, uint32_t Delta_Time) {
  ...
  // show back rectangle
  if (m_idVBRect >= 0)
    m_rs->DrawPrimitive(D3DPT_TRIANGLESTRIP, m_idVBRect, sizeof(XI_NOTEX_VERTEX), 0, 2,
                        "iRectangle");
  // show background of the edit string
  if (m_idVBRect >= 0)
    m_rs->DrawPrimitive(D3DPT_TRIANGLESTRIP, m_idVBRect, sizeof(XI_NOTEX_VERTEX), 4, 2,
                        "iRectangle");
  ...
  if (tmpstr)
    m_rs->ExtPrint(m_nStrFontNum, 0xFFFFFFFF, 0,PR_ALIGN_CENTER, true, m_fStrScale, m_screenSize.x,
...
void CXI_EDITBOX::ChangePosition(XYRECT& rNewPos) {
  ...
  m_nTopOffset = static_cast<long>(pv[5].pos.y = pv[7].pos.y = static_cast<float>(m_rect.top) + m_nTopOffset + m_rs->
    CharHeight(m_nStrFontNum) * 1.06f) + 4;
