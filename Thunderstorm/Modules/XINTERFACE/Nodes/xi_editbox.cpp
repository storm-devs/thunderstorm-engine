...
void CXI_EDITBOX::Draw(bool bSelected, uint32_t Delta_Time)
{
	...
	// show back rectangle
	if (m_idVBRect >= 0) m_rs->DrawPrimitive(D3DPT_TRIANGLESTRIP, m_idVBRect, sizeof(XI_NOTEX_VERTEX), 0, 2,
	                                         "iRectangle");
	// show background of the edit string
	if (m_idVBRect >= 0) m_rs->DrawPrimitive(D3DPT_TRIANGLESTRIP, m_idVBRect, sizeof(XI_NOTEX_VERTEX), 4, 2,
	                                         "iRectangle");
...
void CXI_EDITBOX::ChangePosition(XYRECT& rNewPos)
{
	...
	m_nTopOffset = long(
		pv[5].pos.y = pv[7].pos.y = (float)m_rect.top + m_nTopOffset + m_rs->CharHeight(m_nStrFontNum) * 1.06f) + 4;
