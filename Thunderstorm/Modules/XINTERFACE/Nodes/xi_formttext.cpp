...
void CXI_FORMATEDTEXT::ChangePosition(XYRECT& rNewPos) {
  ...
  if (m_nAlignment == PR_ALIGN_CENTER) m_nPrintLeftOffset = m_leftOffset + static_cast<long>((m_rect.left + m_rect.right
    - m_leftOffset) * .5f);
...
bool CXI_FORMATEDTEXT::GetLineNext(int fontNum, char* & pInStr, char* buf, int bufSize) const {
  ...
  while (pInStr[0]) {
    if (static_cast<unsigned>(*pInStr) <= ' ') pInStr++;
    else break;
  }
...
void CXI_FORMATEDTEXT::SetVertexToNewGroup(bool bUpDirect, long upIdx, long downIdx) {
  ...
  if (pv != nullptr) {
    ...
    if (m_bBackRectangle) {
      pv[12].pos.y = pv[14].pos.y = static_cast<float>(m_rect.top - m_nUpRectOffset - m_rBorderOffset.top + m_vertOffset
        *
        upIdx);
...
void CXI_FORMATEDTEXT::ControlSyncronouseNodes() {
  for (long n = 0; n < static_cast<long>(m_asSyncNodes.size()); n++) {
    CINODE* pNode = static_cast<XINTERFACE*>(EntityManager::GetEntityPointer(g_idInterface))->FindNode(
