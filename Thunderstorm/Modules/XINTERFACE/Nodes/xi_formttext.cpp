	...
	for (auto sd = m_listCur; sd != nullptr && i < m_allStrings; sd = sd->next, i++)
	{
        ...
        auto nX = m_nPrintLeftOffset;
          ...
          auto nCurWidth = m_rs->StringWidth((char*)sd->m_tags[n].str.c_str(), m_idFont, m_fFontScale);
				...
				for (long n = 0; n < sd->m_tags.size(); n++)
				{
					m_rs->ExtPrint(m_idFont, sd->m_tags[n].dwColor, 0,PR_ALIGN_LEFT, true, m_fFontScale, m_screenSize.x,
					               m_screenSize.y, nX, curY,
...
bool CXI_FORMATEDTEXT::Init(INIFILE* ini1, const char * name1, INIFILE* ini2, const char * name2, VDX9RENDER* rs, XYRECT& hostRect,
                            XYPOINT& ScreenSize)
{
    ...
    auto pOldCur = m_listCur;
      ...
      auto fpntMouse = ptrOwner->GetMousePoint();
		...
		if (!m_bSelectableCursor)
		{
			...
			switch (wActCode)
			{
			case ACTION_UPSTEP: SetCurLine(m_listCur->prev);
				ControlSyncronouseNodes();
				break;
			case ACTION_DOWNSTEP: SetCurLine(m_listCur->next);
				ControlSyncronouseNodes();
				break;
			case ACTION_SPEEDUP:
				{
		...
		else
		{
			...
			if (m_nStringGroupQuantity > 1)
				api->Event("SetScrollerPos", "sf", m_nodeName,
				           (float)m_nCurGroupNum / (float)(m_nStringGroupQuantity - 1));
			else
...
bool CXI_FORMATEDTEXT::IsClick(int buttonID, long xPos, long yPos)
{
	...
	if (!retVal && m_bDownEnable && m_idDownEnableTexture)
		if (xPos >= m_frDownPos.left && xPos <= m_frDownPos.right && yPos >= m_frDownPos.top && yPos <= m_frDownPos.
			bottom) retVal = true;
	...
	for (int i = 0; i < nNum && pdescr != nullptr; i++)
		pdescr = pdescr->next;
	if (pdescr != nullptr && m_nCurGroupNum != pdescr->strGroup) SetVertexToNewGroup(
		true, FindUpGroup(pdescr->strGroup), FindDownGroup(pdescr->strGroup));
...
void CXI_FORMATEDTEXT::ChangePosition(XYRECT& rNewPos)
{
	...
	if (m_nAlignment == PR_ALIGN_CENTER) m_nPrintLeftOffset = m_leftOffset + long(
		(m_rect.left + m_rect.right - m_leftOffset) * .5f);
...
{
	if (m_bSelectableCursor)
		SetVertexToNewGroup(true, FindUpGroup(long(m_nStringGroupQuantity * fPos)),
		                    FindDownGroup(long(m_nStringGroupQuantity * fPos)));
	else
	{
		m_listCur = m_listRoot;
...
void CXI_FORMATEDTEXT::SetVertexToNewGroup(bool bUpDirect, long upIdx, long downIdx)
{
	...
	if (pv != nullptr)
	{
		...
		if (m_bBackRectangle)
		{
			pv[12].pos.y = pv[14].pos.y = (float)(m_rect.top - m_nUpRectOffset - m_rBorderOffset.top + m_vertOffset *
				upIdx);
			pv[13].pos.y = pv[15].pos.y = (float)(m_rect.top - m_nUpRectOffset + m_rBorderOffset.bottom + m_vertOffset *
				(downIdx + 1));
		}
		m_rs->UnLockVertexBuffer(m_idVBuf);
	}
...
void CXI_FORMATEDTEXT::CheckScrollButtons()
{
	...
	if (m_idUpEnableTexture == -1 && m_idUpDisableTexture == -1 &&
		m_idDownEnableTexture == -1 && m_idDownDisableTexture == -1)
		return;
...
void CXI_FORMATEDTEXT::ScrollerUpdate()
{
	...
	CINODE* pNode = ((XINTERFACE*)EntityManager::GetEntityPointer(g_idInterface))->FindNode(m_sScrollerName, nullptr);
	if (!pNode) return;
