    ...
    for (auto i = (nItemNum == -1 ? 0 : nItemNum); i < (nItemNum == -1 ? 4 : nItemNum + 1); i++) {
      ...
      if (pAttrTmp != nullptr) {
        ...
        if (m_oneTexID[i] == -1) m_oneImgID[i] = -1;
          ...
          m_oneImgID[i] = pPictureService->GetImageNum(m_sGroupName[m_oneTexID[i]],
                                                       pAttrTmp->GetAttribute("img1"));
        if (m_twoTexID[i] == -1) m_twoImgID[i] = -1;
        else
          m_twoImgID[i] = pPictureService->GetImageNum(m_sGroupName[m_twoTexID[i]],
