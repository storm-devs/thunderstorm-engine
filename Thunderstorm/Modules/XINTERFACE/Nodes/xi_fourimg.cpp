...
void CXI_FOURIMAGE::Draw(bool bSelected, uint32_t Delta_Time)
{
	...
	if (m_bUse)
	{
		...
		if (m_bShowBorder && bSelected)
		{
			...
			pV[0].pos.x = (float)m_imgRect[m_nSelectItem].left;
			pV[0].pos.y = (float)m_imgRect[m_nSelectItem].top;
			pV[1].pos.x = (float)m_imgRect[m_nSelectItem].left;
			pV[1].pos.y = (float)m_imgRect[m_nSelectItem].bottom;
			pV[2].pos.x = (float)m_imgRect[m_nSelectItem].right;
			pV[2].pos.y = (float)m_imgRect[m_nSelectItem].top;
			pV[3].pos.x = (float)m_imgRect[m_nSelectItem].right;
			pV[3].pos.y = (float)m_imgRect[m_nSelectItem].bottom;
			// show select border
        ...
        const auto color = 0xFFFFFFFF;
				...
				if (bUseOneString)
				{
					...
					if (m_pOneStr[i] != nullptr)
						...
						m_rs->ExtPrint(m_oneStrFont, color, 0, nAlignment1, false, 1.f, m_screenSize.x,
						               m_screenSize.y, posX + m_xOneOffset, posY, "%s",
				...
				if (bUseTwoString)
				{
					...
					if (m_pTwoStr[i] != nullptr)
						...
						m_rs->ExtPrint(m_twoStrFont, color, 0, nAlignment2, false, 1.f, m_screenSize.x,
						               m_screenSize.y, posX + m_xTwoOffset, posY, "%s",
    ...
    auto* pA = pAttribute->GetAttributeClass("ImagesGroup");
      ...
      auto* pAttrTmp = pAttribute->GetAttributeClass(param);
			if (pAttrTmp != nullptr)
			{
				...
				if (m_oneTexID[i] != -1 && m_oneTexID[i] < m_nTexturesQuantity) m_oneImgID[i] = pPictureService->
					GetImageNum(m_sGroupName[m_oneTexID[i]], pAttrTmp->GetAttribute("img1"));
				else m_oneImgID[i] = -1;
	...
	if (pAttribute != nullptr)
	{
      ...
      auto* pAttrTmp = pAttribute->GetAttributeClass(param);
			if (pAttrTmp != nullptr)
			{
				...
				else m_oneImgID[i] = pPictureService->GetImageNum(m_sGroupName[m_oneTexID[i]],
				                                                  pAttrTmp->GetAttribute("img1"));
				if (m_twoTexID[i] == -1) m_twoImgID[i] = -1;
				else m_twoImgID[i] = pPictureService->GetImageNum(m_sGroupName[m_twoTexID[i]],
				                                                  pAttrTmp->GetAttribute("img2"));
