...
void CXI_GLOWCURSOR::SetRectanglesToPosition(const XYRECT& rectXY) {
  ...
  m_pTexVert[1].pos.x = m_pTexVert[3].pos.x = m_pTexVert[7].pos.x = m_pTexVert[9].pos.x = static_cast<float>(rectXY.
    right);
  m_pTexVert[2].pos.y = m_pTexVert[3].pos.y = m_pTexVert[9].pos.y = m_pTexVert[11].pos.y = static_cast<float>(rectXY.
    bottom);
