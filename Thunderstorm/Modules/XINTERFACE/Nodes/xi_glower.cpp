  ...
  for (long i = 0; i < m_nQuantity; i++) {
    if (m_glows[i].action != GLOW_ACTION_NONE) {
      ...
      if (m_glows[i].action == GLOW_ACTION_BLEND)
        m_glows[i].rect.dwColor = ColorInterpolate(m_dwMaxColor, m_dwMinColor,
                                                   1.f - static_cast<float>(m_glows[i].curTime) / m_glows[i].allTime);
      // show this rectangle
      ...
      if ((m_glows[i].curTime -= Delta_Time) < 0) {
        ...
        case GLOW_ACTION_COLORUP:
          ...
          m_glows[i].curTime = m_glows[i].allTime = static_cast<long>(m_minShowTime + (m_maxShowTime - m_minShowTime) *
            rand() / RAND_MAX);
          break;
        case GLOW_ACTION_SHOW:
          m_glows[i].action = GLOW_ACTION_BLEND;
          m_glows[i].curTime = m_glows[i].allTime = static_cast<long>(m_minGlowTime + static_cast<float>(m_maxGlowTime -
            m_minGlowTime) * rand() / RAND_MAX);
          break;
