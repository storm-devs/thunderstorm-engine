		...
		if (m_glows[i].action != GLOW_ACTION_NONE)
		{
			...
			if (m_glows[i].action == GLOW_ACTION_COLORUP)
				m_glows[i].rect.dwColor = ColorInterpolate(m_dwMaxColor, m_dwMinColor,
				                                           (float)m_glows[i].curTime / m_glows[i].allTime);
			if (m_glows[i].action == GLOW_ACTION_BLEND)
				m_glows[i].rect.dwColor = ColorInterpolate(m_dwMaxColor, m_dwMinColor,
				                                           1.f - (float)m_glows[i].curTime / m_glows[i].allTime);
			// show this rectangle
...
bool CXI_GLOWER::Init(INIFILE* ini1, const char * name1, INIFILE* ini2, const char * name2, VDX9RENDER* rs, XYRECT& hostRect,
                      XYPOINT& ScreenSize)
{
