...
uint32_t CXI_LINECOLLECTION::MessageProc(long msgcode, MESSAGE& message)
{
	...
	case 1: // добавить динию и вернуть ее номер
      ...
      const long nLineNum = m_aLines.size() / 2;
			...
			m_aLines[nLineNum * 2].vPos.x = (float)nLeft;
			m_aLines[nLineNum * 2 + 1].vPos.x = (float)nRight;
			m_aLines[nLineNum * 2].vPos.y = (float)nTop;
			m_aLines[nLineNum * 2 + 1].vPos.y = (float)nBottom;
			return nLineNum;
		}
		break;
	}
