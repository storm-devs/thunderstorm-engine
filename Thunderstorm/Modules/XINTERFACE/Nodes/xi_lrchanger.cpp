...
void CXI_LRCHANGER::Draw(bool bSelected, uint32_t Delta_Time)
{
	if (m_bUse)
	{
		...
		if (bSelected) curCol = ColorInterpolate(m_dwDarkSelCol, m_dwLightSelCol,
...
bool CXI_LRCHANGER::Init(INIFILE* ini1, const char * name1, INIFILE* ini2, const char * name2, VDX9RENDER* rs, XYRECT& hostRect,
                         XYPOINT& ScreenSize)
{
