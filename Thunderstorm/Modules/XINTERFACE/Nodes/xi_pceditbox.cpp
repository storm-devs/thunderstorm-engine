      ...
      for (long n = 0; n < core.Controls->GetKeyBufferLength(); n++) {
        if (pKeys[n].bSystem) {
          ...
          case VK_HOME: m_nEditPos = 0;
            break;
          case VK_DELETE: if (m_nEditPos < static_cast<long>(str.size())) str.erase(m_nEditPos, 1);
            break;
          case VK_LEFT: if (m_nEditPos > 0) m_nEditPos--;
            break;
          case VK_RIGHT: if (m_nEditPos < static_cast<long>(str.size())) m_nEditPos++;
            break;
          }
        }
...
void CXI_PCEDITBOX::ShowCursorPosition(std::string& str) {
  ...
  if (m_nEditPos > static_cast<long>(str.size())) m_nEditPos = static_cast<long>(str.size());
...
void CXI_PCEDITBOX::InsertSymbol(std::string& str, char chr) {
  if (static_cast<unsigned char>(chr) < 32) return;
  ...
  if (m_nMaxSize >= 0 && static_cast<long>(str.size()) >= m_nMaxSize) return;
  ...
  m_nEditPos++;
  if (m_nEditPos > static_cast<long>(str.size())) m_nEditPos = str.size();
}
