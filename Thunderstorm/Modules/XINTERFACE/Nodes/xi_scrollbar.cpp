        ...
        auto pcStr = pA->GetAttribute("str");
				if (pcStr)
				{
					m_rs->ExtPrint(m_nFontID, m_dwFontColor, 0, PR_ALIGN_CENTER, true, m_fFontScale, m_screenSize.x,
					               m_screenSize.y,
					               (m_rect.left + m_rect.right) / 2 + m_pntFontOffset.x, m_rect.top + m_pntFontOffset.y,
					               "%s", pcStr);
				}
			}
		}
...
bool CXI_SCROLLBAR::Init(INIFILE* ini1, const char * name1, INIFILE* ini2, const char * name2, VDX9RENDER* rs, XYRECT& hostRect,
                         XYPOINT& ScreenSize)
{
