        ...
        if (pA) {
          ...
          if (pcGroupName) {
            ...
            if (m_aImg[i].fpSize.x > 0.f && m_aImg[i].fpSize.y > 0.f)
              m_aImg[i].pImg->SetSize(static_cast<long>(m_aImg[i].fpSize.x), static_cast<long>(m_aImg[i].fpSize.y));
          }
          else {
  ...
  if (pAttribute) {
    ...
    if (pA) {
      pA->SetAttributeUseFloat(
        "x", (frNewUV.right - frNewUV.left) * m_fpBaseSize.x / static_cast<float>(m_rect.right - m_rect.left));
      pA->SetAttributeUseFloat(
        "y", (frNewUV.bottom - frNewUV.top) * m_fpBaseSize.y / static_cast<float>(m_rect.bottom - m_rect.top));
    }
  }
}
