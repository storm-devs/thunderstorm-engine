...
void CXI_SCROLLEDPICTURE::ChangeUV(FXYRECT& frNewUV)
{
  ...
  auto pAttribute = api->Entity_GetAttributeClass(g_idInterface, m_nodeName);
    ...
    auto pA = pAttribute->GetAttributeClass("offset");
		...
		if (pA)
		{
			pA->SetAttributeUseFloat(
				"x", (frNewUV.right - frNewUV.left) * m_fpBaseSize.x / (float)(m_rect.right - m_rect.left));
			pA->SetAttributeUseFloat(
				"y", (frNewUV.bottom - frNewUV.top) * m_fpBaseSize.y / (float)(m_rect.bottom - m_rect.top));
		}
	}
}
