...
bool CXI_SCROLLER::Init(INIFILE* ini1, const char * name1, INIFILE* ini2, const char * name2, VDX9RENDER* rs, XYRECT& hostRect,
                        XYPOINT& ScreenSize)
{
...
float CXI_SCROLLER::GetOwnedStep()
{
	...
	for (long n = 0; n < (long)m_asOwnedNodes.size(); n++)
	{
		pNode = ((XINTERFACE*)EntityManager::GetEntityPointer(g_idInterface))->FindNode(
			m_asOwnedNodes[n].c_str(), nullptr);
		if (pNode) break;
	}
	if (!pNode) return 0.f;
