...
bool CXI_SCROLLER::Init(INIFILE *ini1,char *name1, INIFILE *ini2,char *name2, VDX9RENDER *rs, XYRECT &hostRect, XYPOINT &ScreenSize)
{
...
void CXI_SCROLLER::MakeOwnedControl()
{
	...
	{
		CINODE * pNode = ((XINTERFACE*)EntityManager::GetEntityPointer(g_idInterface))->FindNode( m_asOwnedNodes[n].c_str(), nullptr );
		if( !pNode ) continue;
...
float CXI_SCROLLER::GetOwnedStep()
{
	...
	for(long n=0; n<(long)m_asOwnedNodes.size(); n++ )
	{
		pNode = ((XINTERFACE*)EntityManager::GetEntityPointer(g_idInterface))->FindNode( m_asOwnedNodes[n].c_str(), nullptr );
		if( pNode ) break;
	}
	if( !pNode ) return 0.f;
	switch( pNode->m_nNodeType )
