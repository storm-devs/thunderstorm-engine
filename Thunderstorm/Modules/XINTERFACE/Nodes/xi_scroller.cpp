...
void CXI_SCROLLER::MakeOwnedControl() {
  for (long n = 0; n < static_cast<long>(m_asOwnedNodes.size()); n++) {
    auto* const pNode = static_cast<XINTERFACE*>(EntityManager::GetEntityPointer(g_idInterface))->FindNode(
...
float CXI_SCROLLER::GetOwnedStep() {
  ...
  for (long n = 0; n < static_cast<long>(m_asOwnedNodes.size()); n++) {
    pNode = static_cast<XINTERFACE*>(EntityManager::GetEntityPointer(g_idInterface))->FindNode(
      m_asOwnedNodes[n].c_str(), nullptr);
    if (pNode) break;
  }
  if (!pNode) return 0.f;
