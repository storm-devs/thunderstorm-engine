						...
						if (m_bUseTwoString)
						{
							if (m_Image[pScroll->imageNum].str2 != -1L)
								ptrOwner->PrintIntoWindow(m_rect.left, m_rect.right,
								                          ...
								                          m_nTwoStrAlign, true, m_nTwoStrScale, m_screenSize.x,
								                          m_screenSize.y,
								                          long(pScroll->pCenter.x + m_lTwoStrX),
								                          m_rect.top + m_lTwoStrOffset,
								                          pStringService->GetString(m_Image[pScroll->imageNum].str2),
								                          (int)(m_ImageSize.x * pScroll->fCurScale),
								                          (int)(-24 * m_nTwoStrScale));
...
bool CXI_SCROLLIMAGE::Init(INIFILE* ini1, const char * name1, INIFILE* ini2, const char * name2, VDX9RENDER* rs, XYRECT& hostRect,
                           XYPOINT& ScreenSize)
{
