...
bool CXI_SCROLLTEXT::Init(INIFILE* ini1, const char* name1, INIFILE* ini2, const char* name2, VDX9RENDER* rs,
  ...
  SetText(
    "proba string ^fnormal continue proba string one, two, three etc. Continue next stringes from full screen filling");
  SetGlowCursor(false);
  return true;
}
...
long CXI_SCROLLTEXT::FillPices(char* pt, size_t beg, size_t size, long& idx, STRING_PICE* spl, long wid) {
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (static_cast<unsigned long>(chQuantity + sw) > size) break;
    }
