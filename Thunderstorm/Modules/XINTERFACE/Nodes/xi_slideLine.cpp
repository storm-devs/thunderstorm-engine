...
int CXI_SLIDELINE::CommandExecute(int wActCode) {
  if (m_bUse) {
    switch (wActCode) {
    case ACTION_LEFTSTEP: SetNewValue(m_nCurValue - 1);
      break;
    case ACTION_SPEEDLEFT:
      if (m_nCurValue >= m_nSpeedSlide) SetNewValue(m_nCurValue - m_nSpeedSlide);
      else SetNewValue(0);
      break;
    case ACTION_RIGHTSTEP: SetNewValue(m_nCurValue + 1);
      break;
...
void CXI_SLIDELINE::DoMouseControl() {
  ...
  if (m_bDoChangeSlider) {
    if (fmp.x < m_rect.left + m_nBaseLeft)
      ...
      SetNewValue(
        static_cast<long>((fmp.x - m_rect.left - m_nBaseLeft) / (m_rect.right - m_rect.left - m_nBaseLeft - m_nBaseLeft)
          *
          m_nGrateQuantity)
      );
  }
...
void CXI_SLIDELINE::LoadIni(INIFILE* ini1, const char* name1, INIFILE* ini2, const char* name2) {
  ...
  if (m_idVBuf == -1) throw std::exception("can not create the vertex buffers");
  ...
  if (pA != nullptr) m_nCurValue = static_cast<long>(pA->GetAttributeAsFloat("value", 0.f) * m_nGrateQuantity);
...
void CXI_SLIDELINE::SetNewValue(long newValue) {
  ...
  api->PostEvent("eSlideChange", 0, "slf", m_nodeName, m_nCurValue, static_cast<float>(m_nCurValue) / m_nGrateQuantity);
}
