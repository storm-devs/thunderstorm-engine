...
void CXI_STRCOLLECTION::SaveParametersToIni()
{
  ...
  auto* pIni = fio->OpenIniFile((char*)ptrOwner->m_sDialogFileName.c_str());
	...
	for (long n = 0; n < m_nStr; n++)
	{
		...
		{
		case PR_ALIGN_CENTER: pcState[0] = 'C';
			break;
		case PR_ALIGN_RIGHT: pcState[0] = 'R';
			break;
		default: pcState[0] = 'L';
		}
		...
		if (m_pStrDescr[n].strStr)
			sprintf_s(param, sizeof(param),
			          ...
			          ALPHA(m_pStrDescr[n].backColor), RED(m_pStrDescr[n].backColor), GREEN(m_pStrDescr[n].backColor),
