...
void XI_TableCellDescribe::LoadImageParam(ImgDescribe* pImg,ATTRIBUTES* pA)
{
	...
	if( pA->GetAttribute("group") ) {
		...
		if( pA->GetAttribute("texturepointer") )
			pImg->pImage->SetPointerToTexture( (IDirect3DTexture9*)pA->GetAttributeAsQword("texturepointer") );
...
bool CXI_TABLE::Init(INIFILE *ini1,char *name1, INIFILE *ini2,char *name2, VDX9RENDER *rs, XYRECT &hostRect, XYPOINT &ScreenSize)
{
