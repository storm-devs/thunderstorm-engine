...
void XI_TableCellDescribe::Draw(float fLeft, float fTop) {
  ...
  for (n = 0; n < m_aStrings.size(); n++) {
    ...
    CXI_UTILS::PrintTextIntoWindow(m_pTable->m_rs,
                                   ...
                                   m_dwColor, PR_ALIGN_LEFT, true, m_fScale, m_pTable->m_screenSize.x,
                                   m_pTable->m_screenSize.y,
                                   static_cast<long>(fLeft + m_aStrings[n].offset.x), static_cast<long>(fTop + fY),
                                   m_aStrings[n].str.c_str(),
                                   static_cast<long>(fLeft), static_cast<long>(fTop),
...
void XI_TableCellDescribe::SetData(long nColIndex, ATTRIBUTES* pAttr, bool bHeader) {
  ...
  // читаем строку
  m_dwColor = pAttr->GetAttributeAsDword(
    "color", bHeader ? m_pTable->m_dwFontTitleColor : m_pTable->m_dwFontCellColor);
  ...
  auto nWidth = m_pTable->m_anColsWidth[nColIndex] - 2 * m_pTable->m_pntSpaceSize.x - m_nLeftLineWidth - static_cast<
    long>(m_TextOffset.x);
  if (nWidth <= 0) {
    nWidth = 32;
  }
  ...
  for (long n = 0; n < asStr.size(); n++) {
    ...
    if (m_nAlignment == PR_ALIGN_CENTER)
      m_aStrings[n].offset.x += static_cast<float>((nWidth - m_pTable->m_rs->StringWidth(
        (char*)asStr[n].c_str(), m_nFontID,
        m_fScale)) / 2);
...
void XI_TableCellDescribe::LoadImageParam(ImgDescribe* pImg, ATTRIBUTES* pA) const {
  ...
  if ((pcStr = pA->GetAttribute("valign")) != nullptr) {
    ...
    if (nImgAlign != ALIGN_TOP) {
      nH = m_pLine->GetLineHeight() - 2 * m_pTable->m_pntSpaceSize.y - m_nTopLineHeight - pImg
        ->pImage->GetHeight();
...
int CXI_TABLE::CommandExecute(int wActCode) {
  if (m_bUse) {
    ...
    case ACTION_SPEEDRIGHT:
      if (m_bDoColsSelect && m_nSelectIndex != -1 && m_nSelectColIndex < static_cast<long>(m_anColsWidth.size()) - 1) {
        m_nSelectColIndex++;
        SelectCol(m_nSelectColIndex);
      }
      break;
  ...
  if (m_nVLineWidth > 0) {
    ...
    for (c = 0; c < m_nColQuantity - 1; c++) {
      ...
      WriteSquare(&pV[n], m_nBorderIcon_VLine, m_dwBorderColor,
                  nLeft, m_rect.top + m_nBorderWidth, m_nVLineWidth,
...
void CXI_TABLE::WriteSquare(XI_ONETEX_VERTEX* pV, long nImgID, uint32_t dwCol, long nX, long nY, long nW,
                            long nH) const {
  if (!pV) return;
...
void CXI_TABLE::SelectRow(long nRowNum, long nColNum) {
  if (m_bDoColsSelect) {
    if (nColNum < 0 || nColNum >= m_anColsWidth.size()) m_nSelectColIndex = -1;
...
long CXI_TABLE::GetRowTop(long nRow) {
  if (nRow < 0 || nRow >= m_anRowsHeights.size()) return m_rect.top; // ошибочная ситуация
  long nTop = m_rect.top;
...
long CXI_TABLE::GetColLeft(long nCol) {
  if (nCol < 0 || nCol >= m_anColsWidth.size()) return m_rect.left; // ошибочная ситуация
...
void CXI_TABLE::RecalculateLineHeights() {
  ...
  for (; nY < m_rect.bottom; n++) {
    ...
    if (n < m_anRowsHeights.size()) m_anRowsHeights[n] = nH;
    else m_anRowsHeights.push_back(nH);
    nY += nH;
  }
