...
void XI_TableCellDescribe::Draw(float fLeft, float fTop)
{
	...
	for (n = 0; n < m_aStrings.size(); n++)
	{
		...
		CXI_UTILS::PrintTextIntoWindow(m_pTable->m_rs,
		                               ...
		                               (long)(fLeft + m_aStrings[n].offset.x), (long)(fTop + fY),
...
void XI_TableCellDescribe::SetData(long nColIndex, ATTRIBUTES* pAttr, bool bHeader)
{
	...
	while ((long)m_aImage.size() > nIconQuantity)
	...
	for (long n = 0; n < asStr.size(); n++)
	{
		...
		if (m_nAlignment == PR_ALIGN_CENTER)
			m_aStrings[n].offset.x += (float)((nWidth - m_pTable->m_rs->StringWidth((char*)asStr[n].c_str(), m_nFontID,
			                                                                        m_fScale)) / 2);
...
void CXI_TABLE::SetTopIndexForSelect(long nSelIndex)
{
	...
	else if (nSelIndex >= m_nTopIndex + (long)m_anRowsHeights.size() - (m_pHeader ? 1 : 0))
