...
CXI_ToolTip::CXI_ToolTip(VXSERVICE* pPicService, VSTRSERVICE* pStrService, XYPOINT& pntScrSize) : m_pntScreenSize(
  pntScrSize) {
...
void CXI_ToolTip::SetByFormatString(XYRECT& rectOwner, INIFILE* pDefIni, const char* pFmtStr) {
  ...
  for (auto* pcParam = (char*)pFmtStr; pcParam && pcParam[0];) {
    ...
    if (CXI_UTILS::StringGetTokenString(pcParam, tokenString, sizeof(tokenString))) {
      ...
      case InterfaceToken_text:
        ...
        break;
      case InterfaceToken_class: sprintf_s(pcToolTipType, sizeof(pcToolTipType), "%s", tokenString);
        break;
  ...
  if (pDefIni) {
    ...
    m_pntTextOffset = CINODE::GetIniLongPoint(pDefIni, pcToolTipType, nullptr, nullptr, "str_offset",
                                              m_pntTextOffset);
  ...
  m_nUseHeight = m_aSubText.size() * static_cast<long>(m_rs->CharHeight(m_nFontID) * m_fFontScale) + 2 * m_pntTextOffset
    .x;
...
void CXI_ToolTip::UpdateVertexBuffer() {
  ...
  CXI_UTILS::WriteSquareToVertexBuffer(&m_pV[0], m_dwBackColor, m_uvBackMiddle,
                                       m_rPos.left + m_nLeftSideWidth, m_rPos.top, m_rPos.right - m_nRightSideWidth,
                                       m_rPos.bottom);
