...
public:
	...
	void GetRecordTextList(std::vector<std::string> & asStringList, const char* pcQuestID,const char* pcTextID, const char* pcUserData);

	static bool AssembleStringToBuffer(const char* pSrc, long nSrcSize, char* pBuf, long nBufSize,std::vector<UserData>& aUserData);
