...
void STRSERVICE::SetLanguage(const char* sLanguage) {
  ...
  while (true) {
    ...
    else core.Trace("WARNING! Not found directory record for language %s", sLanguage);
    ...
    else core.Trace("WARNING! Not found common strings file record");
...
uint32_t _DialogAssembleStr(VS_STACK* pS) {
  ...
  char param[4096];
  const char* pcStr = g_StringServicePointer
                        ? static_cast<STRSERVICE*>(g_StringServicePointer)->TranslateForDialog(pcID)
                        : pcID;
  if (pcStr) {
