...
void GIEditorList::SetPosition(float fLeft, float fTop, float fRight, float fBottom) {
  ...
  m_nLineQuantity = static_cast<long>((m_frBackRect.bottom - m_frBackRect.top - m_frStrOffset.top - m_frStrOffset.bottom
    ) /
    m_fStrLineStep);
...
void GIEditorList::MakeMouseClick(float fX, float fY) {
  ...
  if (fX >= frString.left && fX <= frString.right &&
    ...
    long nLineNum = 0;
    if (m_fStrLineStep > 0.f) nLineNum = static_cast<long>(fLineOffset / m_fStrLineStep);
