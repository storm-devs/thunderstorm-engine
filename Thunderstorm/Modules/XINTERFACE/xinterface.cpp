...
bool XINTERFACE::SFLB_DoSaveFileData(char* saveName, char* saveData) const {
  ...
  if (dscr.Height > 0) {
    ...
    else core.Trace("Can`t lock screenshot texture");
  }
...
void XINTERFACE::SaveOptionsFile(char* fileName, ATTRIBUTES* pAttr) {
  ...
  if (fileName == nullptr || pAttr == nullptr) return;
	...
	{
		core.Trace("cant create options ini file");
		return;
	}
...
void XINTERFACE::LoadOptionsFile(char* fileName, ATTRIBUTES* pAttr) {
  ...
  if (fileName == nullptr || pAttr == nullptr) return;
	...
	{
		core.Trace("cant open options ini file");
		return;
	}
	strcpy_s(FullPath,PathBuffer);
	strcat_s(FullPath,fileName);
