...
bool XSERVICE::GetTexturePos(long pictureNum, FXYRECT& texRect) {
  if (pictureNum >= 0 && pictureNum < m_dwImageQuantity) {
    ...
    for (gn = 0; gn < m_dwListQuantity; gn++)
      if (pictureNum >= m_pList[gn].pictureStart && pictureNum < m_pList[gn].pictureStart + m_pList[gn].
        pictureQuantity)
        break;
    if (gn < m_dwListQuantity) {
      ...
      texRect.bottom = static_cast<float>(m_pImage[pictureNum].pTextureRect.bottom - m_fHAdd) / m_pList[gn].
        textureHeight;
      return true;
    }
  }
