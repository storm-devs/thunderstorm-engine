...
bool FONT::MakeLong(char* * pDataPointer, long* result) {
  ...
  while (true) {
    ...
    index++;
    if (index > 8) throw std::exception("inf loop");
  }
  return false;
}
...
bool FONT::Init(const char* font_name, const char* iniName, IDirect3DDevice9* _device, VDX9RENDER* _render) {
  ...
  for (n = 30; n < USED_CODES; n++) {
    ...
    else if (n == '=') sprintf_s(key_name, "char_equ");
    else sprintf_s(key_name, "char_%c", static_cast<char>(n));
    ...
    pData = buffer;
    if (!MakeLong(&pData, &ltmp)) throw std::exception("invalid font record");
    CharT[n].Pos.x1 = 0;
