...
//--------------------------------------------------------------------
IVBufferManager::IVBufferManager(VDX9RENDER* renderer_, long vertex_type, int vertex_size, size_t index_count, size_t vertex_count, size_t max_size) :
  renderer_(renderer_), max_size_(max_size), vertex_type_(vertex_type), vertex_size_(vertex_size), index_count_(index_count), vertex_count_(vertex_count),
...
void IVBufferManager::DrawBuffers(const char* _technique) const {
  ...
  renderer_->DrawBuffer(vertex_buffer_,
                       vertex_size_,
                       index_buffer_,
                       0,
                       actual_size_ * vertex_count_,
                       0,
                       actual_size_ * index_count_ / 3,
                       _technique);
}
