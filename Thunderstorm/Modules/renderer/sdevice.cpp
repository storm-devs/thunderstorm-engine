...
#endif
  {
    ...
    if (bShowFps) {
      ...
      */
      ...
      if (bShowExInfo) {
        Print(80, 70, "t: %d, %.3f Mb", dwTotalTexNum, static_cast<float>(dwTotalTexSize) / (1024.0f * 1024.0f));
        Print(80, 90, "v: %d, %.3f Mb", dwTotalVB, static_cast<float>(dwTotalVBSize) / (1024.0f * 1024.0f));
        Print(80, 110, "i: %d, %.3f Mb", dwTotalIB, static_cast<float>(dwTotalIBSize) / (1024.0f * 1024.0f));
...
void DX9RENDER::RunStart() {
  ...
  if (bSeaEffect) {
    ...
    for (long y = 0; y < 32; y++) {
      for (long x = 0; x < 32; x++) {
        qv[x + y * 32].vPos = Vector4(sx * static_cast<float>(x) / 31.0f, sy * static_cast<float>(y) / 31.0f, 0.0f,
                                      1.0f);

        qv[x + y * 32].u0 = (sx * static_cast<float>(x) / 31.0f + sinf(
            fSin + static_cast<float>(x) / 31.0f * PI * 16.0f) * fDX + fDX) /
          sx2;
        qv[x + y * 32].v0 = (sy * static_cast<float>(y) / 31.0f + sinf(
            fSin + static_cast<float>(y) / 31.0f * PI * 16.0f) * fDY + fDY) /
          sy2;
      }
    }
  }
...
HRESULT DX9RENDER::CreateOffscreenPlainSurface(UINT Width, UINT Height, D3DFORMAT Format,
...
void DX9RENDER::ProgressView() {
  ...
  if (abs(static_cast<long>(progressUpdateTime - time)) < 50) return;
  ...
  v[0].x = 0.0f;
  v[0].y = 0.0f;
  v[1].x = static_cast<float>(vp.Width);
  v[1].y = 0.0f;
  v[2].x = 0.0f;
  v[2].y = static_cast<float>(vp.Height);
  v[3].x = static_cast<float>(vp.Width);
  v[3].y = static_cast<float>(vp.Height);
  v[0].u = 0.0f;
  v[0].v = 0.0f;
  v[1].u = 1.0f;
  v[1].v = 0.0f;
  ...
  v[0].u = fx / static_cast<float>(sizeX);
  v[0].v = fy / static_cast<float>(sizeY);
  v[1].u = (fx + 1.0f) / static_cast<float>(sizeX);
  v[1].v = fy / static_cast<float>(sizeY);
  v[2].u = fx / static_cast<float>(sizeX);
  v[2].v = (fy + 1.0f) / static_cast<float>(sizeY);
  v[3].u = (fx + 1.0f) / static_cast<float>(sizeX);
  v[3].v = (fy + 1.0f) / static_cast<float>(sizeY);
  //Рисуем
...
void DX9RENDER::SetColorParameters(float fGamma, float fBrightness, float fContrast) {
  ...
  for (uint32_t i = 0; i < 256; i++) {
    float fRamp = fContrast * 255.0f * 256.0f * powf(static_cast<float>(i / 255.0f), 1.0f / fGamma) + fBrightness *
      256.0f;
