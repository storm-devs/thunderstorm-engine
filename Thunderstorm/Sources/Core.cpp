...
bool CORE::Run() {
  ...
  if (pVYear) pVYear->Set(static_cast<long>(st.wYear));
  if (pVMonth) pVMonth->Set(static_cast<long>(st.wMonth));
  if (pVDay) pVDay->Set(static_cast<long>(st.wDay));

  //if (!bNetActive)
  {
    if (Controls && Controls->GetDebugAsyncKeyState('R') < 0) Timer.Delta_Time *= 10;
    if (Controls && Controls->GetDebugAsyncKeyState('Y') < 0)
      Timer.Delta_Time = static_cast<uint32_t>(Timer.
        Delta_Time * 0.2f);
  ...
  if (Controls && Controls->GetDebugAsyncKeyState(VK_F8) < 0 &&
    ...
    for (long y = 0; y < nSplitScreenshotGrid; y++)
      for (long x = 0; x < nSplitScreenshotGrid; x++) {
        auto tmpang = ang;
        tmpang.y += (static_cast<float>(x) - (nSplitScreenshotGrid - 1) * 0.5f) * (fPersp / nSplitScreenshotGrid);
        tmpang.x += (static_cast<float>(y) - (nSplitScreenshotGrid - 1) * 0.5f) * (fPersp / nSplitScreenshotGrid);
...
void CORE::ProcessEngineIniFile() {
  ...
  if (res) {
    ...
    if (core.Controls == nullptr) core.Controls = static_cast<CONTROLS*>(MakeClass("controls"));
  }
  else {
    delete Controls;
    Controls = nullptr;
