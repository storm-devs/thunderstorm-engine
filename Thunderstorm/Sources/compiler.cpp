...
void COMPILER::FindErrorSource()
{
	...
	do
	{
		...
		ip++;
		if ((uint8_t)pCodeBase[ip] < 0xff)
		{
			token_data_size = (uint8_t)pCodeBase[ip];
			ip++;
		}
		else
		{
...
void COMPILER::CompileToken(SEGMENT_DESC& Segment, S_TOKEN_TYPE Token_type, uint32_t data_blocks_num,...)
{
	...
	if (write_size < 0xff)
	{
		...
		Segment.pCode[Segment.BCode_Program_size] = (uint8_t)write_size;
	...
	else
	{
		...
		Segment.pCode[Segment.BCode_Program_size] = (uint8_t)0xff;
		Segment.BCode_Program_size += 1;
			...
			if (inout == 0)
			{
				if (!bFunctionBlock)
				{
					...
					if (Token.GetType() == OPEN_BRACKET)
					{
						...
						else if (bImport) fi.offset = INVALID_FUNC_OFFSET;
						else
                            ...
                            uint32_t continue_jump, uint32_t break_offset, STRINGS_LIST& BreakTable)
{
	...
	*/
	...
	switch (bound_type)
	{
	case CLOSE_BRACKET: balance_bracket = 1;
		break;
	case BLOCK_OUT: balance_block = 1;
		break;
	}
	...
	do
	{
		...
		case BLOCK_OUT:
			...
			if (balance_block == 0)
			{
				switch (bound_type)
				{
				case CLOSE_BRACKET: SetError("missed ')'");
					break;
				case BLOCK_OUT: SetError("missed '}'");
					break;
				default: SetError("syntax error");
					break;
				}
				return false;
			}
		...
		case IF_BLOCK:
			...
			{
				inout++;
				if (!CompileBlock(Segment, bFunctionBlock, inout, BLOCK_OUT, continue_jump, break_offset,
				                  BreakTable)) return false;
			}
			else
			{
				Token.StepBack();
				if (!CompileBlock(Segment, bFunctionBlock, inout, SEPARATOR, continue_jump, break_offset,
				                  BreakTable)) return false;
			}
			memcpy(&Segment.pCode[update_offset], &Segment.BCode_Program_size, sizeof(uint32_t));
		...
		case WHILE_BLOCK:
			...
			else
			{
				...
				Token.StepBack();
				if (!CompileBlock(Segment, bFunctionBlock, inout, SEPARATOR, jump_offset, 6, BreakUpdateTable)) return
					false;
				// **** ?
			}
			CompileToken(Segment, JUMP, 1, (char *)&jump_offset, sizeof(uint32_t));
		...
		case SWITCH_COMMAND:
			...
			if (Token_type == BLOCK_IN)
			{
				inout++;
				//if(!CompileBlock(Segment,bFunctionBlock,inout,BLOCK_OUT,jump_offset,6,BreakUpdateTable)) return false;
				if (!CompileBlock(Segment, bFunctionBlock, inout, BLOCK_OUT, continue_jump, 6, BreakUpdateTable)) return
					false;
			}
			else
			{
		...
		case CASE_COMMAND:
			...
			//if(!CompileBlock(Segment,bFunctionBlock,inout,BREAK_COMMAND,INVALID_OFFSET,0,BreakTable)) return false;
			// ??? why INVALID_OFFSET on continue_jump here? dont remember; anyway now change to normal
			if (!CompileBlock(Segment, bFunctionBlock, inout, BREAK_COMMAND, continue_jump, 0, BreakTable)) return
				false;
		...
		case TVOID:
			...
			if (Token_type == CALL_FUNCTION)
			{
				...
				do
				{
					...
					case DEBUG_LINEFEED:
						break;
					default: SetError("Invalid function declaration");
						return false;
					}
				}
...
*/
		...
		case FALSE_CONST:
			...
			// write value
		case NUMBER: CompileNumber(Segment);
			break;
		case FLOAT_NUMBER: CompileFloatNumber(Segment);
			break;
		case STRING: CompileString(Segment);
			break;
		...
		case CLASS_DECL:
			...
			while (nBalance > 0)
			{
				switch (Token.Get())
				{
				case BLOCK_IN: nBalance++;
					break;
				case BLOCK_OUT: nBalance--;
					break;
				}
			}
			break;
...
S_TOKEN_TYPE COMPILER::BC_TokenGet(uint32_t& ip, uint32_t& token_data_size)
{
	...
	InstructionPointer++;
	if ((uint8_t)pRunCodeBase[InstructionPointer] < 0xff)
	{
		token_data_size = (uint8_t)pRunCodeBase[InstructionPointer];
		InstructionPointer++;
	}
	else
	{
	...
	do
	{
		...
		case ACCESS_WORD_CODE:
			...
			if (nLeftOperandIndex == INVALID_ARRAY_INDEX)
			{
				...
				pLeftOperandAClass = pLeftOperandAClass->VerifyAttributeClassByCode(
					*((long *)&pRunCodeBase[TLR_DataOffset]));
				break;
			}
			if (pLeftOperandAClass == nullptr)
			{
		...
		case BLOCK_OUT:
			...
			return false;
			break;
		default: SetError("Incorrect instruction code");
			return false;
		...
		case OP_DECADD:
			...
			switch (OpType)
			{
			case OP_INCADD: pV->Plus(pVV);
				break;
			case OP_DECADD: pV->Minus(pVV);
				break;
			case OP_MULTIPLYEQ: pV->Multiply(pVV);
				break;
			case OP_DIVIDEEQ: pV->Divide(pVV);
				break;
			}
...
uint32_t COMPILER::SetScriptFunction(IFUNCINFO* pFuncInfo)
{
	...
	if (fi.decl_file_name == nullptr) fi.decl_file_name = "unknown";
