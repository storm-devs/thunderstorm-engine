	...
	case CALL:
		if (bSkip)
		{
			...
			do
			{
				switch (BC_TokenGet())
				{
				case OPEN_BRACKET: local_in++;
					break;
				case CLOSE_BRACKET: local_in--;
					break;
				}
			}
			while (local_in > 0);
...
bool COMPILER::CompileExpression_L7(SEGMENT_DESC& Segment)
{
	...
	case UNKNOWN:
		...
		case CALL_FUNCTION:
			...
			// call function
			if (sttFuncCallType == CALL_FUNCTION) CompileToken(Segment, CALL_FUNCTION, 1, (char *)&dwRCode,
			                                                   sizeof(uint32_t));
			else
			{
