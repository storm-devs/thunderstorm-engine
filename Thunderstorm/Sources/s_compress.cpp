...
bool COMPRESS::AppendCode(char* & pDestination, uint32_t& nPackedSize, uint32_t nCode) {
  ...
  if (pDestination == nullptr) pDestination = static_cast<char*>(malloc(nBSize));
  else pDestination = static_cast<char*>(realloc(pDestination, nPackedSize + nBSize));
...
bool COMPRESS::AAppendCode(char* & pDest, uint32_t& dwDestinationBufferSize, uint32_t dwCode, uint32_t& dwBitsOnCode,
  ...
  for (uint32_t n = 0; n < dwBitsOnCode; n++) {
    ...
    if (DestMask == 0) {
      ...
      if (pDest == nullptr) pDest = static_cast<char*>(malloc(dwDestinationBufferSize));
      else pDest = static_cast<char*>(realloc(pDest, dwDestinationBufferSize));
...
bool COMPRESS::AppendData(char* & pDestination, uint32_t& nUnpackedSize, char* pData, uint32_t nSize) {
  if (pDestination == nullptr) pDestination = static_cast<char*>(malloc(nSize));
  else pDestination = static_cast<char*>(realloc(pDestination, nUnpackedSize + nSize));
  if (pDestination == nullptr) return false;
