	...
	if (CDebug.SourceView)
		...
		case WM_KEYDOWN:
			...
			case VK_HOME:
				...
				break;
			case VK_END:
				if (GetAsyncKeyState(VK_CONTROL) < 0) CDebug
				                                      .SourceView->SetActiveLine(CDebug.SourceView->nLinesNum - 1);
				break;
			case 'G':
				break;
			case 'F':
			...
			case VK_F3:
				...
				break;
#endif
			case VK_UP: CDebug.SourceView->DoStep(-1);
				break;
			case VK_DOWN: CDebug.SourceView->DoStep(1);
				break;
			case VK_PRIOR: CDebug.SourceView->DoStep(-CDebug.SourceView->nClientLinesSize);
				break;
			case VK_NEXT: CDebug.SourceView->DoStep(CDebug.SourceView->nClientLinesSize);
				break;
			case VK_F4:
			...
			case VK_F9:
				...
				SelectionRect.top = (CDebug.SourceView->nActiveLine - CDebug.SourceView->nTopLine) * CDebug
				                                                                                     .SourceView->
				                                                                                     nFontHeight;
					...
					if (CDebug.SourceView->nEndSelection > CDebug.SourceView->nStartSelection)
						memcpy(lptstrCopy,
						       CDebug.SourceView->pSourceFile + CDebug.SourceView->pLineOffset[CDebug
						                                                                       .SourceView->nActiveLine]
						       + CDebug.SourceView->nStartSelection, dwBytes - 1);
					else
						memcpy(lptstrCopy,
						       CDebug.SourceView->pSourceFile + CDebug.SourceView->pLineOffset[CDebug
						                                                                       .SourceView->nActiveLine]
						       + CDebug.SourceView->nEndSelection, dwBytes - 1);
		...
		case WM_LBUTTONUP:
			...
			if (CDebug.SourceView->bDrag)
			{
				...
				if (CDebug.SourceView->sCopyPasteBuffer.size() && WindowFromPoint(pnt) == CDebug
				                                                                          .WatcherList->
				                                                                          GetWindowHandle())
				{
					long iItemCount = ListView_GetItemCount(CDebug.WatcherList->GetWindowHandle());
		...
		case WM_LBUTTONDOWN:
			...
			{
				SelectionRect = CDebug.SourceView->Pos;
				SelectionRect.top = (CDebug.SourceView->Cursor.line - CDebug.SourceView->nTopLine) * CDebug
				                                                                                     .SourceView->
				                                                                                     nFontHeight;
				SelectionRect.bottom = SelectionRect.top + CDebug.SourceView->nFontHeight;
				InvalidateRect(hwnd, &SelectionRect, true);
			...
			if (aline >= CDebug.SourceView->nLinesNum) CDebug.SourceView->nActiveLine = CDebug.SourceView->nLinesNum -
				1;
		...
		case WM_VSCROLL:
			...
			case SB_PAGEUP:
			case SB_PAGEDOWN:
				CDebug.SourceView->nTopLine += (CDebug.SourceView->nClientLinesSize - 1) * ((nScrollCode == SB_PAGEDOWN)
					                                                                            ? 1
					                                                                            : -1);
...
void SOURCE_VIEW::OnPaint()
{
	...
	if (pSourceFile)
	{
		...
		for (uint32_t n = nTopLine; n < nTopLine + nClientLinesSize + 1; n++)
		{
			...
			if (n != nLinesNum - 1)
			{
				if (nTextLen < 2) nTextLen = 0;
				else nTextLen = nTextLen - 2;
			}
...
{
	for (long i = 0; i < nLinesNum; i++) pBookmarks[i] = false;
	htBookmarks.clear();
	InvalidateRect(hMain, nullptr, true);
}
...
bool SOURCE_VIEW::SetVariableOnChange(const char* pString, bool bSet)
{
	...
	if (sToken == "[")
	{
		...
		auto* pV = (VDATA *)api->GetScriptVariable(sVarName.c_str(), nullptr);
		if (!pV) return false;
