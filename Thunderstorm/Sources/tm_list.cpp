...
void TM_LIST::AddColumn(const char* name, long length) {
  ...
  if (name) strcpy_s(string, name);
  else strcpy_s(string, "");
          ...
          if (edit_item >= 0 && edit_subitem >= 0) {
            ...
            for (long i = 0; i < static_cast<long>(lines); i++) {
              ...
              *(uint16_t*)TextEditBuffer = MAX_STR_SIZE - 2;
              const long chars = SendMessage(hEdit,EM_GETLINE, i, (LPARAM)static_cast<LPCSTR>(TextEditBuffer));
    ...
    case NM_DBLCLK:
      ...
      if (lpnmlv) {
        if (lpnmlv->iItem >= 0) {
          ...
          hEdit = CreateWindowEx(WS_EX_TOPMOST, "EDIT", "",
