...
void NoiseCloud::Draw(VDX9RENDER * rs)
{
	...
	for(long i = 0; i < sizeof(rect)/sizeof(Rect); i++)
	{
		...
		buf[i*6 + 0].u = 0.0f; buf[i*6 + 0].v = 0.0f;
		buf[i*6 + 1].pos = pos + CVECTOR(size*(-cs - sn), 0.0f, size*(sn - cs));
		buf[i*6 + 1].u = 0.0f; buf[i*6 + 1].v = 1.0f;
		buf[i*6 + 2].pos = pos + CVECTOR(size*(cs + sn), 0.0f, size*(-sn + cs));
		buf[i*6 + 2].u = 1.0f; buf[i*6 + 2].v = 0.0f;
		buf[i*6 + 3].pos = buf[i*6 + 2].pos;
		buf[i*6 + 3].u = 1.0f; buf[i*6 + 3].v = 0.0f;
	...
	rs->DrawPrimitiveUP(D3DPT_TRIANGLELIST, D3DFVF_XYZ | D3DFVF_DIFFUSE | D3DFVF_TEX1, 2*sizeof(rect)/sizeof(Rect), buf, sizeof(Vertex), "TornadoClouds");
}
