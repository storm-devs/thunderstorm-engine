	...
	for(long i = 0; i < numRects; i++)
	{
		...
		if(r.dwColor & 0x0000ff00)
		{
			if(r.dwColor & 0x00000400)
			{
				...
				lght[0].c = lightningColor; lght[0].u = u; lght[0].v = 0.01f;
				lght[1].pos = r.vPos + vx*width - CVECTOR(0.0f, 3.0f, 0.0f);
				lght[1].c = lightningColor; lght[1].u = u + kFrames; lght[1].v = 0.01f;
				lght[2].pos = lght[0].pos; lght[2].pos.y = 0.5f;
				lght[2].c = lightningColor; lght[2].u = u; lght[2].v = 0.99f;
				lght[3].pos = lght[1].pos; lght[3].pos.y = 0.5f;
				lght[3].c = lightningColor; lght[3].u = u + kFrames; lght[3].v = 0.99f;
				rs->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, D3DFVF_XYZ | D3DFVF_DIFFUSE | D3DFVF_TEX1, 2, lght, sizeof(Vertex), "WdmLightning");
			}
