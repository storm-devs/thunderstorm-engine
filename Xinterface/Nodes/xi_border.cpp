...
bool CXI_BORDER::Init(INIFILE *ini1,char *name1, INIFILE *ini2,char *name2, VDX9RENDER *rs, XYRECT &hostRect, XYPOINT &ScreenSize)
{
...
void CXI_BORDER::LoadIni(INIFILE *ini1,char *name1, INIFILE *ini2,char *name2)
{
	...
	} else pPictureService->GetTexturePos( m_nLeftTopPicture, m_frLeftTopUV );
	...
	} else pPictureService->GetTexturePos( m_nRightTopPicture, m_frRightTopUV );
	...
	} else pPictureService->GetTexturePos( m_nLeftBottomPicture, m_frLeftBottomUV );
	...
	} else pPictureService->GetTexturePos( m_nRightBottomPicture, m_frRightBottomUV );
	...
	} else pPictureService->GetTexturePos( m_nTopLinePicture, m_frTopLineUV );
	...
	} else pPictureService->GetTexturePos( m_nBottomLinePicture, m_frBottomLineUV );
	...
	} else pPictureService->GetTexturePos( m_nLeftLinePicture, m_frLeftLineUV );
	...
	} else pPictureService->GetTexturePos( m_nRightLinePicture, m_frRightLineUV );
	m_nRightLineWidth = GetIniLong( ini1,name1, ini2,name2, "rightline_width", 5 );

	if( m_pCaptionImage && m_nCaptionHeight>0 ) {
		XYRECT rCapRect; ZERO(rCapRect);
...
void CXI_BORDER::FillVertexBuffers()
{
	...
	// top line
	WriteVertexForSquare( &pV[0], m_frTopLineUV, m_dwColor,
			m_rect.left + m_pntLeftTopSize.x, m_rect.top,
			m_rect.right - m_pntRightTopSize.x, m_rect.top + m_nTopLineHeight );
	// bottom line
	WriteVertexForSquare( &pV[4], m_frBottomLineUV, m_dwColor,
		m_rect.left + m_pntLeftTopSize.x, m_rect.bottom - m_nBottomLineHeight,
		m_rect.right - m_pntRightTopSize.x, m_rect.bottom );
	// left line
	WriteVertexForSquare( &pV[8], m_frLeftLineUV, m_dwColor,
		m_rect.left, m_rect.top + m_pntLeftTopSize.y,
		m_rect.left + m_nLeftLineWidth, m_rect.bottom - m_pntLeftBottomSize.y );
	// right line
	WriteVertexForSquare( &pV[12], m_frRightLineUV, m_dwColor,
		m_rect.right - m_nRightLineWidth, m_rect.top + m_pntRightTopSize.y,
		m_rect.right, m_rect.bottom - m_pntRightBottomSize.y );
	// left top corner
	WriteVertexForSquare( &pV[16], m_frLeftTopUV, m_dwColor,
		m_rect.left, m_rect.top,
		m_rect.left + m_pntLeftTopSize.x, m_rect.top + m_pntLeftTopSize.y );
	// right top corner
	WriteVertexForSquare( &pV[20], m_frRightTopUV, m_dwColor,
		m_rect.right - m_pntRightTopSize.x, m_rect.top,
		m_rect.right, m_rect.top + m_pntRightTopSize.y );
	// left bottom corner
	WriteVertexForSquare( &pV[24], m_frLeftBottomUV, m_dwColor,
		m_rect.left, m_rect.bottom - m_pntLeftBottomSize.y,
		m_rect.left + m_pntLeftBottomSize.x, m_rect.bottom );
	// right bottom corner
	WriteVertexForSquare( &pV[28], m_frRightBottomUV, m_dwColor,
		m_rect.right - m_pntRightBottomSize.x, m_rect.bottom - m_pntRightBottomSize.y,
		m_rect.right, m_rect.bottom );

	// caption
	if( m_nCaptionHeight>0 && m_pCaptionImage ) {
		// caption line
		WriteVertexForSquare( &pV[32], m_frTopLineUV, m_dwColor,
			m_rect.left + m_nLeftLineWidth, m_rect.top + m_nCaptionHeight,
			m_rect.right - m_nRightLineWidth, m_rect.top + m_nCaptionHeight + m_mCaptionDividerHeight );
	}
