...
bool CXI_BOUNDER::Init(INIFILE *ini1,char *name1, INIFILE *ini2,char *name2, VDX9RENDER *rs, XYRECT &hostRect, XYPOINT &ScreenSize)
{
...
void CXI_BOUNDER::LoadIni(INIFILE *ini1,char *name1, INIFILE *ini2,char *name2)
{
	int i;
	char param[256];
	char param2[256];
    ...
    for(i=0; i<4; i++)
    {
        ...
        {
        case 0:
            fRectTmp.left = (float)m_rect.left;
            fRectTmp.right = (float)m_rect.left + m_fAngleWidth;
            fRectTmp.top = (float)m_rect.top;
            fRectTmp.bottom = (float)m_rect.top + m_fAngleHeight;
            pPictureService->GetTexturePos(m_idAngle,texRectTmp);
            break;
        case 1:
            fRectTmp.left = (float)m_rect.right - m_fAngleWidth;
            fRectTmp.right = (float)m_rect.right;
            fRectTmp.top = (float)m_rect.top;
            fRectTmp.bottom = (float)m_rect.top + m_fAngleHeight;
        ...
        case 2:
            fRectTmp.left = (float)m_rect.left;
            fRectTmp.right = (float)m_rect.left + m_fAngleWidth;
            fRectTmp.top = (float)m_rect.bottom - m_fAngleHeight;
            fRectTmp.bottom = (float)m_rect.bottom;
	    ...
	    pVert[vnum].tu = texRectTmp.left;     pVert[vnum].tv = texRectTmp.top;
	    pVert[vnum+1].tu = texRectTmp.right;  pVert[vnum+1].tv = texRectTmp.top;
	    pVert[vnum+2].tu = texRectTmp.left;   pVert[vnum+2].tv = texRectTmp.bottom;
	    pVert[vnum+3].tu = texRectTmp.right;  pVert[vnum+3].tv = texRectTmp.bottom;

        pVert[vnum].pos.x = fRectTmp.left;    pVert[vnum].pos.y = fRectTmp.top;
        pVert[vnum+1].pos.x = fRectTmp.right; pVert[vnum+1].pos.y = fRectTmp.top;
        pVert[vnum+2].pos.x = fRectTmp.left;  pVert[vnum+2].pos.y = fRectTmp.bottom;
        pVert[vnum+3].pos.x = fRectTmp.right; pVert[vnum+3].pos.y = fRectTmp.bottom;
	...
	for(i=0; i<m_nHorzLineQuantity*2+m_nVertLineQuantity*2; i++)
	{
        ...
        {
		    pVert[vnum].tu = tmpRect1.left;       pVert[vnum].tv = tmpRect1.top;
		    pVert[vnum+1].tu = tmpRect1.right;    pVert[vnum+1].tv = tmpRect1.top;
        	pVert[vnum+2].tu = tmpRect1.left;     pVert[vnum+2].tv = tmpRect1.bottom;
        	pVert[vnum+3].tu = tmpRect1.right;    pVert[vnum+3].tv = tmpRect1.bottom;
        }
        else
        {
        	pVert[vnum].tu = tmpRect2.left;       pVert[vnum].tv = tmpRect2.top;
        	pVert[vnum+1].tu = tmpRect2.right;    pVert[vnum+1].tv = tmpRect2.top;
        	pVert[vnum+2].tu = tmpRect2.left;     pVert[vnum+2].tv = tmpRect2.bottom;
		    pVert[vnum+3].tu = tmpRect2.right;    pVert[vnum+3].tv = tmpRect2.bottom;
        }
        ...
        if(lineType<2)
        {
		    pVert[vnum].pos.x = fRectTmp.left;      pVert[vnum].pos.y = fRectTmp.top;
		    vnum++;
		    pVert[vnum].pos.x = fRectTmp.right; 	pVert[vnum].pos.y = fRectTmp.top;
		    vnum++;
		    pVert[vnum].pos.x = fRectTmp.left;	    pVert[vnum].pos.y = fRectTmp.bottom;
		    vnum++;
		    pVert[vnum].pos.x = fRectTmp.right;     pVert[vnum].pos.y = fRectTmp.bottom;
		    vnum++;
        }
        else
        {
