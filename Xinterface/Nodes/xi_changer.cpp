...
bool CXI_CHANGER::Init(INIFILE *ini1,char *name1, INIFILE *ini2,char *name2, VDX9RENDER *rs, XYRECT &hostRect, XYPOINT &ScreenSize)
{
...
void CXI_CHANGER::LoadIni(INIFILE *ini1,char *name1, INIFILE *ini2,char *name2)
{
	...
	if(m_fCurM_UpSpeed>0.1f)	m_fCurM_UpSpeed = 0.001f/m_fCurM_UpSpeed;
	else m_fCurM_UpSpeed = 1.f;
	if(m_fCurM_DownSpeed>0.1f)	m_fCurM_DownSpeed = 0.001f/m_fCurM_DownSpeed;
	else m_fCurM_DownSpeed = 1.f;
...
int CXI_CHANGER::CommandExecute(int wActCode)
{
	if(m_bUse)
	{
		...
		case ACTION_SPEEDUP:
            bChangePosition = true;
            if(m_nCurrentPos>0)	m_nCurrentPos--;
			else	m_nCurrentPos = m_nPlaceQuantity-1;
            break;
		case ACTION_DOWNSTEP:
		case ACTION_SPEEDDOWN:
            bChangePosition = true;
            if(m_nCurrentPos<m_nPlaceQuantity-1)	m_nCurrentPos++;
			else	m_nCurrentPos = 0;
			break;
		}
...
void CXI_CHANGER::SetRectanglesToPosition(int nPos)
{
    ...
    m_pTexVert[4].pos.x = left-m_xOffset;    m_pTexVert[4].pos.y = top-m_yOffset;
    m_pTexVert[5].pos.x = right+m_xOffset;   m_pTexVert[5].pos.y = top-m_yOffset;
    m_pTexVert[6].pos.x = left-m_xOffset;    m_pTexVert[6].pos.y = bottom+m_yOffset;
    m_pTexVert[7].pos.x = right+m_xOffset;   m_pTexVert[7].pos.y = bottom+m_yOffset;
    // set front rectangle position
    m_pTexVert[0].pos.x = left;     m_pTexVert[0].pos.y = top;
    m_pTexVert[1].pos.x = right;    m_pTexVert[1].pos.y = top;
    m_pTexVert[2].pos.x = left;     m_pTexVert[2].pos.y = bottom;
    m_pTexVert[3].pos.x = right;    m_pTexVert[3].pos.y = bottom;
}
