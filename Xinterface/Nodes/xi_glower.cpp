	...
	for(long i=0; i<m_nQuantity; i++)
	{
		if(m_glows[i].action!=GLOW_ACTION_NONE)
		{
			...
			if(m_glows[i].action == GLOW_ACTION_COLORUP)
				m_glows[i].rect.dwColor = ColorInterpolate(m_dwMaxColor,m_dwMinColor,(float)m_glows[i].curTime/m_glows[i].allTime);
			if(m_glows[i].action == GLOW_ACTION_BLEND)
				m_glows[i].rect.dwColor = ColorInterpolate(m_dwMaxColor,m_dwMinColor,1.f-(float)m_glows[i].curTime/m_glows[i].allTime);
			// show this rectangle
			m_rs->DrawRects(&m_glows[i].rect,1,"iGlow");

			m_glows[i].rect.fAngle += m_glows[i].angleSpeed*Delta_Time;
			while(m_glows[i].rect.fAngle>2.f*PI) m_glows[i].rect.fAngle-=2.f*PI;
			if( (m_glows[i].curTime-=Delta_Time)<0 )
			{
				...
				case GLOW_ACTION_COLORUP:
					...
					m_glows[i].curTime = m_glows[i].allTime = long(m_minShowTime+(m_maxShowTime-m_minShowTime)*rand()/RAND_MAX);
					break;
				case GLOW_ACTION_SHOW:
					m_glows[i].action = GLOW_ACTION_BLEND;
					m_glows[i].curTime = m_glows[i].allTime = long(m_minGlowTime+(float)(m_maxGlowTime-m_minGlowTime)*rand()/RAND_MAX);
					break;
...
bool CXI_GLOWER::Init(INIFILE *ini1,char *name1, INIFILE *ini2,char *name2, VDX9RENDER *rs, XYRECT &hostRect, XYPOINT &ScreenSize)
{
...
void CXI_GLOWER::LoadIni(INIFILE *ini1,char *name1, INIFILE *ini2,char *name2)
{
	...
	if(ini1->ReadString(name1,"pos",param,sizeof(param)-1,""))
		do m_nQuantity++; while(ini1->ReadStringNext(name1,"pos",param,sizeof(param)-1));
