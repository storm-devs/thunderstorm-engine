...
bool CXI_RECTANGLE::Init(INIFILE *ini1,char *name1, INIFILE *ini2,char *name2, VDX9RENDER *rs, XYRECT &hostRect, XYPOINT &ScreenSize)
{
...
bool CXI_RECTANGLE::IsClick(int buttonID,long xPos,long yPos)
{
	...
	if(	xPos>=m_rect.left && xPos<=m_rect.right &&
		yPos>=m_rect.top && yPos<=m_rect.bottom ) return true;
	return false;
}
