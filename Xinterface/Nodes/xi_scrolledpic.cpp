				...
				if( pA )
				{
					...
					if( pcGroupName )
					{
						...
						if( m_aImg[i].fpSize.x > 0.f && m_aImg[i].fpSize.y > 0.f )
							m_aImg[i].pImg->SetSize( (long)m_aImg[i].fpSize.x, (long)m_aImg[i].fpSize.y );
					}
					else
					{
						m_aImg[i].pImg->LoadFromFile( pA->GetAttribute( "file" ) );
					}
					m_aImg[i].fpSize.x = (float)m_aImg[i].pImg->GetWidth();
					m_aImg[i].fpSize.y = (float)m_aImg[i].pImg->GetHeight();
				}
			}
		}
...
void CXI_SCROLLEDPICTURE::ChangeUV( FXYRECT &frNewUV )
{
	...
	if( pAttribute ) {
		...
		if( pA ) {
			pA->SetAttributeUseFloat( "x", (frNewUV.right-frNewUV.left) * m_fpBaseSize.x / (float)(m_rect.right-m_rect.left) );
			pA->SetAttributeUseFloat( "y", (frNewUV.bottom-frNewUV.top) * m_fpBaseSize.y / (float)(m_rect.bottom-m_rect.top) );
		}
	}
}
...
{
	if( m_rect.right - m_rect.left <= 0 ) m_fUTexPerPixel = 0.f;
	else m_fUTexPerPixel = (m_v[3].tu - m_v[0].tu) / (m_rect.right - m_rect.left);

	if( m_rect.bottom - m_rect.top <= 0 ) m_fVTexPerPixel = 0.f;
	else m_fVTexPerPixel = (m_v[3].tv - m_v[0].tv) / (m_rect.bottom - m_rect.top);
}
...
void CXI_SCROLLEDPICTURE::UpdateBuildenImages()
{
	for(long n=0; n<m_aImg.size(); n++ )
	{
		...
		m_aImg[n].pImg->SetPosition( (long)fx,(long)fy, IPType_Center );
		m_aImg[n].bShow = true;
	}
}
...
void CXI_SCROLLEDPICTURE::SetPosToCenter( float fX, float fY )
{
	...
	if( fTop <= fHeight * .5f ) fTop = 0.f;
	else fTop -= fHeight * .5f;
