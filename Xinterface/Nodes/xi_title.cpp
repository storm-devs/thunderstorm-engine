...
bool CXI_TITLE::Init(INIFILE *ini1,char *name1, INIFILE *ini2,char *name2, VDX9RENDER *rs, XYRECT &hostRect, XYPOINT &ScreenSize)
{
...
void CXI_TITLE::LoadIni(INIFILE *ini1,char *name1, INIFILE *ini2,char *name2)
{
	...
	pVert[0].pos.y = (float)m_rect.top;
	pVert[1].pos.x = float(m_StringCenter.x + m_nStringWidth/2);
	pVert[1].pos.y = (float)m_rect.top;
	pVert[2].pos.x = float(m_StringCenter.x - m_nStringWidth/2);
	pVert[2].pos.y = (float)m_rect.bottom;
	pVert[3].pos.x = float(m_StringCenter.x + m_nStringWidth/2);
	pVert[3].pos.y = (float)m_rect.bottom;
	pVert[0].tu = pVert[2].tu = centerRect.left;  pVert[1].tu = pVert[3].tu = centerRect.right;
	pVert[0].tv = pVert[1].tv = centerRect.top;   pVert[2].tv = pVert[3].tv = centerRect.bottom;
	...
	pVert[5].pos.x = pVert[7].pos.x = float(m_StringCenter.x - tmp1);
	pVert[8].pos.x = pVert[10].pos.x = float(m_StringCenter.x + tmp2);
	pVert[9].pos.x = pVert[11].pos.x = float(m_StringCenter.x + tmp1);
	...
	for(i=0; i<m_nTiledQuantity; i++)
	{
		...
		pVert[idx+0].pos.y = pVert[idx+1].pos.y = (float)m_rect.top;
	...
	xpos = float(m_StringCenter.x + tmp2);
	xposDelta = ((float)m_rect.right-xpos)/m_nTiledQuantity;
	for(i=0; i<m_nTiledQuantity; i++)
	{
		pVert[idx+0].pos.x = pVert[idx+2].pos.x = xpos + xposDelta;
		pVert[idx+1].pos.x = pVert[idx+3].pos.x = xpos;
